...
Процедура ПриОткрытии(Отказ)
	...
	// ++ 08.02.2022 Лыков А.А.
	Попытка
		
		Если ЗначениеЗаполнено(Объект.ИмяФайла) Тогда
			
			АдресВХранилище = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(Объект.ИмяФайла));
			ЗагрузитьЗапросыИзФайла(АдресВХранилище);
			
			МассивДляУдаления = Новый Массив;
			Для Каждого СтрокаЗапрос Из Объект.Запросы Цикл
				
				Если СтрокаЗапрос.Имя = "Новый запрос" Тогда
					МассивДляУдаления.Добавить(СтрокаЗапрос);
				КонецЕсли;
				
			КонецЦикла;
			Для Каждого СтрокаДляУдаления Из МассивДляУдаления Цикл
				Объект.Запросы.Удалить(СтрокаДляУдаления);
			КонецЦикла;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ПроисходитОчисткаФайловТЖ Тогда
		...
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , , "Объект");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьИзXML(Команда)
	...
	Если Найденные.Количество() = 0 Тогда
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выберите запрос.'"), , , "Объект");
		Возврат;
	КонецЕсли;
...
Процедура СчитатьПараметрыЗапроса(ТекстЗапроса, Удалять, ИдентификаторЗапроса)
	...
	Если ТипЗнч(РезультатСтруктура) = Тип("Строка") Тогда
		Спец_ОбщегоНазначения.СообщитьПользователю(РезультатСтруктура, , , "Объект.Параметры");
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьОшибкуПользователю(Знач ТекстОшибки)
	...
	Если Позиция > 0 Тогда
		...
		КонецЕсли;
	Иначе
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, , , "Объект");
	КонецЕсли;
...
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения)
	...
	Если ЗначениеЗаполнено(ОтчетПоВыполнениюЗапроса.ТекстСообщения) Тогда
		Спец_ОбщегоНазначения.СообщитьПользователю(ОтчетПоВыполнениюЗапроса.ТекстСообщения);
	КонецЕсли;
...
Процедура СохранитьФайлЗапросаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		КонецЕсли;
	Иначе
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Без расширения работы с файлами невозможно работать с файлами.'"), , , "Объект");
	КонецЕсли;
...
Процедура ЧтениеФайлаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Без расширения работы с файлами невозможно сохранять и загружать запросы в файл.'"), , , "Объект");
	КонецЕсли;
	...
	Для каждого СчитанныйПараметр Из РезультатСтруктура Цикл
		...
		Если Не НайденПараметр Тогда
			Если Не ФлагВыводаСообщения Тогда
				Спец_ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Найденные параметры были добавлены автоматически.'"), , , "Объект");
...
Процедура ЗаполнитьПараметрыИзXMLСервер(Знач СтрокаXML, Знач Контекст)
	...
	Исключение
		...
		Спец_ОбщегоНазначения.СообщитьПользователю(СтрШаблон(ТекстОшибки, Спец_ОбщегоНазначенияКлиентСервер.КраткоеПредставлениеОшибкиЧерезОбработкуОшибок(ИнформацияОбОшибке())));
		Возврат;
	КонецПопытки;
