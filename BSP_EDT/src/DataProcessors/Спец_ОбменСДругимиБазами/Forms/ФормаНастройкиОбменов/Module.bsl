...
Процедура ЗаполнитьТаблицыПоОбъектамНаКлиенте()
	...
	Иначе
		...
		КомпонентаПодключенияКБазеДанных = Спец_КомпонентаДополнительныеФункцииКлиентСерверПовтИсп.ПолучитьКомпонентуPostgreSQL(Ложь);
		Если КомпонентаПодключенияКБазеДанных <> Неопределено Тогда
			
			СоответствиеКэшДругой = Новый Соответствие;
			СоответствиеКэшДругой["КомпонентаPostgreSQL"] = КомпонентаПодключенияКБазеДанных;
			
			Если ТипБД = ПредопределенноеЗначение("Перечисление.Спец_ТипыБазыДанных.PostgreSQL") Тогда
				
				ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьТаблицыПоОбъектамНаКлиенте_ПослеПопыткиПодключенияКPostgreSQL", ЭтотОбъект);
				Спец_КомпонентаДополнительныеФункцииКлиент.PostgreSQLНачатьУстановкуСоединения(ОписаниеОповещения,
						СтрокаПодключенияКБД,
						СоответствиеКэшДругой);
				
			Иначе
				
				Элементы.ДекорацияСтраницаНастройкиОбменовНадписьПодключениеКБД.Заголовок = "Не заполнены настройки подключения для БД " + СокрЛП(ТипБД);
				
			КонецЕсли;
			
		Иначе
			
			Элементы.ДекорацияСтраницаНастройкиОбменовНадписьПодключениеКБД.Заголовок = "База данных " + СокрЛП(ТипБД) + " не поддерживается";
...
&НаКлиенте
Процедура ЗаполнитьТаблицыПоОбъектамНаКлиенте_ПослеПопыткиПодключенияКPostgreSQL(Знач Успешно, Знач ДополнительныеПараметры) Экспорт

	Если КомпонентаПодключенияКБазеДанных = Неопределено Тогда
		ВызватьИсключение "Не удалось кэшировать компоненту PostgreSQL";
	КонецЕсли;
	
	Если КомпонентаПодключенияКБазеДанных.СоединениеУстановлено Тогда
		
		Элементы.ДекорацияСтраницаНастройкиОбменовНадписьПодключениеКБД.Заголовок  = "Подключение к базе PostgreSQL установлено: имя базы " + КомпонентаПодключенияКБазеДанных.НаименованиеБазы;
