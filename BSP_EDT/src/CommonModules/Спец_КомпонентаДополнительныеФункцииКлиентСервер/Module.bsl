...
Процедура Пауза(Знач Миллисекунд = 500, Кэш = Неопределено) Экспорт
	...
	Если Кэш.КомпонентаBaseFunction = Неопределено Тогда
		...
		Если Спец_ОбщегоНазначенияКлиентСервер.ЭтоWindows() Тогда
			...
			ТекстКоманды = "powershell start-sleep -Seconds " + Формат(Миллисекунд / 1000, "ЧРД=.; ЧГ=");
...
Процедура BaseFunction_ПодключитьЕслиНеПодключали(Кэш) Экспорт
	...
	Если Кэш.Свойство("КомпонентаBaseFunction") И Не Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(Кэш.КомпонентаBaseFunction, "ТекстПоследнейОшибки") Тогда Кэш.Удалить("КомпонентаBaseFunction"); КонецЕсли;
	// -- Спец_БСП.Версия1С_20_Ниже
...
Процедура HTTPClient_ПодключитьЕслиНеПодключали(Кэш) Экспорт
	...
	// Причём на тип внешней компоненты не проверить, поэтому обращаемся к любому свойству.
	Если Кэш.Свойство("КомпонентаHTTPClient") И Не Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(Кэш.КомпонентаHTTPClient, "ТекстПоследнейОшибки") Тогда Кэш.Удалить("КомпонентаHTTPClient"); КонецЕсли;
	// -- Спец_БСП.Версия1С_20_Ниже
...
Процедура PostgreSQL_ПодключитьЕслиНеПодключали(Кэш, Знач БезИнициализацииПодключения = Ложь) Экспорт
	...
	// Причём на тип внешней компоненты не проверить, поэтому обращаемся к любому свойству.
	Если Кэш.Свойство("КомпонентаPostgreSQL") И Не Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(Кэш.КомпонентаPostgreSQL, "ТекстПоследнейОшибки") Тогда Кэш.Удалить("КомпонентаPostgreSQL"); КонецЕсли;
	// -- Спец_БСП.Версия1С_20_Ниже
