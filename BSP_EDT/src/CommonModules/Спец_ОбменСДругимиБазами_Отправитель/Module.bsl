...
Процедура ПередЗаписьюОбъекта(Знач Источник, Отказ, Знач ДополнительныеСвойства, Знач Кэш) Экспорт
	...
	Если Не ДополнительныеСвойства.ЭтоСсылочныйОбъект Тогда
		...
		Для Каждого ЭлементОтбора Из Источник.Отбор Цикл
			...
			Если Спец_ОбщегоНазначения.ЭтоСсылка(ЭлементОтбора.Значение, Кэш)
				И Спец_ОбщегоНазначения.ЭтоПустаяИлиБитаяСсылка(ЭлементОтбора.Значение, Кэш) Тогда
	...
	|	МАКСИМУМ(ПараметрыОбменовСДругимиБазами.ПолноеИмяОбъектаМетаданныхИсточник) КАК ПолноеИмяОбъектаМетаданныхИсточник,
	|	МАКСИМУМ(ПараметрыОбменовСДругимиБазами.РегистрироватьСОбменомДанными) КАК РегистрироватьСОбменомДанными,
	|	МАКСИМУМ(ПараметрыОбменовСДругимиБазами.ИмяРеквизитаДляПроверкиВКлючевыхОбъектах) КАК ИмяРеквизитаДляПроверкиВКлючевыхОбъектах
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ПараметрыОбменовСДругимиБазами.База
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ПолноеИмяОбъектаМетаданныхИсточник", ДополнительныеСвойства.ПолноеИмяМетаданных);
	...
	ВыборкаРезультат = ПакетыЗапросов[ВГраница - 1].Выбрать();
	Пока ВыборкаРезультат.Следующий() Цикл
		
		РежимРегистрации = ПередЗаписьюОбъекта_РежимРегистрацииОбъекта(ВыборкаРезультат,
				Источник,
		...
		Если РежимРегистрации = 0 Тогда
			...
			ДополнительныеСвойства.МассивБазДляОбменаОтправитель.Добавить(ВыборкаРезультат.База);
...
Процедура ПриЗаписиОбъекта(Знач Источник, Отказ, Знач ДополнительныеСвойства, Знач Кэш) Экспорт
	...
	Если Спец_РаботаСКоллекциямиКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "ЭтоУдалениеОбъекта", Ложь) Тогда
	...
	|	РегистрСведений.Спец_ПараметрыОбменовСДругимиБазами КАК ПараметрыОбменовСДругимиБазами
	|ГДЕ
	|	ПараметрыОбменовСДругимиБазами.База В(&МассивБазДляОбмена)
	|	И ПараметрыОбменовСДругимиБазами.ПолноеИмяОбъектаМетаданныхИсточник = &ПолноеИмяОбъектаМетаданныхИсточник
	|	И ПараметрыОбменовСДругимиБазами.ОбменВключен
	|	И ПараметрыОбменовСДругимиБазами.ТекстЗапросаИсключения_Отправитель <> """"";
	
	Запрос.УстановитьПараметр("ПолноеИмяОбъектаМетаданныхИсточник", ДополнительныеСвойства.ПолноеИмяМетаданных);
	Запрос.УстановитьПараметр("МассивБазДляОбмена", ДополнительныеСвойства.МассивБазДляОбменаОтправитель);
	...
	Пока ВыборкаРезультат.Следующий() Цикл
		...
		Если Не ДополнительныеСвойства.ЭтоСсылочныйОбъект Тогда
			...
			Если Спец_СтроковыеФункцииКлиентСервер.ЕстьКакоеЛибоУсловиеВЗапросе(ЗапросДляПроверки.Текст) Тогда
				ЗапросДляПроверки.Текст = ЗапросДляПроверки.Текст + " И " + ВыборкаРезультат.ТекстЗапросаИсключения_Отправитель;
			Иначе
				ЗапросДляПроверки.Текст = ЗапросДляПроверки.Текст + " ГДЕ " + ВыборкаРезультат.ТекстЗапросаИсключения_Отправитель;
			КонецЕсли;
		...
		Иначе
			...
			ЗапросДляПроверки.Текст = СтрШаблон(
					...
					ВыборкаРезультат.ТекстЗапросаИсключения_Отправитель);
			
			ЗапросДляПроверки.УстановитьПараметр("Ссылка", Источник.Ссылка);
			
		КонецЕсли;
		
		СтруктураПараметров = Новый Структура("База", ВыборкаРезультат.База);
...
Функция ОбработатьИзмененияВТекущейБазе_СсылочныеОбъекты_ОбработатьПакет_ЗаписатьНапрямую_PostgreSQL_ПоместитьСсылкиВоВременнуюТаблицу(Знач МассивНормальныхСсылок,
	...
	Успешно = Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLВыполнитьЗапрос(ТекстЗапроса, 0, , КэшОтправитель);
...
Функция ОбработатьИзмененияВТекущейБазе_Регистры_ОбработатьПакет_ЗаписатьНапрямую_PostgreSQL_ПоместитьОтборыВоВременнуюТаблицу(Знач ПолноеИмяМетаданных, Знач ИмяРеквизита,
	...
	Иначе // Примитивный тип
		...
		Если Успешно <> Истина Тогда
			...
			ОбработатьИзмененияВТекущейБазе_ЗаписатьНапрямую_PostgreSQL_ОбработатьОшибки("",
					"Не удалось создать временную таблицу с примитивными значениями: " + Успешно.ТекстОшибки,
					КэшОтправитель,
					КэшПолучатель,
					ТекстыОшибок);
...
Процедура ОбработатьИзмененияВТекущейБазе_ЗаписатьНапрямую_PostgreSQL_ВыполнитьЗапрос(Знач ТекстЗапроса, Знач СтруктураПараметровДляЗапроса, Знач КэшДляВыполнения)
	...
	Успешно = Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLВыполнитьЗапрос(ТекстЗапроса, 0, , КэшДляВыполнения);
	Если Успешно <> Истина Тогда
		ВызватьИсключение Успешно.ТекстОшибки;
	КонецЕсли;
	...
	Если ПоляСтолбца.Количество() <> 1 Тогда
		...
		ИначеЕсли ТипСтолбца = "4" Тогда // Дата
			...
			Возврат ?(ЗначениеДата = Неопределено,
					Дата(1, 1, 1),
					Спец_СтроковыеФункцииКлиентСервер.СтрокаВДату(ЗначениеДата, ЧастиДаты.ДатаВремя));
		...
		ИначеЕсли ТипСтолбца = "8" Тогда // Ссылочный
			...
			НаименованиеТаблицыВ1С = Спец_ОбщегоНазначения.ПолучитьСтруктураПервойВыборкиЗапроса(Запрос, "ПолноеИмяОбъектаМетаданныхИсточник");
...
Процедура ИнициализироватьЧтениеФайловFTPУБазыПолучателя(Знач ИдентификаторЗадания, Знач СтруктураПараметров, Кэш, ТекстыОшибок)
	...
	СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктураЗапроса("hs/specialExchange/changes",
...
Функция ОбновитьПараметрыОбменовПоОбъекту_СформироватьТекстЗапросаДляОбменов(Знач ДляБазыОтправителя, Знач БазаПолучатель, Знач СтрокаПараметрДляОбработки, Кэш)
	...
	|ГДЕ
	|	СоответствиеОбъектовДляОбмена.База = &База
	|	И СоответствиеОбъектовДляОбмена.ИмяОбъектаМетаданныхИсточник = &ИмяОбъектаМетаданныхИсточник";
	
	Запрос.УстановитьПараметр("База", БазаПолучатель);
	Запрос.УстановитьПараметр("ИмяОбъектаМетаданныхИсточник", СтрокаПараметрДляОбработки.ПолноеИмяОбъектаМетаданныхИсточник);
	
	ТаблицаСоответствиеСтрок = Запрос.Выполнить().Выгрузить();
...
Функция ОбновитьПараметрыОбменовПоОбъекту_СформироватьТекстЗапросаДляЗаписиНапрямую_Отправитель(Знач СтрокаПараметрДляОбработки, Знач БазаПолучатель, ТекстыОшибок, Кэш)
	...
	Если ТаблицаСтруктураИзДругойБазы.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ТаблицаОбщиеПараметрыОбменов.Количество() = 0 Или ТаблицаСтруктураИзДругойБазы.Количество() = 0 Или ТаблицаСоответствиеОбъектов.Количество() = 0 Тогда
	...
	МассивТабличныхЧастей = Спец_РаботаСКоллекциямиКлиентСервер.СортироватьМассив(ТаблицаСоответствиеОбъектов.Скопировать(СтруктураОтбора).ВыгрузитьКолонку("ИмяТабличнойЧастиИсточник"), , Истина);
...
Процедура ПодготовитьДополнительныеПараметры(СтруктураПараметров, ТекстыОшибок, Кэш, Знач ИмяФоновогоЗадания) Экспорт
	...
	|ИНДЕКСИРОВАТЬ ПО
	|	ПараметрыОбменовСДругимиБазами.База,
	|	ПараметрыОбменовСДругимиБазами.ПолноеИмяОбъектаМетаданныхИсточник,
	...
	|			И СоответствиеОбъектовДляОбмена.ИмяОбъектаМетаданныхИсточник = втПараметрыДляОбмена.ПолноеИмяОбъектаМетаданныхИсточник
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СоответствиеОбъектовДляОбмена.ИмяОбъектаМетаданныхИсточник,
	|	СоответствиеОбъектовДляОбмена.ИмяТабличнойЧастиИсточник,
	|	СоответствиеОбъектовДляОбмена.ИмяРеквизитаИсточник";
...
Функция ЗарегистрироватьЗапросКОтправке(СтруктураПараметров, Знач ИдентификаторЗапроса, Знач АдресЗапроса, Знач МетодЗапроса, ДанныеДляОтправки,
	...
	Иначе
		...
		СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктураЗапроса("hs/specialExchange" + АдресЗапроса,
...
Процедура ИнициализироватьДополнительныеФункцииДляБД_PostrgeSQL(Знач База, Знач КэшДругойБазы, ТекстыОшибок, Кэш)
	...
	РезультатВыполнения = Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLВыполнитьЗапрос(ТекстЗапроса, 0, , Кэш);
	Если РезультатВыполнения <> Истина Тогда
		ВызватьИсключение РезультатВыполнения.ТекстОшибки;
	КонецЕсли;
	...
	РезультатВыполнения = Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLВыполнитьЗапрос(ТекстЗапроса, 0, , КэшДругойБазы);
	Если РезультатВыполнения <> Истина Тогда
		ВызватьИсключение РезультатВыполнения.ТекстОшибки;
	КонецЕсли;
