...
Процедура НачатьПолучениеОбъектаДрайвера(ОповещениеПриЗавершении, ПараметрыПодключения) Экспорт
	...
	Если ОбъектДрайвера = Неопределено Тогда
		Если ПараметрыПодключения.ПодключениеИзМакета Тогда    
			...
			ПодключитьКомпонентуИзМакета(Оповещение, ПараметрыПодключения.ИдентификаторОбъекта, ПараметрыПодключения.ИмяМакетаДрайвера, Параметры);
		ИначеЕсли ПараметрыПодключения.ПодключениеЛокальноПоИдентификатору Тогда
			ПодключитьКомпонентуИзРеестраWindows(Оповещение,  ПараметрыПодключения.ИдентификаторОбъекта);
		Иначе
			Параметры = ПараметрыПодключения();
			Параметры.ТекстПояснения = ТекстСообщения;
			Параметры.Кэшировать = Ложь;
			ПодключитьКомпоненту(Оповещение,  ПараметрыПодключения.ИдентификаторОбъекта, ПараметрыПодключения.ВерсияДрайвера, Параметры);
		КонецЕсли;
	КонецЕсли;   
...
Процедура НачатьПолучениеОписанияДрайвера_ПолучитьНомерВерсииЗавершение(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Ошибка получения параметров драйвера.'") + Символы.ПС + ОписаниеОшибки);
	КонецПопытки;
...
Процедура НачатьПолучениеОписанияДрайвера_ПолучитьПараметрыЗавершение(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Исключение
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Ошибка получения описания драйвера.'"));
	КонецПопытки;
...
Функция ОбработатьСобытие(ОбъектДрайвера, ПараметрыПодключения, Событие, Данные) Экспорт
	...
	ИначеЕсли Событие = "ДанныеКарты" Или Событие = "TracksData" Тогда
		...
		Если Параметры.ПараметрыДорожек <> Неопределено 
			...
			МодульОборудованиеУстройстваВводаКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеУстройстваВводаКлиент");
...
#Область ДляРаботоспособностиБезБСП
