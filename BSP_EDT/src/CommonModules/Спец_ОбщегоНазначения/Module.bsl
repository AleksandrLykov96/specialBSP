...
Функция ПолучитьКлючЗаписиРегистраПоСтруктуреЗначений(Знач ОбъектМетаданныхИлиПолноеИмя, Знач СтруктураЗначений, Знач Кэш = Неопределено) Экспорт
...
//  Наименование - Строка - Наименование константы.
...
Функция ПолучитьКонстантуСпец(Знач КодИлиСсылкаПВХ, Знач ЗначениеПоУмолчанию = Неопределено, Знач Наименование = "", Знач НаДату = Неопределено) Экспорт
	...
	Возврат ?(ТипЗнч(КодИлиСсылкаПВХ) = Тип("ПланВидовХарактеристикСсылка.Спец_Константы"),
			ПолучитьКонстантуСпецПоПВХ(КодИлиСсылкаПВХ, ЗначениеПоУмолчанию, НаДату),
			ПолучитьКонстантуСпецПоКоду(КодИлиСсылкаПВХ, ЗначениеПоУмолчанию, Наименование, НаДату));
...
Процедура УстановитьКонстантуСпец(Знач КодИлиСсылкаПВХ, Знач Значение, Знач ХранитьИсторию = Истина) Экспорт
	...
	ПВХ = ?(ТипЗнч(КодИлиСсылкаПВХ) = Тип("ПланВидовХарактеристикСсылка.Спец_Константы"),
			КодИлиСсылкаПВХ,
			ПолучитьКонстантуСпецПоКоду(КодИлиСсылкаПВХ, Значение, КодИлиСсылкаПВХ, ПериодКонстанты, Истина));
	
	ПолучитьКонстантуСпецЗаписатьЗначениеПоПВХ(ПВХ, Значение, ПериодКонстанты, ТипЗнч(Значение));
...
Процедура ОбновлениеКонфигурации(Знач ИдентификаторЗадания = "") Экспорт
	...
	ПроверитьСоздатьПредопределенныеОбъекты(Ложь, "", ОбщийКэш);
	...
	Спец_МенеджерОборудования.ОбновитьПоставляемыеДрайвера(ОбщийКэш);
	// -- Спец_БСП.Спец_ПодключаемоеОборудование
...
// Метод создаёт и проверяет наличие и создаёт все предопределённые объекты в конфигурации.
// 
// Параметры:
//  ПроверятьВсе - Булево - Если ИСТИНА, то проверяет все объекты конфигурации, иначе только те, которые относятся СПЕЦ подсистемам.
//  ТекстыОшибок - Строка - Тексты ошибок
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
Процедура ПроверитьСоздатьПредопределенныеОбъекты(Знач ПроверятьВсе = Истина, ТекстыОшибок = "", Знач Кэш = Неопределено) Экспорт
	...
	Если ПроверятьВсе Тогда
		...
		МассивИменМетаданныхДляПроверки = Спец_РаботаСКоллекциямиКлиентСервер.СкопироватьМассив(Спец_ОбщегоНазначенияПовтИсп.ВсеИменаМетаданныхИзБазы(Истина), Ложь); // Массив из Строка
		
	Иначе
		
		МассивИменМетаданныхДляПроверки = Новый Массив; // Массив из Строка
		
		МассивИменМетаданныхДляПроверки.Добавить(Метаданные.Справочники.Спец_Базы.ПолноеИмя());
		МассивИменМетаданныхДляПроверки.Добавить(Метаданные.ПланыВидовХарактеристик.Спец_Константы.ПолноеИмя());
	...
	Для Каждого ИмяМетаданных Из МассивИменМетаданныхДляПроверки Цикл
		
		МассивИменПредопределенных = Спец_ОбщегоНазначенияПовтИсп.МассивИменПредопределенныхПоМетаданным(ИмяМетаданных);
		Если МассивИменПредопределенных.Количество() > 0 Тогда
			МассивТекстЗапроса.Добавить(СтрШаблон(ШаблонТекстаЗапроса, ИмяМетаданных));
		КонецЕсли;
	...
	Запрос.Текст = СтрШаблон(
			...
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ГотовыеДанные.ПолноеИмяМетаданных КАК ПолноеИмяМетаданных,
			|	ГотовыеДанные.ИмяПредопределенныхДанных КАК ИмяПредопределенныхДанных
			|ИЗ
			|	(%1) КАК ГотовыеДанные",
			
			СтрСоединить(МассивТекстЗапроса, Спец_СтроковыеФункцииКлиентСервер.РазделительОбъединяющихЗапросов()));
	
	ТаблицаРезультат = Запрос.Выполнить().Выгрузить();
	ТаблицаРезультат.Индексы.Добавить("ПолноеИмяМетаданных");
	
	Для Каждого ИмяМетаданных Из МассивИменМетаданныхДляПроверки Цикл
		
		МассивИменПредопределенныхДолжноБыть = Спец_ОбщегоНазначенияПовтИсп.МассивИменПредопределенныхПоМетаданным(ИмяМетаданных);
		МассивИменПредопределенныхСуществует = ТаблицаРезультат.Скопировать(Новый Структура("ПолноеИмяМетаданных", ИмяМетаданных)).ВыгрузитьКолонку("ИмяПредопределенныхДанных");
		
		МассивРазность = Спец_РаботаСКоллекциямиКлиентСервер.РазностьОбъектов(МассивИменПредопределенныхДолжноБыть,
				МассивИменПредопределенныхСуществует,
				Спец_ОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока(150)); // Массив из Строка
		
		Если МассивРазность.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураПоМетаданным = ПолучитьСтруктуруИмениТипаДанныхОбъекта(ИмяМетаданных, Кэш);
		Для Каждого ИмяПредопределенного Из МассивРазность Цикл
			
			ЭлементОбъект = СтруктураПоМетаданным.МенеджерОбъекта.СоздатьЭлемент(); // см. ЗаписатьОбъект.ОбъектДляЗаписи
			ЭлементОбъект.Наименование              = ИмяПредопределенного;
			ЭлементОбъект.ИмяПредопределенныхДанных = ИмяПредопределенного;
			...
			ЗаписатьОбъект(ЭлементОбъект, , ТекстыОшибок, Ложь, Кэш);
...
// Возвращаемое значение:
//  Структура:
//	* Отправитель - СправочникСсылка - Если передан заголовок UID, то определяет магазин или организацию отправителя HTTP запроса.
// 	* База - Неопределено, СправочникСсылка.Спец_Базы, Строка - Если передан заголовок Database, то определяет базу-отправителя HTTP запроса.
// 	* УровеньБазы - ПеречислениеСсылка.УровеньБазы - Если передан заголовок UB, то определяет уровень базы-отправителя HTTP запроса. По умолчанию ГлавныйУзел
// 	* РаботаетВБазеРайона - Булево - Если передан заголовок RVR, то определяет, работает ли аптека-отправитель в базе района.
...
Функция ПолучитьСтруктуруВходящегоПотокаHTTPСервисЗапроса(Знач HTTPСервисЗапрос) Экспорт
	...
	// Отправитель
	UID = Спец_РаботаСКоллекциямиКлиентСервер.СвойствоСоответствия(HTTPСервисЗапрос.Заголовки, "UID", ""); // Строка
	МассивГУИДов = СтрРазделить(UID, ";", Ложь);
	
	Если МассивГУИДов.Количество() > 0
		И Метаданные.Справочники.Найти("Магазины") <> Неопределено
		И Метаданные.Справочники.Найти("Организации") <> Неопределено Тогда
		...
		Запрос.Текст =
			...
			|ИЗ
			|	(ВЫБРАТЬ
			|		Магазины.Ссылка КАК Отправитель
			|	ИЗ
			|		Справочник.Магазины КАК Магазины
			|	ГДЕ
			|		Магазины.ГУИД В(&МассивГУИДов)
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			...
			|	ГДЕ
			|		Организации.ГУИД В(&МассивГУИДов)) КАК Отправители";
		
		Запрос.УстановитьПараметр("МассивГУИДов", МассивГУИДов);
		
		МассивОтправителей = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Отправитель");
		Если МассивОтправителей.Количество() = 1 Тогда
			СтруктураПотока.Отправитель = МассивОтправителей.Получить(0);
		ИначеЕсли МассивОтправителей.Количество() > 1 Тогда
			СтруктураПотока.Отправитель = МассивОтправителей;
		КонецЕсли;
		
	КонецЕсли;
	
	// Имя базы
	Database = Спец_РаботаСКоллекциямиКлиентСервер.СвойствоСоответствия(HTTPСервисЗапрос.Заголовки, "Database", ""); // Строка
	Если Не ПустаяСтрока(Database) И Спец_ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("Спец_БСП.Спец_ОбменыСДругимиБазами") Тогда
		База = Справочники.Спец_Базы.НайтиПоКоду(Database, Истина);
	КонецЕсли;
	
	СтруктураПотока.База = ?(База = Неопределено, Database, База);
	
	// Уровень базы
	UB = Спец_РаботаСКоллекциямиКлиентСервер.СвойствоСоответствия(HTTPСервисЗапрос.Заголовки, "UB", ""); // Строка
	Если Не ПустаяСтрока(UB) Тогда
		СтруктураПотока.УровеньБазы = Перечисления.УровеньБазы.Получить(Спец_СтроковыеФункцииКлиентСервер.СтрокаВЧисло(UB));
	КонецЕсли;
	
	// Работает в базе района
	RVR = Спец_РаботаСКоллекциямиКлиентСервер.СвойствоСоответствия(HTTPСервисЗапрос.Заголовки, "RVR", ""); // Строка
	СтруктураПотока.РаботаетВБазеРайона = RVR = "1" Или RVR = 1;
...
// Возвращаемое значение:
//  Тип, Неопределено - 
Функция ПолучитьТипРеквизитаВ1С(Знач ОбъектМетаданныхИлиПолноеИмя, Знач ИмяРеквизита, Знач Кэш = Неопределено) Экспорт
...
Функция ПолучитьСтруктуруИмениТипаДанныхОбъекта(Знач Объект, Знач Кэш = Неопределено) Экспорт
...
// Возвращаемое значение:
//  Строка
Функция ПолучитьИмяМетаданныхПоТипу(Знач ТипДляПроверки, Знач Кэш = Неопределено) Экспорт
...
Функция ЕстьГУИДВРеквизитах(Знач ОбъектМетаданныхИлиПолноеИмя, Знач Кэш = Неопределено) Экспорт
...
//						- Массив из см. ПолучитьИмяТипаМетаданныхРеквизита.МетаданныеРеквизит - Если ВозвращатьИмена = ЛОЖЬ
Функция ПолучитьСтруктуруВсехРеквизитовОбъекта(Знач ОбъектМетаданныхИлиПолноеИмя, Знач ВозвращатьИмена = Истина, Знач Кэш = Неопределено) Экспорт
...
// Возвращает метаданные реквизита по именам объекта и реквизита в 1С.
// 
// Параметры:
//  ОбъектМетаданныхИлиПолноеИмя - см. ПолучитьТипРеквизитаВ1С.ОбъектМетаданныхИлиПолноеИмя
//  ИмяРеквизита - Строка
...
Функция ПолучитьМетаданныеРеквизитаПоИмени(Знач ОбъектМетаданныхИлиПолноеИмя, Знач ИмяРеквизита, Знач Кэш = Неопределено) Экспорт
...
// Возвращаемое значение:
//  Массив из Строка
Функция ПолучитьМассивИменРеквизитовДляУникальностиЗаписиРегистра(Знач ОбъектМетаданныхИлиПолноеИмя, Знач Кэш = Неопределено) Экспорт
...
Функция ПолучитьМассивРеквизитовГдеУчаствуетОбъект(Знач ОбъектМетаданныхИлиПолноеИмя, Знач Кэш = Неопределено) Экспорт
...
// Возвращаемое значение:
//  Строка
Функция ПолучитьИмяТипаМетаданныхРеквизита(Знач МетаданныеРеквизит, Знач Кэш = Неопределено) Экспорт
...
// Возвращаемое значение:
//  Массив из Строка
Функция ПолучитьМассивИменТабличныхЧастей(Знач ОбъектМетаданныхИлиПолноеИмя, Знач Кэш = Неопределено) Экспорт
...
Функция ПолучитьОбъектСБлокированием(Знач СсылкаНаОбъект, Знач Кэш = Неопределено, ТекстОшибки = "") Экспорт
...
Функция ЗаписатьОбъект(Знач ОбъектДляЗаписи, Знач ПараметрЗаписи = Неопределено, ТекстОшибки = "", Отказ = Ложь,
		Знач Кэш = Неопределено, Знач КоличествоОграничениеПопытокЗаписи = 1, Знач СтруктураРеквизитовОтбор = Неопределено, Знач ПроверятьМодифицированность = Истина) Экспорт
...
Функция УдалитьДублиПоСсылке(Знач НормальнаяСсылка, Знач МассивДублейСсылки, Знач Кэш = Неопределено, Знач МожноЧерезБД = Ложь, ТекстыОшибок = "") Экспорт
...
Функция ОбъектМодифицирован(Знач ОбъектДляПроверки, Знач СтруктураРеквизитовОтбор = Неопределено, Знач Кэш = Неопределено) Экспорт
	
	Если Кэш = Неопределено Тогда Кэш = Спец_ПолучитьКэш(); КонецЕсли;
	
	МетаданныеОбъекта = ОбъектДляПроверки.Метаданные();
	Если Спец_ОбработчикиПодписокНаСобытия.ПропуститьВсеПроверкиПоОбъекту(МетаданныеОбъекта, Кэш) Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ПолучитьСтруктуруИлиТаблицуОбъектаДоЗаписи(Знач ОбъектДляПроверки, Знач ОбъектМетаданных, Знач Кэш = Неопределено) Экспорт
	...
	Если ЭтоСсылка(ОбъектДляПроверки, Кэш, ТипОбъекта) Тогда
		...
		Возврат ПолучитьСтруктураПервойВыборкиЗапроса(Запрос);
...
Функция ЗначенияРеквизитовОбъекта(Знач СсылкаИлиОбъект, Знач ИменаРеквизитов, Знач ВыбратьРазрешенные = Ложь, Знач Кэш = Неопределено) Экспорт
...
Функция ЗначениеРеквизитаОбъекта(Знач СсылкаИлиОбъект, Знач ИмяРеквизита, Знач ВыбратьРазрешенные = Ложь, Знач Кэш = Неопределено) Экспорт
...
Функция ЗначенияРеквизитовОбъектов(Знач МассивСсылокИлиОбъектов, Знач ИменаРеквизитов, Знач ВыбратьРазрешенные = Ложь, Знач Кэш = Неопределено) Экспорт
	...
	Запрос = Новый Запрос(СтрШаблон(
			...
			СтрСоединить(МассивТекстыЗапросов, Спец_СтроковыеФункцииКлиентСервер.РазделительОбъединяющихЗапросов())));
...
Функция ЗначениеРеквизитаОбъектов(Знач МассивСсылокИлиОбъектов, Знач ИмяРеквизита, Знач ВыбратьРазрешенные = Ложь, Знач Кэш = Неопределено) Экспорт
...
Функция НайтиПоСсылкамПрограммно(Знач МассивОбъектовДляПоиска, Знач МассивОбъектовМетаданныхИлиПолныхИмен = Неопределено, Знач МассивМетаданныхИлиИменДляИсключения = Неопределено,
		...
		Знач ВключитьУпорядочиваниеДляСсылочных = Ложь, Знач Кэш = Неопределено) Экспорт
	...
	Для Каждого ПакетПоМетаданным Из МассивПоПакетам Цикл // Массив из см. ПолучитьСтруктуруИлиТаблицуОбъектаДоЗаписи.ОбъектМетаданныхИлиПолноеИмя
		...
		Если МассивТекстыСсылочныхЗапросов.Количество() > 0 Тогда
			...
			Если ВозвращатьТолькоДанные Тогда
				...
				МассивТекстыЗапросов.Добавить(СтрШаблон(
						...
						Спец_СтроковыеФункцииКлиентСервер.СместитьВесьТекстНаРазделитель(СтрСоединить(МассивТекстыСсылочныхЗапросов, Спец_СтроковыеФункцииКлиентСервер.РазделительОбъединяющихЗапросов())),
		...
		Запрос = Новый Запрос(СтрСоединить(МассивТекстыЗапросов, Спец_СтроковыеФункцииКлиентСервер.ПолучитьРазделительЗапросов()));
...
// Возвращаемое значение:
//  Булево
Функция ЭтоПустаяИлиБитаяСсылка(Знач СсылкаДляПроверки, Знач Кэш = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(СсылкаДляПроверки) Тогда Возврат Истина; КонецЕсли; // Отсеиываем пустые ссылки
...
Функция ПолучитьСтруктураПервойВыборкиЗапроса(Знач ЗапросДляПолучения, Знач ИмяКолонки = "") Экспорт
...
// 
// Параметры:
//  пСтрокаТаблицыЗначений - СтрокаТаблицыЗначений, ВыборкаИзРезультатаЗапроса - Строка таблицы для преобразования
...
Функция ПреобразоватьСтрокуТаблицыЗначенийВСтруктуру(Знач пСтрокаТаблицыЗначений, Знач КолонкиТаблицыЗначений = Неопределено) Экспорт
	
	СтруктураРезультат = Новый Структура();
	Если КолонкиТаблицыЗначений = Неопределено Тогда КолонкиТаблицыЗначений = пСтрокаТаблицыЗначений.Владелец().Колонки; КонецЕсли;
	
	Для Каждого Колонка Из КолонкиТаблицыЗначений Цикл
		
		ТипЗначения = ТипЗнч(пСтрокаТаблицыЗначений[Колонка.Имя]);
		Если ТипЗначения = Тип("ТаблицаЗначений") Тогда
			
			//@skip-check invocation-parameter-type-intersect
			СтруктураРезультат.Вставить(Колонка.Имя, ПреобразоватьТаблицуЗначенийВМассивСтрок(пСтрокаТаблицыЗначений[Колонка.Имя]));
			
		ИначеЕсли ТипЗначения = Тип("РезультатЗапроса") Тогда
			
			МассивРезультат = Новый Массив; // Массив из Структура
			РезультатЗапроса = пСтрокаТаблицыЗначений[Колонка.Имя]; // РезультатЗапроса
		...
		Иначе
			...
			СтруктураРезультат.Вставить(Колонка.Имя, пСтрокаТаблицыЗначений[Колонка.Имя]);
...
// Параметры:
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
Процедура УстановитьКонстантыПоУмолчанию(Знач Кэш = Неопределено) Экспорт
	...
	// 2.1. HTTPЗапросы
	Спец_ПолучитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ГУИДТекущейБазы, "", , , Ложь);
	Спец_ПолучитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.РаботаетВБазеРайона, Ложь, , , Ложь);
	Спец_ПолучитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ЧисловойУровеньБазы, 0, , , Ложь);
	...
	Спец_ПолучитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ТаймаутОтправкиEMail, 60, , , Ложь);
	...
	Спец_ПолучитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ЛимитПамятиДляПотоков, 262144, , , Ложь);
	...
	Спец_ПолучитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.РазмерБуфераДляПотоков, 65536, , , Ложь);
	...
	// 5. Обмены с другими базами
	Спец_ОбменСДругимиБазами.УстановитьКонстантыПоУмолчанию();
	// -- Спец_БСП.ОбменСДругимиБазами_ЧтоТоЕсть
...
// Возвращаемое значение:
//  Булево
Функция ЭтоНеЛокальныйЧисловойIP(Знач IPДляПроверки) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	&СтрокаДляПроверки ПОДОБНО ""%.%.%.%""
	|		И &СтрокаДляПроверки <> ""127.0.0.1""
	|		И НЕ &СтрокаДляПроверки ПОДОБНО ""%.%.1.0""
	|		И НЕ &СтрокаДляПроверки ПОДОБНО ""%.%.1.1""
	|		И НЕ &СтрокаДляПроверки ПОДОБНО ""%.%.0.1""
	|		И НЕ &СтрокаДляПроверки ПОДОБНО ""%.%.0.0"" КАК МожноПисать";
	
	Запрос.УстановитьПараметр("СтрокаДляПроверки", IPДляПроверки);
	Возврат ПолучитьСтруктураПервойВыборкиЗапроса(Запрос, "МожноПисать");
...
//		* НужнаПереустановкаКомпонент - Булево
Функция ПолучитьСтруктуруПараметровДляЗапуска() Экспорт
	...
	Исключение
		...
		ПроверитьСоздатьПредопределенныеОбъекты(Ложь);
		Возврат СтруктураРезультат;
	...
	Если СтруктураРезультат.УстанавливатьЗаголовокБазы Тогда
		...
		СтруктураРезультат.ЗаголовокДляУстановки = СтрШаблон("%1 - %2 (%3)",
				...
				Спец_ПолучитьЗначениеПеременной("ТекущийПользовательПолноеИмя"),
				Спец_ПолучитьЗначениеПеременной("ИмяПользователяОСКлиент"));
	...
	Если ЗначениеЗаполнено(ПараметрыСеанса.Спец_ТекущийПользователь) Тогда
		...
		//@skip-check statement-type-change
		СтруктураРезультат.НужнаПереустановкаКомпонент = ЗначениеРеквизитаОбъекта(ПараметрыСеанса.Спец_ТекущийПользователь, "ПереустановитьКомпоненту", , Кэш); // Булево
		
		// Сразу снимаем признак переустановки (чтобы лишний раз не бегать на сервер)
		Если СтруктураРезультат.НужнаПереустановкаКомпонент Тогда

			СправочникОбъект = ПолучитьОбъектСБлокированием(ПараметрыСеанса.Спец_ТекущийПользователь, Кэш);
			Если СправочникОбъект <> Неопределено Тогда
				
				СправочникОбъект.ПереустановитьКомпоненту = Ложь;
				ЗаписатьОбъект(СправочникОбъект, , "", Ложь, Кэш);
...
Функция СтруктураПараметровБазы(Знач База = Неопределено) Экспорт
	...
	//@skip-check constructor-function-return-section
	Возврат Новый ФиксированнаяСтруктура(ПолучитьСтруктураПервойВыборкиЗапроса(Запрос));
...
Функция ПолучитьТаблицуСтруктурыХраненияБД(Знач МассивМетаданныхОтбор = Неопределено, Знач ВИменахБД = Истина) Экспорт
	...
	Если МассивМетаданныхОтбор.Количество() > 0 Тогда
...
Функция ПолучитьИмяТаблицыВ1СПоКодуЧерез1С(Знач КодТаблицы, Знач Кэш = Неопределено) Экспорт
...
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
Процедура НайтиСоздатьОбъектыРегистраЧерез1С(Знач ИсходныеДанные, Знач СтруктураПараметров, ТекстыОшибок = "", Знач Кэш = Неопределено) Экспорт
...
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
Процедура УдалитьДанныеПоОбъекту1СВБазеДанныхЧерез1С(Знач ИсходныеДанные, Знач ОбъектМетаданныхИлиПолноеИмя, ТекстыОшибок = "", Знач Кэш = Неопределено) Экспорт
...
Функция ПолучитьМассовоеЗначениеПоПВХ(Знач ПВХ) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПВХ) Тогда
		Возврат Справочники.Спец_МассивЗначенийКонстант.ПустаяСсылка();
	КонецЕсли;
	...
	Возврат ПолучитьСтруктураПервойВыборкиЗапроса(Запрос, "МассовоеЗначение");
...
Процедура УстановитьПараметрыСеанса(Знач ИменаПараметровСеанса) Экспорт
	...
	// Если неопределено, то это первая инициализация
	...
	Для Каждого ИмяПараметра Из ИменаПараметровСеанса Цикл
		...
		ИначеЕсли ИмяПараметра = Метаданные.ПараметрыСеанса.Спец_ТекущийПользователь.Имя Тогда
			
			ИмяПользователя = Спец_ОбщегоНазначенияКлиентСервер.ТекущийПользователь1С();
			
			ТекущийПользовательСсылка = Справочники.Спец_Пользователи.НайтиПоНаименованию(ИмяПользователя, Истина);
			Если Не ЗначениеЗаполнено(ТекущийПользовательСсылка) Тогда
				...
				ТекущийПользовательСсылка = ТекущийПользовательОбъект.Ссылка;
				
			КонецЕсли;
			
			ПараметрыСеанса.Спец_ТекущийПользователь = ТекущийПользовательСсылка;
		...
		// ++ Спец_БСП.Спец_ПодключаемоеОборудование
		ИначеЕсли ИмяПараметра = Метаданные.ПараметрыСеанса.Спец_РабочееМестоКлиента.Имя Тогда
			
			ПараметрыСеанса.Спец_РабочееМестоКлиента = Спец_МенеджерОборудованияВызовСервера.РабочееМестоПоИдентификаторуКлиента(
					Спец_МенеджерОборудованияКлиентСервер.ИдентификаторКлиентаДляРабочегоМеста()); 
			
			Если Не ЗначениеЗаполнено(ПараметрыСеанса.Спец_РабочееМестоКлиента) Тогда
				
				НовыйЭлементСправочник = Справочники.Спец_РабочиеМеста.СоздатьЭлемент();
				НовыйЭлементСправочник.ИмяКомпьютера = Спец_ОбщегоНазначенияКлиентСервер.ПолучитьИмяКомпьютера(Истина);
				НовыйЭлементСправочник.Наименование  = СтрШаблон("%1 (%2)",
						НовыйЭлементСправочник.ИмяКомпьютера,
						Спец_ОбщегоНазначенияКлиентСервер.ТекущийПользовательСистемы(Истина));
				
				ЗаписатьОбъект(НовыйЭлементСправочник);
...
Функция ПолучитьСтруктуруУведомленияДляОтправкиТекущемуПользователю() Экспорт
	...
	СтруктураРезультат = ПолучитьСтруктураПервойВыборкиЗапроса(Запрос); // см. ПолучитьСтруктуруУведомленияДляОтправкиТекущемуПользователю
...
// Параметры:
//  пТаблица - ТаблицаЗначений
// 
// Возвращаемое значение:
//  ТаблицаЗначений
Функция СкопироватьТаблицуБезNull(Знач пТаблица)
	
	ТаблицаРезультат = Новый ТаблицаЗначений;
	Для Каждого КолонкаТаблицы Из пТаблица.Колонки Цикл ТаблицаРезультат.Колонки.Добавить(КолонкаТаблицы.Имя, Спец_ОбщегоНазначенияКлиентСервер.ОписаниеТиповБезNull(КолонкаТаблицы.ТипЗначения)); КонецЦикла;
...
Процедура УстановитьКонстантыПоУмолчанию_ОбновитьИнформациюПоБазе(Знач Кэш)
	...
	Если Не ЭтоНеЛокальныйЧисловойIP(СправочникБазаОбъект.АдресСервераИБ) И ЭтоНеЛокальныйЧисловойIP(СтруктураАдресаИБ.ИмяСервераИБ) Тогда
...
Функция НайтиПоСсылкамПрограммно_СформироватьМассивМетаданныхДляПоиска(Знач МассивМетаданныхИлиИменДляПоиска, Знач МассивМетаданныхИлиИменДляИсключения, Знач Кэш)
	...
	Если МассивМетаданныхИлиИменДляИсключения.Количество() > 0 Тогда
	...
	ИначеЕсли КоличествоЗапросов > 1 Тогда
		...
		Если ВозвращатьТолькоДанные Тогда
			...
			МассивРезультат.Добавить(СтрШаблон(
					...
					Спец_СтроковыеФункцииКлиентСервер.СместитьВесьТекстНаРазделитель(СтрСоединить(МассивГотовыйЗапрос, Спец_СтроковыеФункцииКлиентСервер.РазделительОбъединяющихЗапросов())),
					ТекстЗапросаОграничение,
					ТекстИзмеренийДляЗапроса));
...
Функция ПолучитьКонстантуСпецПоПВХ(Знач ПВХ, Знач ЗначениеПоУмолчанию, Знач НаДату)
...
////	|	ХранимыеЗначенияКонстант.Период УБЫВ";
	...
	Возврат ?(ВыборкаРезультат.Следующий(),
			ПолучитьКонстантуСпецОбработатьЗначениеРезультат(ВыборкаРезультат.ЗначениеКонстанты),
			ПолучитьКонстантуСпецЗаписатьЗначениеПоПВХ(ПВХ, ЗначениеПоУмолчанию, НаДату, ТипЗнч(ЗначениеПоУмолчанию)));
...
// Возвращаемое значение:
//  см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
Функция ПолучитьКонстантуСпецПоКоду(Знач Код, Знач ЗначениеПоУмолчанию, Знач Наименование, Знач НаДату, Знач ВернутьПВХ = Ложь)
	...
	Возврат ?(ВГраница = 0,
			ПолучитьКонстантуСпецПоКодуБезРодителей(НормальныйКод, ЗначениеПоУмолчанию, Наименование, НаДату, ВернутьПВХ),
			ПолучитьКонстантуСпецПоКодуСРодителями(МассивКодов, ВГраница, ЗначениеПоУмолчанию, Наименование, НаДату, ВернутьПВХ));
...
// Возвращаемое значение:
//  см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
Функция ПолучитьКонстантуСпецПоКодуБезРодителей(Знач Код, Знач ЗначениеПоУмолчанию, Знач Наименование, Знач НаДату, Знач ВернутьПВХ)
...
////	|	ЕСТЬNULL(ХранимыеЗначенияКонстант.Период, ДАТАВРЕМЯ(2000, 1, 1)) УБЫВ";
	...
	Иначе
		...
		ПВХ = ПолучитьКонстантуСпецПерезаписатьЭлементПВХ(Неопределено, Код, Наименование, ТипЗначенияПоУмолчанию);
		Возврат ?(ВернутьПВХ,
				ПВХ,
				ПолучитьКонстантуСпецЗаписатьЗначениеПоПВХ(ПВХ, ЗначениеПоУмолчанию, НаДату, ТипЗначенияПоУмолчанию));
...
// Возвращаемое значение:
//  см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
Функция ПолучитьКонстантуСпецПоКодуСРодителями(Знач МассивКодов, Знач ВГраница, Знач ЗначениеПоУмолчанию, Знач Наименование, Знач НаДату, Знач ВернутьПВХ)
	...
	|			%3
	|	ГДЕ
	|		КонстантыРодитель0.Код = &Родитель0Код
...
////	|	ЕСТЬNULL(ХранимыеЗначенияКонстант.Период, ДАТАВРЕМЯ(2000, 1, 1)) УБЫВ";
	...
	МассивТекстСоединения.Добавить(СтрШаблон("ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.Спец_Константы КАК КонстантыЗначение
			...
			|		И КонстантыЗначение.Код = &КодКонстанты
	...
	Для i = 0 По КоличествоРодителей - 1 Цикл
		...
		МассивТекстСоединения.Добавить(СтрШаблон("	ПО КонстантыРодитель%1.Ссылка = КонстантыРодитель%2.Родитель
				|		И (КонстантыРодитель%2.Код = &Родитель%2Код)
	...
	Если ВыборкаРезультат.Следующий() Тогда
		...
		Если ВсеРодителиЕсть Тогда
			...
			Если ЗначениеЗаполнено(ВыборкаРезультат.Константа) Тогда
				...
				ИначеЕсли ВыборкаРезультат.ЗначениеОтсутствует Тогда
					Возврат ПолучитьКонстантуСпецЗаписатьЗначениеПоПВХ(ВыборкаРезультат.Константа, ЗначениеПоУмолчанию, НаДату, ТипЗнч(ЗначениеПоУмолчанию));
				Иначе
					Возврат ПолучитьКонстантуСпецОбработатьЗначениеРезультат(ВыборкаРезультат.ЗначениеКонстанты);
				КонецЕсли;
			...
			Иначе
				...
				ПВХ = ПолучитьКонстантуСпецПерезаписатьЭлементПВХ(Неопределено, МассивКодов.Получить(ВГраница), Наименование, ТипЗначенияПоУмолчанию, Родитель);
				Если ВернутьПВХ Тогда
					Возврат ПВХ;
				Иначе
					Возврат ПолучитьКонстантуСпецЗаписатьЗначениеПоПВХ(ПВХ, ЗначениеПоУмолчанию, НаДату, ТипЗначенияПоУмолчанию);
				КонецЕсли;
		...
		Иначе
			...
			Для i = ИндексНачальногоРодителя По КоличествоРодителей Цикл
				Родитель = ПолучитьКонстантуСпецСоздатьРодителя(МассивКодов.Получить(i), Родитель);
			КонецЦикла;
	...
	Иначе
		...
		Для i = 0 По КоличествоРодителей Цикл Родитель = ПолучитьКонстантуСпецСоздатьРодителя(МассивКодов.Получить(i), Родитель); КонецЦикла;
...
Функция ПолучитьКонстантуСпецСоздатьРодителя(Знач Код, Знач Родитель)
...
// Возвращаемое значение:
//  ПланВидовХарактеристикСсылка.Спец_Константы
Функция ПолучитьКонстантуСпецПерезаписатьЭлементПВХ(Знач ПВХ, Знач Код, Знач Наименование, Знач ТипЗначенияНовый, Знач Родитель = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ПВХ) Тогда
		...
		ПВХ = ПолучитьСтруктураПервойВыборкиЗапроса(Запрос, "ПВХ"); // ПланВидовХарактеристикСсылка.Спец_Константы
	...
	Если Не ЗначениеЗаполнено(ПВХ) Тогда
		...
		ПВХОбъект.Код = Код;
		ПВХОбъект.Наименование = ?(ПустаяСтрока(Наименование), Код, Наименование);
	...
	Иначе
		...
		ПВХОбъект = ПолучитьОбъектСБлокированием(ПВХ); // ПланВидовХарактеристикОбъект.Спец_Константы
		
	КонецЕсли;
	
	Спец_Проверить(ПВХОбъект <> Неопределено, "Не удалось получить объект ПВХ!");
	
	Если ПолучитьКонстантуСпецЭтоМассовоеЗначение(ТипЗначенияНовый) Тогда
...
//	- см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
//	- Неопределено
Функция ПолучитьКонстантуСпецЗаписатьЗначениеПоПВХ(Знач ПВХ, Знач Значение, Знач НаДату, Знач ТипЗначения)
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		Если ТипЗнч(НаборЗаписей.Получить(0).ЗначениеКонстанты) = Тип("СправочникСсылка.Спец_МассивЗначенийКонстант") Тогда // Если массовое значение, да ещё и записано, то ничего не делаем
			Возврат Неопределено;
		КонецЕсли;
		...
		Иначе
			...
			Если ТипЗнч(ПрошлаяЗапись.ЗначениеКонстанты) <> ТипЗначения Тогда
				ПВХ = ПолучитьКонстантуСпецПерезаписатьЭлементПВХ(ПВХ, "", "", ТипЗначения);
			КонецЕсли;
	...
	Если ПолучитьКонстантуСпецЭтоМассовоеЗначение(ТипЗначения) Тогда
		
		РеквизитыПВХ = ЗначенияРеквизитовОбъекта(ПВХ, "Код, Наименование");
		
		СправочникМассивЗначенийКонстант = ПолучитьМассовоеЗначениеПоПВХ(ПВХ);
		Если ЗначениеЗаполнено(СправочникМассивЗначенийКонстант) Тогда
			СправочникОбъект = ПолучитьОбъектСБлокированием(СправочникМассивЗначенийКонстант);
		Иначе
		...
		СправочникОбъект.Код = РеквизитыПВХ.Код;
		СправочникОбъект.Наименование = РеквизитыПВХ.Наименование;
		...
		ЗаписатьОбъект(СправочникОбъект, , , , , 5);
...
// Возвращаемое значение:
//  Булево
Функция ПолучитьКонстантуСпецЭтоМассовоеЗначение(Знач ТипЗначенияПоУмолчанию)
...
//  - ФиксированныйМассив из см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
//	- см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
Функция ПолучитьКонстантуСпецОбработатьЗначениеРезультат(Знач ЗначениеРезультат)
