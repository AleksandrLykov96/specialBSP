	...
	Если Команда = "CheckFiscalization" Или Команда = "CheckFiscalizationPacket" Тогда
		...
		Если ПараметрыПодключения.ТипОборудования = "ККТ" Тогда   
			...
			Если (ДанныеОперации.Отправляет1СSMS Или ДанныеОперации.Отправляет1СEmail) Тогда
				МодульРассылкаЭлектронныхЧеков = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("РассылкаЭлектронныхЧеков");
				МодульРассылкаЭлектронныхЧековВызовСервера = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("РассылкаЭлектронныхЧековВызовСервера");
...
Функция ДобавитьЧекВОчередьЧековККТ(ПараметрыЧекаККТ, ПараметрыПакетнойОперации = Неопределено) Экспорт
	...
	Исключение                 
		...
		МенеджерОборудования.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Ошибка добавления чека в очередь'", Метаданные.ОсновнойЯзык.КодЯзыка));
		Возврат Неопределено;
	КонецПопытки;
...
Функция ЧекиВОчередиНаФискализацию(КассаККМ = Неопределено) Экспорт
	...
	|	РегистрСведений.ОчередьЧековККТ КАК ОчередьЧековККТ
	|ГДЕ
	|	(ОчередьЧековККТ.КассаККМ = &КассаККМ
	|			ИЛИ &ВсеКассыККМ)
	|	И ОчередьЧековККТ.СтатусЧека = ЗНАЧЕНИЕ(Перечисление.СтатусЧекаККТВОчереди.Новый)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОчередьЧековККТ.Дата УБЫВ");
	
	Запрос.УстановитьПараметр("КассаККМ", КассаККМ);
	Запрос.УстановитьПараметр("ВсеКассыККМ", КассаККМ = Неопределено);
...
Процедура ЗаписатьСтатусЧекаВОчереди(ПараметрыФискализации, СтатусЧека, ОборудованиеККТ = Неопределено, ТекстОшибки = Неопределено) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		МенеджерОборудования.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Запись статуса чека в очереди'", Метаданные.ОсновнойЯзык.КодЯзыка));
		Возврат;
	КонецПопытки;
...
Функция ВыполненаПроверкаОбязательностиИПравильностиЗаполненияТэгов(Параметры, ИдентификаторУстройства, ОписаниеОшибки) Экспорт
	...
	Если ИспользуетсяФорматноЛогическийКонтроль() Тогда
		МодульФорматноЛогическийКонтрольВызовСервера = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ФорматноЛогическийКонтрольВызовСервера");
...
Функция ПолучитьСтатусПоследнейСмены(ИдентификаторУстройства) 
	...
	Если ИспользуетсяКассовыеСмены() Тогда
		МодульКассовыеСменыВызовСервера = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("КассовыеСменыВызовСервера");
...
Процедура ВывестиШтрихкод(Контекст, ТипШтрихкода, Штрихкод)
	...
	ПараметрыШтрихкода = Спец_ГенерацияЧтениеШтрихКодаКлиентСервер.ПодготовитьПараметрыГенерацииШтрихКода();
	...
	Изображение = Спец_ГенерацияЧтениеШтрихКодаКлиентСервер.ПолучитьИзображениеШтрихкода(ПараметрыШтрихкода);
