...
//	Отказ - Булево
//
Процедура ПриОткрытии(Знач Форма, Отказ = Ложь) Экспорт
	...
	Если Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(Форма, "Спец_ИспользоватьПодключаемоеОборудование") Тогда
		...
		Если Форма.Спец_ИспользоватьПодключаемоеОборудование Тогда
			
			Спец_МенеджерОборудованияКлиент.НачатьПодключениеОборудования(Форма,
					ПоддерживаемыеТипыПодключаемогоОборудования(Форма),
					,
					Новый ОписаниеОповещения("ПодключитьОборудованиеЗавершение", Спец_РаботаСФормамиКлиент, Форма));
...
Процедура ПриЗакрытии(Знач Форма, Знач ЗавершениеРаботы = Ложь) Экспорт
	...
	Если Не ЗавершениеРаботы
		...
		Спец_МенеджерОборудованияКлиент.НачатьОтключениеОборудованияПоФорме(Форма,
				Новый ОписаниеОповещения("ОтключитьОборудованиеЗавершение", Спец_РаботаСФормамиКлиент, Форма));
...
Процедура АвтоПодборТекстаИзСпискаВыбораСтрок(Знач Элемент, Знач Текст, ДанныеВыбора, Знач ПараметрыПолученияДанных, Знач Ожидание, СтандартнаяОбработка) Экспорт
	...
	МассивРезультат      = НовыйМассивСтруктурДляАвтоПодбораСтрок();
...
// По умолчанию подключается только сканер штрих-кода. Если нужно что-то ещё - необходимо переопределить метод "Спец_МассивТиповПодключаемогоОборудования" на форме.
// 
// Параметры:
//	Форма - см. Спец_РаботаСФормами.ПриСозданииНаСервере.Форма
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.Спец_ТипыПодключаемогоОборудования
//
Функция ПоддерживаемыеТипыПодключаемогоОборудования(Знач Форма) Экспорт
	...
	Исключение
		...
		Возврат Спец_РаботаСКоллекциямиКлиентСервер.ЗначениеВМассиве(ПредопределенноеЗначение("Перечисление.Спец_ТипыПодключаемогоОборудования.СканерШтрихкода"));
...
Процедура ПодключитьОборудованиеЗавершение(Знач РезультатВыполнения, Знач ФормаРодитель) Экспорт
	...
	Если Не РезультатВыполнения.Успешно И РезультатВыполнения.ТекстОшибки <> "Нет доступного оборудования" Тогда
		
		ТекстСообщения = СтрШаблон("При подключении оборудования произошла ошибка:%1%2",
				Символы.ПС,
				Спец_СтроковыеФункцииКлиентСервер.СместитьВесьТекстНаРазделитель(РезультатВыполнения.ТекстОшибки));
...
// Возвращаемое значение:
//  Массив из см. НоваяСтруктураРезультатаАвтоПодбораСтрок
//
Функция НовыйМассивСтруктурДляАвтоПодбораСтрок()
