...
//		* КомпонентаBaseFunction             - см. Спец_КомпонентаДополнительныеФункцииКлиентСерверПовтИсп.ПолучитьКомпонентуBaseFunction
// 		* КомпонентаHTTPClient               - см. Спец_КомпонентаДополнительныеФункцииКлиентСерверПовтИсп.ПолучитьКомпонентуHTTPClient
//		* КомпонентаPostgreSQL               - см. Спец_КомпонентаДополнительныеФункцииКлиентСерверПовтИсп.ПолучитьКомпонентуPostgreSQL
//		* КомпонентаMySQL                    - см. Спец_КомпонентаДополнительныеФункцииКлиентСерверПовтИсп.ПолучитьКомпонентуMySQL
...
// 			** Значение - Структура:
// 				*** КомпонентаPostgreSQL - см. Спец_КомпонентаДополнительныеФункцииКлиентСерверПовтИсп.ПолучитьКомпонентуPostgreSQL
//		* ПО_ПараметрыПодключения - Массив из см. Спец_МенеджерОборудованияКлиент.ПодключитьОборудование_ПодготовитьСтруктуруПараметраПодключения
//		* ПО_ДрайверыОборудования - Соответствие из КлючИЗначение:
// 			** Ключ - Строка
// 			** Значение - см. Спец_ОбщегоНазначенияКлиентСервер.ПодключитьВнешнююКомпонентуДляРаботы
...
//				*** Ключ - Тип
//				*** Значение - Строка
//		* ДляПроверкиЦелостности - ТаблицаЗначений
...
//		* МассивЗарегистрированныхЗапросов - Массив из см. Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктураЗапроса -
...
//
Функция Спец_ПолучитьКонстанту(Знач КодИлиСсылкаПВХ, Знач ЗначениеПоУмолчанию = Неопределено, Знач Наименование = "",
		Знач НаДату = Неопределено, Знач ЧитатьИзКэша = Истина) Экспорт
	...
	Если ЧитатьИзКэша Тогда
		...
		Если ЗначениеПоУмолчанию = Неопределено Тогда
			...
			Возврат Спец_ОбщегоНазначенияКлиентСерверПовтИсп.ПолучитьКонстантуСпец(КодИлиСсылкаПВХ, ЗначениеПоУмолчанию, Наименование, НаДату);
		...
		Иначе
			...
			Если Спец_РаботаСКоллекциямиКлиентСервер.ЭтоМассив(ЗначениеПоУмолчанию, ТипЗначения) Или ТипЗначения = Тип("СписокЗначений") Тогда
				...
				#Если ТонкийКлиент Или ВебКлиент Или ТолстыйКлиентУправляемоеПриложение Тогда
					Возврат Спец_ОбщегоНазначенияВызовСервера.ПолучитьКонстантуСпец(КодИлиСсылкаПВХ, ЗначениеПоУмолчанию, Наименование, НаДату, Ложь);
				#Иначе
...
//  ТекстЛога - Строка - Текст лога для записи
