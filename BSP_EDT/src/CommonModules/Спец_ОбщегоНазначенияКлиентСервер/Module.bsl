...
//  пОбщийТекстОшибок - Строка - Общий текст ошибок, куда будет добавлен подробный текст ошибки
//  пОтказ - Булево - Флаг отказа
Процедура ДобавитьОшибку(Знач ТекстОшибкиИлиИнформацияОбОшибке = "", Знач ИмяПроцедуры = "", пОбщийТекстОшибок = "", пОтказ = Ложь) Экспорт
	...
	Спец_СтроковыеФункцииКлиентСервер.ДобавитьСтрокуВТекст(ТекстСообщения, пОбщийТекстОшибок, Символы.ПС + "	------------------------------" + Символы.ПС);
	СообщитьПользователю(ТекстСообщения, , , , пОтказ);
...
Процедура ОтправитьСообщениеВТелеграм(Знач ТекстСообщенияДляОтправки, Знач ТипПарсинга = "", ТекстОшибки = "", Знач Кэш = Неопределено) Экспорт
	...
	#Если Не ВебКлиент Тогда
		...
		Для Каждого ИдентификаторПолучателя Из МассивИдентификаторовПолучателей Цикл
			...
			СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктураЗапроса(ПолныйURLЗапроса,
...
Функция НОД(Знач A, Знач B) Экспорт
	...
	Пока A <> B Цикл Если A > B Тогда Swap(A, B); КонецЕсли; B = B - A; КонецЦикла;
	Возврат A;
...
// 							   - Число
//
Функция ПолучитьСоответствиеJSON(Знач ВходныеДанные, Знач ЧитатьИзФайла = Ложь, Знач ВСоответствие = Истина) Экспорт
	...
	#Если Не ВебКлиент Тогда
		...
		Если Спец_РаботаСКоллекциямиКлиентСервер.ЭтоПоток(ВходныеДанные, ТипВходныхДанных) Тогда
			...
			ЧтениеJSON.ОткрытьПоток(ПотокДляЧтения, "UTF-8");
		...
		Иначе
			...
			Если ЧитатьИзФайла Тогда
				ЧтениеJSON.ОткрытьФайл(ДляЧтения, "UTF-8");
			Иначе
		...
		// Так как иногда попадаются косячные JSON, ограничиваем максимальную вложенность
		МаксимальнаяВложенность = Спец_ПолучитьКонстанту(ПредопределенноеЗначение("ПланВидовХарактеристик.Спец_Константы.РазмерБуфераДляПотоков"), 16384); // Число
		
		Попытка
			
			
			СоответствиеРезультат = ПрочитатьJSON(ЧтениеJSON,
					ВСоответствие,
					, , , , , ,
...
Функция ОписаниеТипаСтрока(Знач ДлинаСтроки = 0) Экспорт
...
Функция ОтправитьHTTPЗапросы(Знач МассивИлиСтруктураЗапросов, Кэш = Неопределено) Экспорт
	...
	Для Каждого СтруктураЗапроса Из МассивИлиСтруктураЗапросов Цикл
		...
		//		Кэш.ТекущийТаймАутHTTPЗапросов,
...
Процедура ЗаполнитьТелоHTTPЗапроса(HTTPЗапрос, Знач ДанныеДляЗапроса, Знач ИсходныйЗапрос = Неопределено) Экспорт
	...
	Иначе
		...
		Попытка
			HTTPЗапрос.УстановитьТелоИзСтроки(ПолучитьXMLСтрокуПоНормальному(ДанныеДляЗапроса), "UTF-8");
		Исключение
			HTTPЗапрос.УстановитьТелоИзСтроки(Строка(ДанныеДляЗапроса), "UTF-8");
		КонецПопытки;
...
Функция ЗаголовкиДляHTTPЗапросовПоУмолчанию() Экспорт
	...
	УровеньБазы         = Спец_ПолучитьКонстанту(ПредопределенноеЗначение("ПланВидовХарактеристик.Спец_Константы.ЧисловойУровеньБазы"), 0); // Число
	РаботаетВБазеРайона = Спец_ПолучитьКонстанту(ПредопределенноеЗначение("ПланВидовХарактеристик.Спец_Константы.РаботаетВБазеРайона"), Ложь); // Булево
	ГУИДТекущейБазы     = Спец_ПолучитьКонстанту(ПредопределенноеЗначение("ПланВидовХарактеристик.Спец_Константы.ГУИДТекущейБазы"), ""); // Строка
	
	Заголовки = Новый Соответствие;
	
	Заголовки.Вставить("Database" , Спец_ОбщегоНазначенияКлиентСерверПовтИсп.СтруктураПараметровБазы().ИмяБазы);
	Заголовки.Вставить("UB"       , Формат(УровеньБазы, "ЧН=0; ЧГ="));
	Заголовки.Вставить("RVR"      , Формат(РаботаетВБазеРайона, "БЛ=0; БИ=1"));
	Заголовки.Вставить("UID"      , ГУИДТекущейБазы);
...
Функция PIDТекущегоПроцесса() Экспорт
	...
	ИначеЕсли ЭтоWindows() Тогда
		...
		Попытка
			...
			oChildProcess = oService.Get("Win32_Process.Handle=" + Формат(oApp.ProcessID, "ЧГ="));
			Результат = Формат(oChildProcess.ParentProcessID, "ЧГ=");
...
Функция БазовыйКэш() Экспорт
	...
	#Если Клиент Тогда
		...
		Кэш.Вставить("ПО_ПараметрыПодключения", Новый Массив());
		Кэш.Вставить("ПО_ДрайверыОборудования", Новый Соответствие());
		// -- Спец_БСП.Спец_ПодключаемоеОборудование
	...
	#Если Сервер Тогда
		Кэш.Вставить("ДляПроверкиЦелостности", Новый ТаблицаЗначений);
	#КонецЕсли
...
//			** Ключ - Строка
//			** Значение - Строка
Функция НоваяСтруктураРезультатаHTTPЗапроса(Знач ИдентификаторЗапроса, Знач URL, Знач ТипЗапроса)
