...
//  Массив из Произвольный
//
Функция ВыгрузитьМассивЗначений(Знач КоллекцияЗначений, Знач ИмяКолонки, Знач ТолькоУникальныеЗначения = Ложь, Знач ИсключаяПустые = Ложь, Знач СтруктураОтбораДляВыгрузки = Неопределено) Экспорт
	
	МассивРезультат = Новый Массив();
	Если СтруктураОтбораДляВыгрузки = Неопределено Тогда
		СтруктураОтбораДляВыгрузки = Новый Структура();
	КонецЕсли;
	
	УникальныеЗначения = Новый Соответствие;
	Для Каждого ЭлементКоллекции Из КоллекцияЗначений Цикл
		
		Если ТолькоУникальныеЗначения И УникальныеЗначения[ЭлементКоллекции[ИмяКолонки]] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УдалитьЗначенияИзМассиваСтруктур(Знач МассивДляУдаления, Знач КлючСтруктуры, Знач ЗначениеДляУдаления) Экспорт
	...
	МассивИндексовДляУдаления = Новый Массив; // Массив из Число
	
	ТекущийИндекс = 0;
	Для Каждого СтруктураИзМассива Из МассивДляУдаления Цикл Если СтруктураИзМассива[КлючСтруктуры] = ЗначениеДляУдаления Тогда МассивИндексовДляУдаления.Добавить(ТекущийИндекс); КонецЕсли; ТекущийИндекс = ТекущийИндекс + 1; КонецЦикла;
	
	СортироватьМассив(МассивИндексовДляУдаления, НаправлениеСортировки.Убыв);
	Для Каждого ИндексДляУдаления Из МассивИндексовДляУдаления Цикл МассивДляУдаления.Удалить(ИндексДляУдаления); КонецЦикла;
...
//  Булево
//
Функция ЕстьРеквизитОбъекта(Знач ИсходныйОбъект, Знач КлючДляПроверки, Знач Кэш = Неопределено) Экспорт
	
	Если ИсходныйОбъект = Неопределено Или ИсходныйОбъект = NULL Тогда Возврат Ложь; КонецЕсли;
	
	УИ = Спец_КомпонентаДополнительныеФункцииКлиентСервер.ПолучитьУникальныйИдентификатор(Кэш);
	
	СтруктураДляПроверки = Новый Структура(КлючДляПроверки, УИ);
	ЗаполнитьЗначенияСвойств(СтруктураДляПроверки, ИсходныйОбъект);
	
	Возврат СтруктураДляПроверки[КлючДляПроверки] <> УИ;
