...
Процедура ЧисткаСтарыхЛишнихДанных() Экспорт
	...
	Если КоличествоХранениеЛогов > 0 Тогда
		...
		Если Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLСоединениеУспешноУстановлено(Кэш) Тогда
			...
			ТекстЗапроса = Спец_РаботаСБазойДанныхНапрямуюВызовСервера.ПодставитьНаименованияТаблицДляТекстаЗапросаНапрямую(ТекстЗапроса);
			ТекстОшибок = "";
	...
	Если КоличествоДнейХраненияОшибок > 0 Тогда
		...
		Если Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLСоединениеУспешноУстановлено(Кэш) Тогда
			...
			ТекстЗапроса = СтрШаблон(
					...
					"DELETE FROM ONLY [РегистрСведений.Спец_РегистрОшибок]
					|WHERE [РегистрСведений.Спец_РегистрОшибок.Период] <= '%1'::timestamp",
					
					СериализаторXDTO.XMLСтрока(КрайняяДата));
		...
		Если Не Успешно Тогда
			...
			Пока ВыборкаРезультат.Следующий() Цикл
				...
				Спец_ОбщегоНазначения.ЗаписатьОбъект(НаборЗаписейРегистрОшибок, , , , Кэш, , , Ложь);
