...
Процедура ПередЗаписьюОбъекта(Знач Источник, Отказ, Знач ДополнительныеСвойства, Знач Кэш) Экспорт
	...
	|	Спец_ОбъектыДляРегистрацииДляОбменов.ПоляДляИзменения КАК ПоляДляИзменения
	|ИЗ
	...
	Запрос.УстановитьПараметр("ПолноеИмяОбъектаМетаданных", ДополнительныеСвойства.ПолноеИмяМетаданных);
	
	ВыборкаРезультат = Запрос.Выполнить().Выбрать();
	Пока ВыборкаРезультат.Следующий() Цикл
		
		РежимРегистрации = ПередЗаписьюОбъекта_РежимРегистрацииОбъекта(ВыборкаРезультат, Источник, ДополнительныеСвойства.МетаданныеОбъекта, ДополнительныеСвойства, Кэш);
		Если РежимРегистрации = 0 Тогда
			
			ДополнительныеСвойства.МассивБазДляОбменаПолучатель.Добавить(ВыборкаРезультат.База);
...
Процедура ПриЗаписиОбъекта(Знач Источник, Отказ, Знач ДополнительныеСвойства, Знач Кэш) Экспорт
	...
	|ГДЕ
	|	ОбъектыДляРегистрацииДляОбменов.База В(&МассивБазДляОбмена)
	...
	Запрос.УстановитьПараметр("МассивБазДляОбмена", ДополнительныеСвойства.МассивБазДляОбменаПолучатель);
	Запрос.УстановитьПараметр("ПолноеИмяОбъектаМетаданных", ДополнительныеСвойства.ПолноеИмяМетаданных);
	...
	Пока ВыборкаРезультат.Следующий() Цикл
		...
		Если Не ДополнительныеСвойства.ЭтоСсылочныйОбъект Тогда
			...
			Если Спец_СтроковыеФункцииКлиентСервер.ЕстьКакоеЛибоУсловиеВЗапросе(ЗапросДляПроверки.Текст) Тогда
				ЗапросДляПроверки.Текст = ЗапросДляПроверки.Текст + " И " + ВыборкаРезультат.ТекстЗапросаИсключения;
			Иначе
				ЗапросДляПроверки.Текст = ЗапросДляПроверки.Текст + " ГДЕ " + ВыборкаРезультат.ТекстЗапросаИсключения;
			КонецЕсли;
		...
		Иначе
			...
			ЗапросДляПроверки.Текст = СтрШаблон(
					...
					ВыборкаРезультат.ТекстЗапросаИсключения);
...
Функция ИсправитьГУИДыПоОбъекту_ИсправитьНапрямую(Знач ИсходнаяСсылка, Знач ТекущийГУИДСсылки, Знач НужныйГУИДСсылки, Знач СтруктураМетаданных, ТекстыОшибок, Кэш)
	...
	Успешно = Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLВыполнитьЗапрос(ТекстЗапроса, 0, , Кэш);
	Если Успешно <> Истина Тогда
		
		Спец_СтроковыеФункцииКлиентСервер.ДобавитьСтрокуВТекст(Успешно.ТекстОшибки, ТекстыОшибок, Символы.ПС + Символы.ПС);
