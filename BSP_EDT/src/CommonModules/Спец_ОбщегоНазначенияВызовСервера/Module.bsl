...
// См. Спец_ОбщегоНазначения.ПроверитьСоздатьПредопределенныеОбъекты
// 
// Параметры:
//  ПроверятьВсе - см. Спец_ОбщегоНазначения.ПроверитьСоздатьПредопределенныеОбъекты.ПроверятьВсе
//  ТекстыОшибок - см. Спец_ОбщегоНазначения.ПроверитьСоздатьПредопределенныеОбъекты.ТекстыОшибок
Процедура ПроверитьСоздатьПредопределенныеОбъекты(Знач ПроверятьВсе = Истина, ТекстыОшибок = "") Экспорт
	
	Спец_ОбщегоНазначения.ПроверитьСоздатьПредопределенныеОбъекты(ПроверятьВсе, ТекстыОшибок);
...
Процедура УстановитьТекущуюБазу() Экспорт
	...
	Если ВыборкаРезультат.Следующий() Тогда
		...
		Если ПустаяСтрока(ВыборкаРезультат.АдресСервераИБ)
			Или (Не Спец_ОбщегоНазначения.ЭтоНеЛокальныйЧисловойIP(ВыборкаРезультат.АдресСервераИБ)
					И Спец_ОбщегоНазначения.ЭтоНеЛокальныйЧисловойIP(СтруктураСервера.ИмяСервераИБ)) Тогда
...
Функция ПолучитьРежимЗапускаТекущегоПользователя() Экспорт
	...
	ТекущийПользователь = Спец_ОбщегоНазначенияПовтИсп.ЗначениеПараметраСеанса("Спец_ТекущийПользователь"); // СправочникСсылка.Спец_Пользователи
	Если ЗначениеЗаполнено(ТекущийПользователь) Тогда
		Возврат Спец_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийПользователь, "РежимЗапуска");
	Иначе
...
Функция ПолучитьСтруктуруПараметровДляЗапуска() Экспорт
	...
	Возврат Спец_ОбщегоНазначения.ПолучитьСтруктуруПараметровДляЗапуска();
...
//  Наименование - см. Спец_ОбщегоНазначения.ПолучитьКонстантуСпец.Наименование
//  НаДату - см. Спец_ОбщегоНазначения.ПолучитьКонстантуСпец.НаДату
//  ЧитатьИзКэша - Булево
// 
// Возвращаемое значение:
//  см. Спец_ОбщегоНазначения.ПолучитьКонстантуСпец
Функция ПолучитьКонстантуСпец(Знач КодИлиСсылкаПВХ, Знач ЗначениеПоУмолчанию = Неопределено, Знач Наименование = "", Знач НаДату = Неопределено, Знач ЧитатьИзКэша = Истина) Экспорт
	
	Возврат ?(ЧитатьИзКэша,
			Спец_ОбщегоНазначенияПовтИсп.ПолучитьКонстантуСпец(КодИлиСсылкаПВХ, ЗначениеПоУмолчанию, Наименование, НаДату),
			Спец_ОбщегоНазначения.ПолучитьКонстантуСпец(КодИлиСсылкаПВХ, ЗначениеПоУмолчанию, Наименование, НаДату));
...
Функция ПолучитьСоответствиеJSON(Знач ВходныеДанные, Знач ЧитатьИзФайла = Ложь, Знач ВСоответствие = Истина) Экспорт
	...
	//@skip-check constructor-function-return-section
	Возврат Спец_ОбщегоНазначенияКлиентСервер.ПолучитьСоответствиеJSON(ВходныеДанные, ЧитатьИзФайла, ВСоответствие);
