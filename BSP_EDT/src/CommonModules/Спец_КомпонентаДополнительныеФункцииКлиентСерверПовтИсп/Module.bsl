...
Функция ПолучитьКомпонентуBaseFunction(Знач КонстантноеИмяКомпоненты = Истина) Экспорт
	...
	Попытка
		...
		Если Спец_КомпонентаДополнительныеФункцииВызовСервера.КомпонентаНеРаботаетУПользователя("ОбщийМакет.Спец_КомпонентаLykovExtFunctions") Тогда
...
Функция ПолучитьКомпонентуHTTPClient(Знач КонстантноеИмяКомпоненты = Истина) Экспорт
	...
	Попытка
		...
		Если Спец_КомпонентаДополнительныеФункцииВызовСервера.КомпонентаНеРаботаетУПользователя("ОбщийМакет.Спец_КомпонентаLykovHTTPClient") Тогда
			Возврат Неопределено;
		КонецЕсли;
...
Функция ПолучитьКомпонентуPostgreSQL(Знач КонстантноеИмяКомпоненты = Истина) Экспорт
	...
	Попытка
		...
		Если Спец_КомпонентаДополнительныеФункцииВызовСервера.КомпонентаНеРаботаетУПользователя("ОбщийМакет.Спец_КомпонентаLykovPostgreSQL") Тогда
			Возврат Неопределено;
		КонецЕсли;
...
Функция ПолучитьКомпонентуMySQL(Знач КонстантноеИмяКомпоненты = Истина) Экспорт
	...
	Попытка
		...
		Если Спец_КомпонентаДополнительныеФункцииВызовСервера.КомпонентаНеРаботаетУПользователя("ОбщийМакет.Спец_КомпонентаLykovMySQL") Тогда
			Возврат Неопределено;
		КонецЕсли;
...
Функция ПолучитьСоответствиеВерсийМоихКомпонент() Экспорт
	...
	СоответствиеРезультат["ОбщийМакет.Спец_КомпонентаLykovExtFunctions"] = НоваяСтруктураВерсииМоейКомпоненты("1.0.0.16", "ПолучитьКомпонентуBaseFunction");
	СоответствиеРезультат["ОбщийМакет.Спец_КомпонентаLykovHTTPClient"]   = НоваяСтруктураВерсииМоейКомпоненты("1.0.0.16", "ПолучитьКомпонентуHTTPClient");
	СоответствиеРезультат["ОбщийМакет.Спец_КомпонентаLykovMySQL"]        = НоваяСтруктураВерсииМоейКомпоненты("1.0.0.16", "ПолучитьКомпонентуMySQL");
	СоответствиеРезультат["ОбщийМакет.Спец_КомпонентаLykovPostgreSQL"]   = НоваяСтруктураВерсииМоейКомпоненты("1.0.0.16", "ПолучитьКомпонентуPostgreSQL");
