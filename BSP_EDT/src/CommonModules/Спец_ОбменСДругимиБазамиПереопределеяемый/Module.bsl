...
Функция Отправитель_ОбработатьИзмененияВТекущейБазе_ТекстЗапросаИсключенияПоОбъектамДляОбработки(Знач СтруктураПараметров, Знач МассивИменМетаданных, Кэш)
	...
	Для Каждого ИмяМетаданных Из МассивИменМетаданных Цикл
		...
		Если СтруктураПараметров.ВидБазы = Перечисления.Спец_ВидыБаз.Бухгалтерия Тогда
			...
			Если Спец_ОбщегоНазначения.ЭтоСправочник(СтруктураТипов.ПолноеИмяОбъекта, Кэш, Тип("Строка"))
				И СтруктураТипов.МетаданныеОбъекта.Владельцы.Количество() > 0 Тогда
	...
	Возврат СтрСоединить(МассивТекстЗапросаИсключения, Спец_СтроковыеФункцииКлиентСервер.РазделительОбъединяющихЗапросов());
...
#Область Отбор_построителя_отчета
	...
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.ГлавноеУправление
		...
		ДополнительнаяКомпонента = Спец_КомпонентаДополнительныеФункцииКлиентСерверПовтИсп.ПолучитьКомпонентуHTTPClient(Ложь);
		...
		Для Каждого СтрокаХранилище Из ТаблицаХранилищГотовая Цикл
			...
			СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктураЗапроса("hs/specialExchange/exceptions/queryBuilder",
...
#Область База_бухгалтерии
	...
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.ГлавноеУправление
		...
		НулеваяТаблица.Колонки.Добавить("ГоловнаяОрганизация"       , Новый ОписаниеТипов("СправочникСсылка.Организации"));
		НулеваяТаблица.Колонки.Добавить("ОбособленноеПодразделение" , Новый ОписаниеТипов("Булево"));
		
		ОрганизацииПоМагазинам = Спец_ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивОбъектовИлиТаблицаРегистра, "ОсновнойСклад.Организация", , Кэш);
		Для Каждого СтрокаТаблицы Из НулеваяТаблица Цикл
			
			СтрокаТаблицы.Владелец                  = ОрганизацииПоМагазинам[СтрокаТаблицы.Ссылка];
			СтрокаТаблицы.ГоловнаяОрганизация       = ОрганизацииПоМагазинам[СтрокаТаблицы.Ссылка];
			СтрокаТаблицы.ОбособленноеПодразделение = Истина;
...
#КонецОбласти
	...
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.Аптека
		...
		Для Каждого СтрокаГрафикРаботы Из ТаблицаГрафикРаботыПодготовка Цикл
			...
			Для НомерДня = НомерДняНач По НомерДняКон Цикл
				...
				Если СтрокаГрафикРаботы["Часов" + Формат(НомерДня, "ЧН=0; ЧГ=")] > 0 И СтрокаДневныеЧасы = Неопределено Тогда
				...
				Если СтрокаГрафикРаботы["ЧасовВечерних" + Формат(НомерДня, "ЧН=0; ЧГ=")] > 0 И СтрокаВечерниеЧасы = Неопределено Тогда
				...
				Если СтрокаГрафикРаботы["ЧасовНочных" + Формат(НомерДня, "ЧН=0; ЧГ=")] > 0 И СтрокаНочныеЧасы = Неопределено Тогда
...
Процедура Отправитель_ОбработатьИзмененияВТекущейБазе_ПередЗаписьюОбъекта(СтруктураПараметров, Объект, Знач СоответствиеПолученныхДанных, Знач ПолноеИмяОбъектаМетаданных, Кэш) Экспорт
	...
	// ++ Спец_БСП.ВидБазы_ГлавноеУправление
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.ГлавноеУправление Тогда
		
		Если СтруктураПараметров.ВидБазы = Перечисления.Спец_ВидыБаз.ЗиУП И ПолноеИмяОбъектаМетаданных = "Справочник.СотрудникиОрганизаций" Тогда
	...
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.Аптека
		...
		Если ПолноеИмяОбъектаМетаданных = "Документ.ВводИндивидуальныхГрафиковРаботыОрганизации" Тогда
			...
			Для Каждого СтрокаТаблицы Из Объект.ГрафикРаботы Цикл
				...
				СтруктураВыборки = Спец_ОбщегоНазначения.ПолучитьСтруктураПервойВыборкиЗапроса(Запрос);
...
Процедура Получатель_ОбработатьИзмененияВТекущейБазе_ДополнительныеДанныеДляОбработки(СтруктураПараметров, Знач ПолноеИмяМетаданных, МассивОбъектовИлиТаблицаРегистра, ТаблицыПоОбъектам, Кэш)
	...
	// ++ Спец_БСП.ВидБазы_ЗиУП
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.ЗиУП Тогда
			...
			Если ПолноеИмяМетаданных = "Справочник.Сотрудники" Тогда
				...
				Для Каждого СтрокаТаблицы Из НулеваяТаблица Цикл
					...
					Если СоответствиеПоКадровымДанным[СтрокаТаблицы.Ссылка] <> Неопределено Тогда
						...
						Если ТаблицаКадровыеДанныеПоСотруднику.Количество() > 0 Тогда
			...
			Если ПолноеИмяМетаданных = "Документ.ИндивидуальныйГрафик" Тогда
				...
				Для Каждого СтрокаТаблицы Из ТаблицаГрафикРаботы Цикл
					...
					Для i = 1 По 31 Цикл
						...
						Если СтрокаТаблицы["Часов" + ЧислоВФормате] > 0
							Или СтрокаТаблицы["ЧасовВечерних" + ЧислоВФормате] > 0
							Или СтрокаТаблицы["ЧасовНочных" + ЧислоВФормате] > 0 Тогда
	...
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.Бухгалтерия Тогда
...
Процедура Получатель_ОбработатьИзмененияВТекущейБазе_ПередЗаписьюОбъекта(СтруктураПараметров, Объект, Знач СоответствиеПолученныхДанных, Знач ПолноеИмяОбъектаМетаданных, Кэш) Экспорт
	...
	// ++ Спец_БСП.ВидБазы_Бухгалтерия
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.Бухгалтерия
		...
		ИначеЕсли ПолноеИмяОбъектаМетаданных = "Справочник.ПодразделенияОрганизаций" Тогда
			...
			Если Спец_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Родитель, "Наименование") = "Аптеки новые" Тогда
				...
				Объект.Родитель = Спец_ОбщегоНазначения.ПолучитьСтруктураПервойВыборкиЗапроса(Запрос, "Ссылка");
	...
	// ++ Спец_БСП.ВидБазы_ЗиУП
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.ЗиУП
		И СтруктураПараметров.ВидБазы = Перечисления.Спец_ВидыБаз.Аптека
...
Процедура Получатель_ОбработатьИзмененияВТекущейБазе_ПослеЗаписиОбъекта(СтруктураПараметров, Объект, Знач СоответствиеПолученныхДанных, Знач ПолноеИмяОбъектаМетаданных, Кэш) Экспорт
	...
	Если Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ВидБазы = Перечисления.Спец_ВидыБаз.Аптека
		...
		Если ЗначениеЗаполнено(ГУИДФизлицаЗиУП) Тогда
			...
			ФизЛицо = Спец_ОбщегоНазначения.ПолучитьСтруктураПервойВыборкиЗапроса(Запрос, "Ссылка");
...
Процедура ЗначениеДополнительногоПараметраПоОбщемуОбъектуВКэш(Знач СтруктураПараметров, Знач Объект, Знач ПолноеИмяОбъектаМетаданных, Кэш) Экспорт
	...
	Если (СтруктураПараметров.ВидБазы = Перечисления.Спец_ВидыБаз.ЗиУП Или СтруктураПараметров.ВидБазы = Перечисления.Спец_ВидыБаз.Бухгалтерия) Тогда
		...
		ИначеЕсли СтруктураПоМетаданным.ПолноеИмяОбъекта = "Справочник.Магазины" Тогда
			...
			РеквизитыМагазина = Спец_ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект, "ОсновнойСклад.Организация КАК Организация, ЭтоГруппа", , Кэш);
...
Процедура ОбработатьСозданиеИнформационнойКарты(Знач КодКарты, Знач ВидДисконтнойКартыНаименование, Знач ОбъектСотрудник, Знач ФизЛицо, Кэш)
	...
	Если ЗначениеЗаполнено(КодКарты) Тогда
		...
		ИнформационнаяКарта = Спец_ОбщегоНазначения.ПолучитьСтруктураПервойВыборкиЗапроса(Запрос, "Ссылка");
