...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Элементы.ДекорацияОбщийРазмерТаблицыЧитаемый.Заголовок       = Спец_ОбщегоНазначенияКлиентСервер.ПеревестиРазмерИзБайтовВЧитемыйВид(Объект.ОбщийРазмерТаблицы);
	Элементы.ДекорацияРазмерТаблицыБезИндексовЧитаемый.Заголовок = Спец_ОбщегоНазначенияКлиентСервер.ПеревестиРазмерИзБайтовВЧитемыйВид(Объект.РазмерТаблицыБезИндексов);
...
Процедура КомандаИндексы_УдалитьВсе(Команда)
	...
	Если ТипБазы = ПредопределенноеЗначение("Перечисление.Спец_ТипыБазыДанных.PostgreSQL") Тогда
		...
		Скрипт = СтрСоединить(МассивТекстСкрипта, ";" + Символы.ПС);
		ВыполнитьТекстЗапросаКPostgreSQL(Скрипт);
...
Процедура ВыполнитьТекстЗапросаКPostgreSQL(Знач ТекстЗапроса)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыполненияЗапросаВБД", ЭтотОбъект);
	Спец_PostgreSQLКлиент.ВыполнитьЗапросPostgreSQLКрасиво(ТекстЗапроса, Объект.Наименование, ЭтотОбъект, ОписаниеОповещения, Истина);
