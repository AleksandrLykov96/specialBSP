	...
	Попытка
		...
		Для Каждого СтруктураПараметров Из ВходнойМассивПараметров Цикл
			...
			СтруктураДляЗаписи = НоваяСтруктураДляЗаписиПараметровОбмена(СтруктураВходныеДанные.База,
					...
					СтруктураПараметров.ПропуститьРегистрациюУдаленныхОбъектов);
...
Функция pingЛюбой(Знач HTTPСервисЗапрос)
	...
	// Берём рандомную таблицу и делаем к ней запрос (не заблокирована ли таблица)
	Кэш = Спец_ПолучитьКэш();
	
	ГСЧ                = Новый ГенераторСлучайныхЧисел();
	ВсеИменаМетаданных = Спец_ОбщегоНазначенияПовтИсп.ВсеИменаМетаданныхИзБазы(Истина);
	ИмяТаблицы         = ВсеИменаМетаданных.Получить(ГСЧ.СлучайноеЧисло(0, ВсеИменаМетаданных.ВГраница()));
	
	Запрос = Новый Запрос;
	Если Спец_ОбщегоНазначения.ЭтоСсылка(ИмяТаблицы, Кэш, Тип("Строка")) Тогда
...
Функция НоваяСтруктураДляЗаписиПараметровОбмена(Знач База, Знач ПолноеИмяОбъектаМетаданных, Знач ТекстЗапросаИсключения, Знач ПоляДляИзменения, Знач ПропуститьРегистрациюУдаленныхОбъектов)
	
	Возврат Новый Структура("База, ПолноеИмяОбъектаМетаданных, ТекстЗапросаИсключения, ПоляДляИзменения, ПропуститьРегистрациюУдаленныхОбъектов",
			База,
			ПолноеИмяОбъектаМетаданных,
			ТекстЗапросаИсключения,
			Спец_ОбщегоНазначенияКлиентСервер.ПолучитьСтрокуПоJSON(ПоляДляИзменения, Ложь),
			ПропуститьРегистрациюУдаленныхОбъектов);
