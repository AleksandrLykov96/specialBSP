...
#КонецЕсли
	...
	МодульЧекопечатающиеУстройства = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеЧекопечатающиеУстройстваКлиент");
	Если МодульЧекопечатающиеУстройства <> Неопределено Тогда
		
		ПараметрыОткрытия = МодульЧекопечатающиеУстройства.ПараметрыФормаНастройкиРегистрацииККТ();
	...
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("ОперацияФискальногоНакопителя_Продолжить", ЭтотОбъект);
		МодульЧекопечатающиеУстройства.ОткрытьФормуНастройкиРегистрацииККТ(ОповещениеПриЗавершении, ПараметрыОткрытия);
