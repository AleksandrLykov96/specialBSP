...
Процедура КомандаЗаписать(Команда)
	...
	Если Пользователь = Спец_ОбщегоНазначенияКлиентСерверПовтИсп.ЗначениеПараметраСеанса("Спец_ТекущийПользователь") Тогда
	...
	Иначе
		...
		ПослеЗакрытияПредупреждения(Неопределено);
	...
	Если ПустаяСтрока(СтильКонфигурации) Тогда
		...
		Для Каждого МетаданныеСтиль Из Метаданные.Стили Цикл
			...
			Попытка
				...
				Если ГлавныйСтиль = БиблиотекаСтилей[МетаданныеСтиль.Имя] Тогда
					
					Найдено = Истина;
					НаименованиеКартинкиРезультат = МетаданныеСтиль.Имя + "Стиль";
...
Функция ЗаписатьОсновнойСтильНаСервере()
	...
	Возврат Спец_ОбщегоНазначения.ЗаписатьОбъект(СправочникОбъект, , , , , , СтруктураРеквизитов);
