...
Попытка
	...
	Если ПредопределенныйЭлемент = Неопределено Тогда
		...
		ПредопределенныйЭлемент = НовыйПредопределенныйОбъект.Ссылка;
		
	КонецЕсли;
	
	СтруктураРеквизитовРезультат = Новый Структура("Наименование, ЭтоГруппа, УникальныйИдентификатор",
			Спец_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПредопределенныйЭлемент, "Наименование", , Кэш),
			Ложь,
			Строка(ПредопределенныйЭлемент.УникальныйИдентификатор()));
	
	Если Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(СтруктураПоМетаданным.МетаданныеОбъекта, "Иерархический")
		И СтруктураПоМетаданным.МетаданныеОбъекта.Иерархический Тогда
		
		СтруктураРеквизитовРезультат.ЭтоГруппа = Спец_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПредопределенныйЭлемент, "ЭтоГруппа", , Кэш);
		
	КонецЕсли;
	
	Возврат Спец_ОбщегоНазначения.ПодготовитьHTTPСервисОтветПоУмолчанию(200, СтруктураРеквизитовРезультат);
		...
		Для Каждого ИмяОбъекта Из МассивОбъектов Цикл
			...
			Для Каждого ОбъектМетаданных Из Метаданные[ИмяОбъекта] Цикл
				...
				Иначе
					...
					Если СтруктураРеквизитов.Свойство("ТабличныеЧасти") И СтруктураРеквизитов.ТабличныеЧасти.Количество() Тогда
