...
// Параметры:
//  Источник - см. Спец_ОбщегоНазначения.ПолучитьСтруктуруИмениТипаДанныхОбъекта.Объект
//  СтруктураДополнительныхСвойств - см. ИнициализироватьОбщиеДополнительныеСвойства
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
// 
// Возвращаемое значение:
//  Булево
//
Функция ПропуститьВсеПроверкиПоОбъекту(Знач Источник, Знач СтруктураДополнительныхСвойств, Знач Кэш) Экспорт
	
	ЭтоОбъектМетаданных = ТипЗнч(Источник) = Тип("ОбъектМетаданных");
	Если Не ЭтоОбъектМетаданных И Спец_РаботаСКоллекциямиКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "Спец_ПропуститьПроверки", Ложь) = Истина Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если СтруктураДополнительныхСвойств = Неопределено Тогда
		СтруктураДополнительныхСвойств = НоваяСтруктураДополнительныхСвойств(?(ЭтоОбъектМетаданных, Источник, Источник.Метаданные()), Кэш);
	КонецЕсли;
	
	Если СтруктураДополнительныхСвойств.ПолноеИмяМетаданных = "РегистрСведений.Спец_ХранимыеЗначенияКонстант" Тогда
		
		Возврат Истина;
		
	ИначеЕсли Не ПустаяСтрока(СтруктураДополнительныхСвойств.ПолноеИмяМетаданных) Тогда
		...
		Если МассивОбъектовПроверятьМодифицированность.Количество() Тогда
	
			Если МассивОбъектовПроверятьМодифицированность.Найти(СтруктураДополнительныхСвойств.ПолноеИмяМетаданных) = Неопределено Тогда
				Возврат Истина;
			КонецЕсли;
		...
		Иначе
			...
			Если МассивОбъектовНеПроверятьМодифицированность.Количество() Тогда
				
				Если МассивОбъектовНеПроверятьМодифицированность.Найти(СтруктураДополнительныхСвойств.ПолноеИмяМетаданных) <> Неопределено Тогда
					Возврат Истина;
				КонецЕсли;
...
Процедура ИнициализироватьОбщиеДополнительныеСвойства(Знач Источник, Знач Кэш, Знач ЭтоУдалениеОбъекта) Экспорт
	...
	Если Не ПропуститьВсеПроверкиПоОбъекту(Источник, СтруктураДополнительныхСвойств, Кэш) Тогда
...
Функция НоваяСтруктураДополнительныхСвойств(Знач МетаданныеОбъекта, Знач Кэш) Экспорт
	...
	СтруктураДляВозврата = Новый Структура(
			...
			?(Спец_ОбщегоНазначения.ПолучитьЗначениеПеременной("ЭтоНачалоРаботы1С") = Истина,
...
Функция ПропуститьВсеПодпискиНаСобытия(Знач Источник)
	...
	Если Спец_РаботаСКоллекциямиКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "Спец_ПропуститьПроверки", Ложь) = Истина Тогда
		Возврат Истина;
	КонецЕсли;
