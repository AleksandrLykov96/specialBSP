...
Функция СтрокаВДату(Знач СтрокаДляПреобразования, Кэш = Неопределено) Экспорт 
	...
	// Самая быстрая проверка XML даты, но отработает только с работающей компонентой
	Если Спец_КомпонентаДополнительныеФункцииКлиентСервер.СтрокаСоответствуетРегулярномуВыражению(СтрокаДляПреобразования, RegexDateTimeISO8601(), Кэш) Тогда
