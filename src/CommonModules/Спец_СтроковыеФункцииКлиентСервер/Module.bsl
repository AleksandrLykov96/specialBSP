...
Функция ПолучитьУникальныйИдентификатор() Экспорт
	...
	Возврат СокрЛП(ИдентификаторРезультат);
...
#Если Не ВебКлиент Тогда
	...
	ИначеЕсли Спец_РаботаСКоллекциямиКлиентСервер.ЭтоHTTPЗапросТип(ИсходныйОбъект, ТипОбъекта) Тогда
		...
		Если ИсходныйОбъект.Заголовки.Количество() Тогда
...
Функция СтрокаВДату(Знач СтрокаДляПреобразования, Кэш = Неопределено) Экспорт 
	...
	ВходнойТип = ТипЗнч(СтрокаДляПреобразования);
	Если ВходнойТип = Тип("Дата") Тогда
...
// Возвращает флаг того, является ли переданная строка НЕ локальным IP
// т.е. это корректный IP в пределах 255.255.255.255 и не 127.0.*.*
// 
// Параметры:
...
// Начиная с версии 8.3.23 работает всё (но только на сервере или на толстом клиенте)
...
Функция ПеревестиРазмерИзБайтовВЧитемыйВид(Знач КоличествоБайт) Экспорт
	...
	Если ТипЗнч(КоличествоБайт) <> Тип("Число") Или КоличествоБайт <= 0 Тогда
		Возврат "0 байт";
	КонецЕсли;
