...
//		* ТипМаркировки - ПеречислениеСсылка.Спец_ТипыМаркируемогоТовара - Тип маркировки
...
Функция ПолучитьДанныеDataMatrixКодаПриСканировании(Знач ШтрихКод) Экспорт
	...
	Для Каждого КлючЗначение Из ДанныеРазбора.ДанныеШтрихКода Цикл
		...
		ИначеЕсли КлючЗначение.Ключ = "00" Тогда
			...
			СтруктураШтрихКода.ЭтоТранспортнаяУпаковка = Истина;
	...
	Если Не ПустаяСтрока(СтруктураШтрихКода.GTIN) Тогда
		...
		#Если ТонкийКлиент Или ВебКлиент Или ТолстыйКлиентУправляемоеПриложение Тогда
			СтруктураШтрихКода.ТипМаркировки = Спец_ГенерацияЧтениеШтрихКодаВызовСервера.ТипМаркировкиПоGTIN(СтруктураШтрихКода.GTIN);
		#Иначе
			СтруктураШтрихКода.ТипМаркировки = Спец_ГенерацияЧтениеШтрихКода.ТипМаркировкиПоGTIN(СтруктураШтрихКода.GTIN);
		#КонецЕсли
