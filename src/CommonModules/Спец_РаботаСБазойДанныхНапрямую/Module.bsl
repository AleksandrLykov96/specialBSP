...
// Преобразует параметры в шаблоне текста запроса в имена таблиц в БД. Все наименования параметров передавать в квадратных скобках.
// ВСЕ ПАРАМЕТРЫ РЕГИСТРОЗАВИСИМЫЕ.
// 
//							---------- ВОЗМОЖНЫЕ ПАРАМЕТРЫ ----------
// 	* [ТекущийUnixTimestampВPostrgeSQL] - Подставляет функцию в PostgreSQL для получения текущего UixTimestamp
// 		Пример: "SELECT [ТекущийUnixTimestampВPostrgeSQL]" -> "SELECT extract(epoch from clock_timestamp())"
// 	
// 	* [ТипСтолбца_<Имя типа>] - Подставляет функцию, означающую нужный тип столбца для составной колонки в 1С.
// 		Возможные типы:
// 		** Null
// 		** Булево
// 		** Число
...
// 		** Строка
// 		** Ссылка
// 		Пример: "SELECT [ТипСтолбца_Ссылка]" -> "SELECT lykov_TypeToBytea(8)"
// 	
// 	* Наименование полного пути к нужной таблице
// 		Пример: "SELECT * FROM [Справочник.Спец_Базы]" -> "SELECT * FROM _reference123"
//				"SELECT * FROM [Справочник.Спец_Базы.HTTP]" -> "SELECT * FROM _reference123_vt456"
// 			
// 	* Наименование полного пути к нужному реквизиту
//		Пример: "SELECT [Справочник.Спец_Базы.Реквизит1]" -> "SELECT _fld123"
//				"SELECT [Справочник.Спец_Базы.HTTP.Реквизит1]" -> "SELECT _fld456"
//	
//	* Подставить UID предопределённого элемента (например, предопределенного справочника или перечисления)
//		Пример: "SELECT [Справочник.Спец_Базы.МДЛП]" -> "SELECT lykov_UIDToRRef('XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX')"
//				"SELECT [Перечисление.Спец_ВидыБаз.ГлавноеУправление]" -> "SELECT lykov_UIDToRRef('XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX')"
//	
//	* Если у реквизита составной тип, необходимо прописать имя нужного типа
//	(как в перечислении Спец_ТипыСоставныхПолейБазыДанных) в скобках после нужного реквизита
//		Пример: "SELECT [Справочник.Спец_Базы.СоставнойРеквизит(ИдентификаторСсылки)]" -> "SELECT _fld123_rref"
//				"SELECT [Справочник.Спец_Базы.СоставнойРеквизит(НомерТаблицы)]" -> "SELECT _fld123_rtref"
// 		
// 	* Чтобы подставить нужный код таблицы (для составного типа для НомерТаблицы),
// 	в скобках после нужной таблицы нужно написать "КодТаблицы"
// 		Пример: "SELECT [Справочник.Спец_Базы(КодТаблицы)]" -> "SELECT lykov_RTRefToBytea(123)"
// 		
...
Функция ПодставитьНаименованияТаблицДляТекстаЗапросаНапрямую(Знач ШаблонТекстаЗапроса, Кэш = Неопределено, Знач База = Неопределено) Экспорт
	...
	// Ищем наименования стандартных параметров (в квадратных скобках)
	...
	ГотовыйТекстЗапроса = СтрЗаменить(ГотовыйТекстЗапроса, "[ТипСтолбца_Null]"                 , "lykov_TypeToBytea(1)");
	ГотовыйТекстЗапроса = СтрЗаменить(ГотовыйТекстЗапроса, "[ТипСтолбца_Булево]"               , "lykov_TypeToBytea(2)");
	ГотовыйТекстЗапроса = СтрЗаменить(ГотовыйТекстЗапроса, "[ТипСтолбца_Число]"                , "lykov_TypeToBytea(3)");
	ГотовыйТекстЗапроса = СтрЗаменить(ГотовыйТекстЗапроса, "[ТипСтолбца_Дата]"                 , "lykov_TypeToBytea(4)");
	ГотовыйТекстЗапроса = СтрЗаменить(ГотовыйТекстЗапроса, "[ТипСтолбца_Строка]"               , "lykov_TypeToBytea(5)");
	ГотовыйТекстЗапроса = СтрЗаменить(ГотовыйТекстЗапроса, "[ТипСтолбца_Ссылка]"               , "lykov_TypeToBytea(8)");
	...
	Пока Истина Цикл
		...
		ИначеЕсли КоличествоЗаписей = 3 Тогда // Имя метаданных + реквизит или имя табличной части
			...
			Если Спец_ОбщегоНазначенияКлиентСервер.ЭтоСправочник(ИмяМетаданных, Кэш, Тип("Строка"))
				Или Спец_ОбщегоНазначенияКлиентСервер.ЭтоПеречисление(ИмяМетаданных, Кэш, Тип("Строка"))
				Или Спец_ОбщегоНазначенияКлиентСервер.ЭтоПланВидовХарактеристик(ИмяМетаданных, Кэш, Тип("Строка"))
				Или Спец_ОбщегоНазначенияКлиентСервер.ЭтоПланСчетов(ИмяМетаданных, Кэш, Тип("Строка"))
				Или Спец_ОбщегоНазначенияКлиентСервер.ЭтоПланВидовРасчета(ИмяМетаданных, Кэш, Тип("Строка"))
				Или Спец_ОбщегоНазначенияКлиентСервер.ЭтоПланОбмена(ИмяМетаданных, Кэш, Тип("Строка")) Тогда
...
Процедура ПодставитьНаименованияТаблицДляТекстаЗапросаНапрямую_ПодставитьИмяТаблицы(ТекстЗапроса, Знач ИмяПараметра, Знач ИмяМетаданных, Знач База)
	...
	Если ПозицияСоставногоТипа > 0 Тогда
		...
		Если ИмяПараметраВСкобках = "КодТаблицы" Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ИмяПараметра, "lykov_RTRefToBytea(" + КодТаблицы + ")");
			Возврат;
...
Процедура ПодставитьНаименованияТаблицДляТекстаЗапросаНапрямую_ПодставитьПредопределенноеЗначение(ТекстЗапроса, Знач ИмяПараметра, Знач ИмяМетаданных,
	...
	Если Не ПустаяСтрока(ГУИДПредопределенного) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ИмяПараметра, "lykov_UIDToRRef('" + ГУИДПредопределенного + "')");
	КонецЕсли;
