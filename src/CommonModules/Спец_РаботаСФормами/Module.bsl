...
Функция ДобавитьНовыйОтборНаПанель(Знач ПараметрыПанели, Знач ИмяПоля, Знач ВидСравнения, Знач Заголовок = "",
		Знач ЧерезСтандартныйПериод = Ложь, Знач ОписаниеТипаРеквизита = Неопределено, Кэш = Неопределено) Экспорт
	
	СтруктураОтбора = НоваяСтруктураОтбораДляПанелиОтборов(ИмяПоля, ВидСравнения, Заголовок, ЧерезСтандартныйПериод, ОписаниеТипаРеквизита, Кэш);
	ПараметрыПанели.ПараметрыОтборов.Добавить(СтруктураОтбора);
	
	ПараметрыПанели.ПорядокОтборов.Добавить(ИмяПоля);
...
Функция РаспределитьОтборыПоГруппам(Знач Форма, Знач ПараметрыПанели, Знач МассивРеквизитовДляДобавления, Знач Кэш)
	...
	Если ПустаяСтрока(ПараметрыПанели.ПолноеИмяОбъектаМетаданных) Тогда
...
Процедура ВывестиЭлементыФормы(Знач Форма, Знач ПараметрыПанели, Знач РодительЭлементов, Знач СтруктураГруппыОтборов)
	...
	Для Каждого ИмяОтбора Из ПараметрыПанели.ПорядокОтборов Цикл
		...
		Для Каждого СтруктураПоляВвода Из СтруктураГруппыОтборов.ПоляВвода Цикл
			
			Если СтруктураПоляВвода.ИмяПоля <> ИмяОтбора Тогда
				Продолжить;
			КонецЕсли;
		...
		КонецЦикла;
		Для Каждого СтруктураПоляКоличества Из СтруктураГруппыОтборов.ПоляКоличеств Цикл
			
			Если СтруктураПоляКоличества.ИмяПоля <> ИмяОтбора Тогда
				Продолжить;
			КонецЕсли;
		...
		КонецЦикла;
		Для Каждого СтруктураПоляФлага Из СтруктураГруппыОтборов.ПоляФлагов Цикл
			
			Если СтруктураПоляФлага.ИмяПоля <> ИмяОтбора Тогда
				Продолжить;
			КонецЕсли;
		...
		КонецЦикла;
		Для Каждого СтруктураПоляФлага Из СтруктураГруппыОтборов.ДекорацииДляПериодов Цикл
...
//		* ИмяРеквизита - Строка
//
Функция НоваяСтруктураОтбораДляПанелиОтборов(Знач ИмяПоля, Знач ВидСравнения, Знач Заголовок, Знач ЧерезСтандартныйПериод, Знач ОписаниеТипаРеквизита, Знач Кэш)
	
	ИмяРеквизита = Спец_СтроковыеФункцииКлиентСервер.ОчиститьИмяДляЗапроса(СтрШаблон("Спец_Отбор_%1_%2",
			ИмяПоля,
			Спец_КомпонентаДополнительныеФункцииКлиентСервер.ПолучитьУникальныйИдентификатор(Кэш)));
