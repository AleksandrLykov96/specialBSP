...
//	ПодключатьОборудованиеПринудительно - Булево - Флаг того, что подключаемое оборудование будет точно подключено.
//
Процедура ПриОткрытии(Знач Форма, Отказ = Ложь, Знач ОписаниеОповещенияПослеПодключенияТО = Неопределено, Знач ПодключатьОборудованиеПринудительно = Истина) Экспорт
	...
	Если Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(Форма, "Спец_ИспользоватьПодключаемоеОборудование")
		И (ПодключатьОборудованиеПринудительно Или Форма.Спец_ИспользоватьПодключаемоеОборудование) Тогда
		
		МассивТиповПодключаемогоОборудования = ПоддерживаемыеТипыПодключаемогоОборудования(Форма);
		Если МассивТиповПодключаемогоОборудования.Количество() Тогда
	...
	Если ЭтоУправляемаяФорма Тогда
		...
		Если Форма.ИмяФормы <> "РегистрСведений.Спец_ЛогиПоОбъектам.Форма.ФормаЗаписи" Тогда
...
Процедура ПриЗакрытии(Знач Форма, Знач ЗавершениеРаботы = Ложь) Экспорт
	...
	Если Не ЗавершениеРаботы
		...
		Спец_РаботаСФормамиКлиентСервер.СохранитьОтборыИзКрасивойПанелиОтборов(Форма, Истина);
...
// Процедура для клиентского вызова при изменении какого-либа элемента отбора (см. Спец_РаботаСФормами.ВывестиПанельОтборовНаФорму)
// 
// Параметры:
//	Форма - см. Спец_РаботаСФормами.ПриСозданииНаСервере.Форма
//	Элемент - ПолеФормы, ТаблицаФормы, КнопкаФормы - Элемент, откуда вызвано событие
//	СтандартнаяОбработка - Булево
//
Процедура ОбработатьСобытиеОтбора(Знач Форма, Знач Элемент, СтандартнаяОбработка = Истина) Экспорт
	...
	// Декорацию меняем принудительно (без всяких проверок и без ухода на сервер)
	Если Элемент.Имя = Спец_РаботаСФормамиКлиентСервер.ИмяДекорацииРазделителяГруппыОсновныхПанелей() Тогда // Какого ... 1С тут при первом вызове уходит на сервер???
	...
	СтруктураДополнительныеПараметры = НоваяСтруктураДополнительныеПараметрыРедактированияПериода(Форма, Элемент);
	...
	// Если это период - открываем форму.
	Если СтруктураДополнительныеПараметры.ЭтоПериод Тогда
...
Процедура ПослеРедактированияПериодаДляОтбора(Знач ПериодРезультат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоПериод Тогда
	...
	СтруктураИнформацииПоОтборам = Спец_ОбщегоНазначенияКлиентСервер.ПолучитьСоответствиеJSON(
			ДополнительныеПараметры.Форма[Спец_РаботаСФормамиКлиентСервер.ИмяРеквизитаДляИнформацииПоОтборам()], , Ложь);
	
	Если СтруктураИнформацииПоОтборам = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	// Если нужно выводить количество записей - то только через сервер (1С не даёт передавать форму / элементы в общих модулях)
	Если СтруктураИнформацииПоОтборам.ВывестиКоличествоЗаписей Тогда
		
		ДополнительныеПараметры.Форма.Подключаемый_ОбработкаСобытияОтбораВызовСервера(ДополнительныеПараметры.ИмяЭлемента);
		
	ИначеЕсли ДополнительныеПараметры.ИмяЭлемента = СтруктураИнформацииПоОтборам.ИмяСписка
		Или ДополнительныеПараметры.ИмяЭлемента = Спец_РаботаСФормамиКлиентСервер.ИмяКомандыДляОбновленияСписка() Тогда
		
		ДополнительныеПараметры.Форма.Элементы[СтруктураИнформацииПоОтборам.ИмяСписка].Обновить();
		
	ИначеЕсли ДополнительныеПараметры.ЭлементСуществует Тогда
...
//		* ИмяЭлемента - Строка
//		* ЭтоПериод - Булево
//		* ЭлементСуществует - Булево
//
Функция НоваяСтруктураДополнительныеПараметрыРедактированияПериода(Знач Форма, Знач Элемент)
	
	СтруктураРезультат = Новый Структура("Форма, ИмяЭлемента, ЭтоПериод, ЭлементСуществует",
			Форма,
			Элемент.Имя,
			Ложь,
			Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(Форма, Элемент.Имя));
	
	СтруктураРезультат.ЭтоПериод = СтруктураРезультат.ЭлементСуществует И ТипЗнч(Форма[Элемент.Имя]) = Тип("СтандартныйПериод");
