...
Процедура ОтправитьHTTPЗапросыКНужнойБазеАсинхронно_ПоИндексуТекущегоURL(Знач ИндексТекущегоURL, Знач МассивБазовыхURL, МассивЗапросовОсталось,
	...
	МассивРезультат = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientОтправитьЗапросыСинхронно(МассивЗапросовДляВыполнения, , , Кэш);
	ОтправитьHTTPЗапросыКНужнойБазеАсинхронно_ПоИндексуТекущегоURL(СтруктураДополнительныеПараметры.ИндексТекущегоURL + 1,
			СтруктураДополнительныеПараметры.МассивБазовыхURL,
			СтруктураДополнительныеПараметры.МассивЗапросовОсталось,
			СтруктураДополнительныеПараметры.МассивРезультатДляВозврата,
			МассивРезультат,
			,
			СтруктураДополнительныеПараметры.ОписаниеОповещения);
