...
Процедура ОтправитьHTTPЗапросыКНужнойБазеЧерезВнешнееСобытие_ПоИндексуТекущегоURL(Знач ИндексТекущегоURL, Знач МассивИлиСтруктураЗапросов,
	...
	Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientЗарегистрироватьЗапросыКВыполнению(МассивЗапросовДляВыполнения, Истина, , Кэш);
...
Процедура ОтправитьHTTPЗапросыКНужнойБазеЧерезВнешнееСобытие_ОбработатьРезультат(Знач РезультатЗапроса) Экспорт
	...
	СтруктураКэш = Кэш.КэшДляВнешнихСобытийHTTPЗапросов[РезультатЗапроса.ИдентификаторЗапроса];
	
	Кэш.КэшДляВнешнихСобытийHTTPЗапросов.Удалить(РезультатЗапроса.ИдентификаторЗапроса);
	
	Спец_ОбменСДругимиБазамиКлиентСервер.ОбработатьРезультатыВыполненияЗапросов(РезультатЗапроса);
	Если РезультатЗапроса.КодОтвета > 0 Тогда
		
		Оповестить(СтрЗаменить(РезультатЗапроса.ИдентификаторЗапроса, СтруктураКэш.Префикс, ""), РезультатЗапроса, "Спец_ОбменыСДругимиБазами");
