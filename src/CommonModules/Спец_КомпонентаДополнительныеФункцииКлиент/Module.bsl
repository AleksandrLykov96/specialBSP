...
//
// Параметры:
//	ОписаниеОповещенияПослеВыполнения - ОписаниеОповещения - Описание оповещения с параметрами, в который передается значение параметра (ВыполнитьОбработкуОповещения)
//	ОбъектКомпоненты - ОбъектВнешнейКомпоненты - объект компоненты
//	ИмяПараметра - Строка - имя параметра (на любом языке)
//
Процедура НачатьПолучениеПараметраИзКомпоненты(Знач ОписаниеОповещенияПослеВыполнения, Знач ОбъектКомпоненты, Знач ИмяПараметра) Экспорт
	
	Если ОбъектКомпоненты = Неопределено Или ПустаяСтрока(ИмяПараметра) Тогда Возврат; КонецЕсли;
	
	#Если Не ВебКлиент Тогда
		Выполнить(СтрШаблон("ОбъектКомпоненты.НачатьПолучение%1(ОписаниеОповещенияПослеВыполнения)", ИмяПараметра));
	#Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещенияПослеВыполнения, ОбъектКомпоненты[ИмяПараметра]);
	#КонецЕсли
...
Процедура PostgreSQL_ОбработатьРезультатВыполненияЗапроса(Знач РезультатВызова, Знач ПараметрыВызова, Знач ОписаниеОповещения) Экспорт
	...
	ВыполнитьОбработкуОповещения(ОписаниеОповещения,
			?(ПустаяСтрока(ПараметрыВызова[2]),
					Спец_ОбщегоНазначенияКлиентСервер.ПолучитьСоответствиеJSON(РезультатВызова, , Ложь),
					РезультатВызова));
...
Процедура ОписаниеОповещения_ОбработкаОшибки(Знач ИнформацияОбОшибке, СтандартнаяОбработка, Знач ДополнительныеПараметры) Экспорт
	...
	////Спец_ОбщегоНазначенияВызовСервера.ЗаписатьОшибки("Ошибка при вызове компоненты",
