...
//  Неопределено
//
Асинх Функция BaseFunction_ПодключитьЕслиНеПодключалиАсинх(Знач Кэш) Экспорт
...
//  Неопределено
//
Асинх Функция HTTPClient_ПодключитьЕслиНеПодключалиАсинх(Знач Кэш) Экспорт
...
//  Неопределено
//
Асинх Функция MySQL_ПодключитьЕслиНеПодключалиАсинх(Знач Кэш) Экспорт
...
//  Неопределено
//
Асинх Функция PostgreSQL_ПодключитьЕслиНеПодключалиАсинх(Знач Кэш, Знач БезИнициализацииПодключения) Экспорт
...
//
Асинх Функция PostgreSQLСоединениеУспешноУстановленоАсинх(Знач Кэш) Экспорт
...
Асинх Функция ОбработатьПереустановкуКомпонентАсинх(Знач СоответствиеДляПереустановки) Экспорт
	...
	Ждать ПодготовитьУстановитьВнешниеКомпонентыАсинх(СоответствиеДляПереустановки);
	ПроверитьУстановитьДоступностьКомпонентАсинх(СоответствиеДляПереустановки);
...
// Параметры:
//  СоответствиеДляПереустановки - см. ОбработатьПереустановкуКомпонентАсинх.СоответствиеДляПереустановки
// 
// Возвращаемое значение:
//  Неопределено
//
Асинх Функция ПодготовитьУстановитьВнешниеКомпонентыАсинх(Знач СоответствиеДляПереустановки)
	
	МассивОбещаний = Новый Массив(); // Массив из Обещание
	Для Каждого КлючЗначение Из СоответствиеДляПереустановки Цикл
		...
		МассивОбещаний.Добавить(УстановитьВнешнююКомпонентуАсинх(КлючЗначение.Ключ));
		
	КонецЦикла;
	
	Для Каждого Обещание Из МассивОбещаний Цикл
		Ждать Обещание;
	КонецЦикла;
...
//  Неопределено
//
Асинх Функция ПроверитьУстановитьДоступностьКомпонентАсинх(Знач СоответствиеДляПереустановки)
	
	ВсегоДляУстановки = СоответствиеДляПереустановки.Количество();
	Если ВсегоДляУстановки = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	МассивПараметровКомпонент = Новый Массив(); // см. Спец_ОбщегоНазначения.УстановитьПараметрРаботыСКомпонентамиМассовый.МассивСтруктурКомпонент
	
	Текущий = 1;
	Для Каждого КлючЗначение Из СоответствиеДляПереустановки Цикл
		
		Ждать Спец_ОбщегоНазначенияКлиент.ВывестиСостояниеАсинх("Идёт переустановка компоненты (спец)...", Текущий / ВсегоДляУстановки * 100, КлючЗначение.Ключ);
		
		ТекстФункцииДляВызова = СтрШаблон("%1Асинх(, Истина)", КлючЗначение.Значение.ИмяФункцииДляВызова);
		
		УспешноПодключено = Истина;
		Попытка
			...
			Если УспешноПодключено
				...
				ВерсияВКомпоненте = Ждать ПолучитьПараметрИзКомпонентыАсинх(КомпонентаРезультат, "ВерсияКомпоненты"); // Строка
				Если ВерсияВКомпоненте <> КлючЗначение.Значение.ТекущаяВерсия Тогда
					Спец_ОбщегоНазначенияКлиент.СообщитьПользователю("Различаются версии компонент для " + КлючЗначение.Ключ);
				КонецЕсли;
		...
		СтруктураДляОбновления = Новый Структура(
				
				"НаименованиеКомпоненты,
				|КомпонентаНеРаботает,
				|НомерВерсии",
				
				КлючЗначение.Ключ,
				Не УспешноПодключено,
				КлючЗначение.Значение.ТекущаяВерсия);
		
		МассивПараметровКомпонент.Добавить(СтруктураДляОбновления);
		Текущий = Текущий + 1;
		
	КонецЦикла;
	
	Ждать Спец_ОбщегоНазначенияКлиент.ВывестиСостояниеАсинх("Запись данных по компонентам в 1С (спец)", 100);
	
	#Если ТонкийКлиент Или ВебКлиент Или ТолстыйКлиентУправляемоеПриложение Тогда
		Спец_ОбщегоНазначенияВызовСервера.УстановитьПараметрРаботыСКомпонентамиМассовый(МассивПараметровКомпонент);
	#Иначе
		Спец_ОбщегоНазначения.УстановитьПараметрРаботыСКомпонентамиМассовый(МассивПараметровКомпонент);
	#КонецЕсли
