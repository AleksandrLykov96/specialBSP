...
Процедура ЛогироватьЗавершениеЗамера(Знач Источник, Знач КоличествоСекундПределДляЗаписи = 0, Кэш = Неопределено,
	...
	Если ЭтоПревышение Или Спец_ЛогированиеОбъектовВызовСервераПовтИсп.ДанныйУровеньЛогированияНужноПисать(ПредопределенноеЗначение("Перечисление.Спец_УровниЛогирования.Информация")) Тогда
		...
		ТекстЛога = "Окончание замера, время выполнения: " + Формат(ВремяВыполнения, "ЧРД=.; ЧН=0; ЧГ=; ЧФ='Ч сек.';");
...
Процедура ЗаписатьГотовыйЛогВРегистр(Знач Источник, Знач УровеньЛога, Знач ТекстДляЗаписи, Знач UnixTimestamp, Знач ДвоичныеДанныеФайла, Кэш)
	...
	Если Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLСоединениеУспешноУстановлено(Кэш.КомпонентаPostgreSQLДляЛогирования) Тогда
		...
		Если Кэш.КэшПараметровЗаполненияТаблиц["ТекстЗапросаЗаписьЛога"] = Неопределено Тогда Кэш.КэшПараметровЗаполненияТаблиц["ТекстЗапросаЗаписьЛога"] = ПодготовитьШаблонЗапросаЗаписиНапрямую(Кэш); КонецЕсли;
		...
		ТекстГотовыйЗапрос = СтрЗаменить(ТекстГотовыйЗапрос, "[&Источник]"       , Спец_ОбщегоНазначенияКлиентСервер.ПолучитьXMLСтрокуПоНормальному(Источник));
		...
		ТекстГотовыйЗапрос = СтрЗаменить(ТекстГотовыйЗапрос, "[&ТекстИзменений]" , ТекстДляЗаписи); 
		
		Если Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLВыполнитьЗапросБезРезультата(ТекстГотовыйЗапрос, "", Кэш.КомпонентаPostgreSQLДляЛогирования) Тогда Возврат; КонецЕсли;
