...
Процедура ОбновитьСтруктуруТаблицPostgreSQL(Знач ИдентификаторЗадания = "") Экспорт
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ОбновитьСтруктуруТаблицPostgreSQL",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Инициализация обновления структур таблиц PostgreSQL",
					 Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ОбновитьСтруктуруТаблицPostgreSQL",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Установка подключения к базе PostgreSQL",
					 Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ОбновитьСтруктуруТаблицPostgreSQL",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Инициализация дополнительных функций для работы с БД",
					 Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ОбновитьСтруктуруТаблицPostgreSQL", Перечисления.Спец_УровниЛогирования.Информация, "Запись ошибок", Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ОбновитьСтруктуруТаблицPostgreSQL", Перечисления.Спец_УровниЛогирования.Информация, "Завершение обновления структур таблиц PostgreSQL", Кэш);
...
Процедура КластеризацияОчисткаВсехТаблиц(Знач ИдентификаторЗадания = "", Кэш = Неопределено) Экспорт
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблиц",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Инициализация кластеризации / очистки таблиц PostgreSQL",
					 Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблиц", Перечисления.Спец_УровниЛогирования.Информация, "Идёт установка подключения к БД", Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблиц", Перечисления.Спец_УровниЛогирования.Информация, "Запись ошибок", Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблиц", Перечисления.Спец_УровниЛогирования.Информация, "Завершение кластеризации / очистки таблиц PostgreSQL", Кэш);
...
Процедура ЗакинутьТаблицыВОперативнуюПамять(Кэш = Неопределено) Экспорт
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ЗакинутьТаблицыВОперативнуюПамять",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Инициализация помещения таблиц в оперативную память в PostgreSQL",
					 Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ЗакинутьТаблицыВОперативнуюПамять", Перечисления.Спец_УровниЛогирования.Информация, "Идёт установка подключения к БД", Кэш);
	Спец_Проверить(Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLСоединениеУспешноУстановлено(Кэш),
				   "Не удалось подключиться к базе PostgreSQL!",
				   "Спец_PostgreSQL.ЗакинутьТаблицыВОперативнуюПамять");

	Спец_ЗаписатьЛог("Спец_PostgreSQL.ЗакинутьТаблицыВОперативнуюПамять", Перечисления.Спец_УровниЛогирования.Информация, "autoprewarm_dump_now()", Кэш);
	...
	Пока ВыборкаРезультат.Следующий() Цикл
		...
		Спец_ЗаписатьЛог("Спец_PostgreSQL.ЗакинутьТаблицыВОперативнуюПамять", Перечисления.Спец_УровниЛогирования.Информация, "pg_prewarm для "
																															  + ВыборкаРезультат.ИмяТаблицыХранения, Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ЗакинутьТаблицыВОперативнуюПамять", Перечисления.Спец_УровниЛогирования.Информация, "Завершение выполнения", Кэш);
...
Функция ПодготовитьСтруктуруДляУстановкиВPostgreSQL(Знач ПолноеИмяМетаданных) Экспорт
	...
	Возврат Новый Структура("ПолноеИмяМетаданных, МассивИндексов, МассивОграничений, СоответствиеТриггеров, Fillfactor, ИмяТабличногоПространства, ТабличныеЧасти, НазначениеТаблицы", ПолноеИмяМетаданных, Новый Массив(), Новый Массив(), Новый Соответствие(), 100, "", Новый Массив(), "Основная");
...
Функция ПодготовитьСтруктуруИндексаДляУстановкиВPostgreSQL(Знач ИменаСтолбцовДляИндексации) Экспорт
	...
	//@skip-check structure-consructor-too-many-keys
	СтруктураИндекса = Новый Структура("МассивПолей, Уникальный, КлассОператоров, Fillfactor, Кластер, ИмяТабличногоПространства, МассивУсловий", СтрРазделить(ИменаСтолбцовДляИндексации,
																																							   ",",
																																							   Ложь), Ложь, "", 95, Ложь, "", Новый Массив());
	
	// Подчищаем лишние пробелы (если есть)
	Для i = 0 По СтруктураИндекса.МассивПолей.ВГраница() Цикл
		СтруктураИндекса.МассивПолей[i] = СокрЛП(СтруктураИндекса.МассивПолей[i]);
	КонецЦикла;
...
Функция ПодготовитьСтруктуруОграниченияДляУстановкиВPostgreSQL(Знач ИменаСтолбцовДляОграничения) Экспорт
	...
	//@skip-check structure-consructor-too-many-keys
	СтруктураОграничения = Новый Структура("МассивПолей, Fillfactor, Кластер, ТипОграничения, ИмяТабличногоПространства", СтрРазделить(ИменаСтолбцовДляОграничения,
																																	   ",",
																																	   Ложь), 90, Ложь, "PRIMARY KEY", "");
	
	// Подчищаем лишние пробелы (если есть)
	Для i = 0 По СтруктураОграничения.МассивПолей.ВГраница() Цикл
		СтруктураОграничения.МассивПолей[i] = СокрЛП(СтруктураОграничения.МассивПолей[i]);
	КонецЦикла;
...
Процедура ОбновитьОсновныеТаблицыИзPostgreSQL(Знач МассивИменТаблиц, Знач ИдентификаторЗадания = "", Кэш = Неопределено, Знач ВсегоПунктов = 1, Знач ТекущийПункт = 1) Экспорт
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ОбновитьОсновныеТаблицыИзPostgreSQL", Перечисления.Спец_УровниЛогирования.Информация, "Идёт установка подключения к БД", Кэш);
	Спец_Проверить(Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLСоединениеУспешноУстановлено(Кэш),
				   "Не удалось подключиться к базе PostgreSQL!",
	...
	Для Каждого СтрокаТаблица Из СтруктураТаблиц.ТаблицаИменаТаблиц Цикл
		...
		Спец_ЗаписатьЛог("Спец_PostgreSQL.ОбновитьОсновныеТаблицыИзPostgreSQL", Перечисления.Спец_УровниЛогирования.Информация, ТекстСтатуса, Кэш);
	...
	Если МассивИменТаблиц.Количество() = 0 Тогда
		...
		Спец_РаботаСФоновымиЗаданиями.СообщитьСтатусВыполненияЗадания("Обновляем структуру таблиц (удаляем старые таблицы)",
																	  ИдентификаторЗадания,
																	  Спец_РаботаСФоновымиЗаданиямиКлиентСервер.РассчитатьКоэффициентСостояния(100, ТекущийПункт, ВсегоПунктов));

		ВыборкаРезультат = ПолучитьВыборкуПоТаблицамДляУдаления(СтруктураТаблиц.ТаблицаИменаТаблиц.ВыгрузитьКолонку("tableName"));
		Пока ВыборкаРезультат.Следующий() Цикл

			Спец_ЗаписатьЛог("Спец_PostgreSQL.ОбновитьОсновныеТаблицыИзPostgreSQL", Перечисления.Спец_УровниЛогирования.Информация, "Пометка на удаление устаревшей таблицы "
																																	+ ВыборкаРезультат.Ссылка, Кэш);
			СправочникОбъект = Спец_ОбщегоНазначения.ПолучитьОбъектСБлокированием(ВыборкаРезультат.Ссылка, Кэш);

			Спец_Проверить(СправочникОбъект <> Неопределено, "Что-то блокирует элемент " + СправочникОбъект.Ссылка, "Спец_PostgreSQL.ОбновитьОсновныеТаблицыИзPostgreSQL");
...
//		* Поля - ФиксированноеСоответствие из КлючИЗначение - Соответствие имени поля в 1С и столбцов в PostgreSQL (так как могут быть составные поля, то это массив имен столбцов):
//			** Ключ - Строка - Имя столбца в 1С
//			** Значение - ФиксированныйМассив из Строка
...
//		* МассивИменКлючевыхИндексов - ФиксированныйМассив из ФиксированныйМассив - Содержит все ключевые индексы данной таблицы,
//                                                  т.е. которые определяют уникальность записи (короче либо констрейнты, либо уникальные индексы) (имена в БД).
//                                                  Нужно для проверки того, что ПоляДляПроверкиКонфликтов при записи напрямую совпадают с данным массивом.
//                                                  Если не совпадают, то перед записью в БД придётся присоединять исходную таблицу
//                                                  (иначе ошибка 'Нет уникального ограничения или ограничения-исключения, соответствующего указанию ON CONFLICT')
//
Функция ПолучитьСтруктуруТаблицыПоИмениМетаданных(Знач НаименованиеВ1С, Знач НазначениеТаблицы = "Основная") Экспорт

	Спец_ЗаписатьЛог("Спец_PostgreSQL.ПолучитьСтруктуруТаблицыПоИмениМетаданных", Перечисления.Спец_УровниЛогирования.Информация, "Получение информации по " + НаименованиеВ1С);

	УстановитьПривилегированныйРежим(Истина);
	
	// Основные данные по таблице
	СтруктураРезультат = НоваяСтруктураТаблицыВPostgreSQL();
	Если ПустаяСтрока(НаименованиеВ1С) Тогда
...
#КонецОбласти
	...
	Пока ВыборкаРезультат.Следующий() Цикл
		...
		ИмяДляСоответствия = ?(ПустаяСтрока(ВыборкаРезультат.ИмяСтолбцаВ1С), ВыборкаРезультат.ИмяСтолбцаВБД, ВыборкаРезультат.ИмяСтолбцаВ1С);
		...
		СтруктураРезультат.Поля[ИмяДляСоответствия].Добавить(СтруктураПоля.ИмяСтолбцаВБД);
		СтруктураРезультат.ПоляPostgreSQL[ВыборкаРезультат.ИмяСтолбцаВБД] = СтруктураПоля;
	...
	// Массив имен столбцов для уникальности записи
	ВыборкаРезультат = ПолучитьСтолбцыУникальныхИндексовИОграничений(СтруктураРезультат.Ссылка);
	Пока ВыборкаРезультат.Следующий() Цикл
		СтруктураРезультат.МассивИменКлючевыхИндексов.Добавить(СтрРазделить(ВыборкаРезультат.Столбцы, ","));
	КонецЦикла;
...
Функция МассивСтандартныхИндексовДляУстановки()
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.МассивСтандартныхИндексовДляУстановки", Перечисления.Спец_УровниЛогирования.Информация, "Формирование массива структур индексов по умолчанию");

	МассивСтруктурДляОбновления = Новый Массив(); // Массив из см. ПодготовитьСтруктуруДляУстановкиВPostgreSQL

#Область Константы
	
	// Для всех констант ставим fillfactor 100
	Для Каждого МетаданныеКонстанта Из Метаданные.Константы Цикл

		СтруктураДляИндексации = ПодготовитьСтруктуруДляУстановкиВPostgreSQL(МетаданныеКонстанта.ПолноеИмя());
		СтруктураДляИндексации.Fillfactor = 100;

		СтруктураОграничения = ПодготовитьСтруктуруОграниченияДляУстановкиВPostgreSQL("_recordkey");
...
#Область Перечисления
	...
	// Для всех перечислений ставим fillfactor 100
...
Процедура ПерестроитьИндексыFillFactors(Знач ИдентификаторЗадания, Знач Кэш, Знач ВсегоПунктов, Знач ТекущийПункт)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ПерестроитьИндексыFillFactors",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Инициализация перестроения FillFactor индексов по умолчанию",
					 Кэш);
	...
	Для Каждого СтрокаИндекс Из МассивИндексовДляПерестроения Цикл
		МассивИменИндексов.Добавить(СокрЛП(СтрокаИндекс.name));
	КонецЦикла;
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Спец_ЗаписатьЛог("Спец_PostgreSQL.ПерестроитьИндексыFillFactors",
						 Перечисления.Спец_УровниЛогирования.Информация,
						 "Обновляем информацию по изменённым индексам / таблицам в 1С",
						 Кэш);
		ОбновитьОсновныеТаблицыИзPostgreSQL(РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Наименование"), ИдентификаторЗадания, Кэш, ВсегоПунктов, ТекущийПункт + 1);
...
Функция ПолучитьМассивИндексовДляПерестроения(Знач Кэш)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ПолучитьМассивИндексовДляПерестроения", Перечисления.Спец_УровниЛогирования.Информация, "Получение массива контрейнстов из PostgreSQL", Кэш);
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ПолучитьМассивИндексовДляПерестроения",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Формирование массива индексов PostrgeSQL для перестроения",
					 Кэш);
...
Процедура УстановитьFillFactorДляИндексовПоУмолчанию(Знач МассивИндексовДляПерестроения, Знач Кэш)
	...
	Для Каждого СтрокаИндекс Из МассивИндексовДляПерестроения Цикл
		...
		Спец_ЗаписатьЛог("Спец_PostgreSQL.УстановитьFillFactorДляИндексовПоУмолчанию",
						 Перечисления.Спец_УровниЛогирования.Информация,
						 "Установка fillfactor по умолчанию для индекса " + СтрокаИндекс.name,
						 Кэш);
...
Функция ПодготовитьДанныеИз1СПоТаблицам()
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ПодготовитьДанныеИз1СПоТаблицам", Перечисления.Спец_УровниЛогирования.Информация, "Получение данных по таблицам из 1С");
...
Функция ПодготовитьДанныеИзPostgreSQLПоТаблицам(Знач МассивИменТаблиц, Знач Кэш)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ПодготовитьДанныеИзPostgreSQLПоТаблицам", Перечисления.Спец_УровниЛогирования.Информация, "Получение данных по таблицам из PostgreSQL", Кэш);
	
	//@skip-check structure-consructor-too-many-keys
	СтруктураДляВозврата = Новый Структура("ТаблицаИменаТаблиц,
										   |ТаблицаСтолбцы,
										   |ТаблицаИндексы,
										   |ТаблицаТриггеры", НоваяТаблицаИменТаблиц(), НоваяТаблицаСтолбцы(), НоваяТаблицаИндексы(), НоваяТаблицаТриггеры());
	...
	Спец_РаботаСКоллекциямиКлиентСервер.ЗначениеВМассив(МассивИменТаблиц);
	Если МассивИменТаблиц.Количество() Тогда
	...
	Спец_Проверить(СтруктураРезультат.Успешно, СтруктураРезультат.ТекстОшибки, "Спец_PostgreSQL.ПодготовитьДанныеИзPostgreSQLПоТаблицам");
...
Процедура ОбновитьДанныеПоТаблицеВСтруктуреТаблицPostgreSQL(Знач СтрокаТаблица, Знач СтруктураТаблицPostgreSQL, Знач ТаблицаДанныеИз1С, Знач Кэш)
	...
	Если СтрокаДанныеИз1С <> Неопределено И Не ПустаяСтрока(СтрокаДанныеИз1С.Метаданные) И (СтрНачинаетсяС(СтрокаДанныеИз1С.Назначение, "Итоги")
		...
		Если МассивСтрокИз1С.Количество() Тогда
...
Функция НайтиДанныеПоТаблицеВСтруктуреТаблицPostgreSQL(Знач СтрокаТаблица, Знач Кэш)
	...
	Если МассивИмяТаблицы.Количество() > 1 Тогда
		...
		ЭлементРодитель = Справочники.Спец_СтруктураТаблицБазыДанных.НайтиПоНаименованию(ИмяРодителя, Истина, Справочники.Спец_СтруктураТаблицБазыДанных.ПустаяСсылка());
	...
	ЭлементСправочникОбъект = ?(ЗначениеЗаполнено(ЭлементСправочник), ЭлементСправочник.ПолучитьОбъект(), Справочники.Спец_СтруктураТаблицБазыДанных.СоздатьЭлемент());
...
Процедура ОбновитьОсновныеДанныеПоТаблицеВСтруктуреТаблицPostgreSQL(Знач ЭлементСправочникОбъект, Знач СтрокаТаблица, Знач СтрокаДанныеИз1С, Кэш)
	...
	Если НачалоFillfactor > 0 Тогда
		...
		ЭлементСправочникОбъект.ПроцентЗаполнения = Спец_КомпонентаДополнительныеФункцииКлиентСервер.СтрокаВЧисло(?(КонецFillFactor = 0,
																													...
																													Сред(ЭлементСправочникОбъект.ПараметрыХранения,
																														 НачалоFillfactor + 11,
																														 КонецFillFactor - НачалоFillfactor - 11)), Кэш);
...
Процедура ОбновитьДанныеПоСтобцамВСтруктуреТаблицPostgreSQL(Знач СтрокаТаблица, Знач ЭлементСправочникОбъект, Знач ТаблицаСтолбцы, Знач СтрокаДанныеИз1С)
	...
	ЭлементСправочникОбъект.ТекстЗапросаДляСозданияКопииВременнойТаблицы = СтрШаблон(ШаблонЗапроса, СтрСоединить(МассивПолейДляЗапроса, "," + Символы.ПС + Символы.Таб));
...
Процедура ОбновитьДанныеПоИндексамВСтруктуреТаблицPostgreSQL(Знач ЭлементСправочникОбъект, Знач СтрокаТаблица, Знач ТаблицаИндексы)
	...
	Для Каждого СтрокаИндекс Из ТаблицаИндексыПоСтроке Цикл
		...
		Иначе
			...
			НоваяСтрокаИндекс.НеключевыеСтолбцы = ПолучитьИменаНеключевыхСтолбцовДляИндекса(ЭлементСправочникОбъект.Столбцы, СтрокаИндекс.columnsIndex, СтрокаИндекс.keyAtts);
...
Функция ПолучитьДанныеПоПеречислениямИзБД(Знач НаименованиеТаблицы, Знач Кэш)
	...
	СтруктураРезультат = Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLВыполнитьЗапрос(ТекстЗапроса, , , Кэш);
	Спец_Проверить(СтруктураРезультат.Успешно, СтруктураРезультат.ТекстОшибки, "Спец_PostgreSQL.ПолучитьДанныеПоПеречислениямИзБД");
...
Функция ПолучитьДанныеПоПредопределеннымЭлементамОбъекта(Знач НаименованиеТаблицы, Знач Кэш)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.ПолучитьДанныеПоПредопределеннымЭлементамОбъекта",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Получение данных по предопределённым значениям " + НаименованиеТаблицы,
					 Кэш);
	...
	СтруктураРезультат = Спец_КомпонентаДополнительныеФункцииКлиентСервер.PostgreSQLВыполнитьЗапрос(ТекстЗапроса, , , Кэш);
	Спец_Проверить(СтруктураРезультат.Успешно, СтруктураРезультат.ТекстОшибки, "Спец_PostgreSQL.ПолучитьДанныеПоПредопределеннымЭлементамОбъекта");
...
Функция ПолучитьДанныеПоТекущемуУзлуПланаОбмена(Знач МенеджерПланаОбмена, Знач НаименованиеТаблицы, Знач Кэш)
	...
	Если СтруктураРезультат.РезультатЗапроса.Количество() Тогда
...
Процедура ОбновитьДанныеПоРазмеруТаблицыВСтруктуреТаблицPostgreSQL(Знач ЭлементСправочникОбъект, Знач Кэш)
	...
	ЭлементСправочникОбъект.РазмерТаблицыБезИндексов = Спец_PostgreSQLКлиентСервер.ПолучитьРазмерОбъектаВPostgreSQL(ЭлементСправочникОбъект.Наименование, Ложь, Кэш);
...
Процедура РеструктуризироватьТаблицыВPostgreSQL(Знач ИдентификаторЗадания, ТекстОшибки, Знач Кэш, Знач ВсегоПунктов, Знач ТекущийПункт)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.РеструктуризироватьТаблицыВPostgreSQL",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Инициализация реструктуризации таблицы в PostgreSQL",
					 Кэш);
	...
	Для Каждого СтруктураДляОбновления Из МассивСтруктурДляОбновления Цикл
		...
		Текущий = Текущий + 1;
		Спец_РаботаСФоновымиЗаданиями.СообщитьСтатусВыполненияЗадания(СтрШаблон("Обновление структур таблиц (только для текущей базы, %1)",
		...
		ПараметрыТаблицы = Спец_PostgreSQLВызовСервераПовтИсп.ПолучитьСтруктуруТаблицыПоИмениМетаданных(СтруктураДляОбновления.ПолноеИмяМетаданных,
																										СтруктураДляОбновления.НазначениеТаблицы);
		...
		Для Каждого СтруктураДляОбновленияТабличнойЧасти Из СтруктураДляОбновления.ТабличныеЧасти Цикл
			...
			НужноОбновление = ОбработатьСтруктуруДляИндексации(ПараметрыТаблицыТабличнойЧасти.НаименованиеВPostgreSQL, СтруктураДляОбновленияТабличнойЧасти, Кэш);
	...
	Если МассивОбновленныхТаблиц.Количество() Тогда
...
Функция ОбработатьСтруктуруДляИндексации(Знач НаименованиеВPostgreSQL, Знач СтруктураДляОбновления, Знач Кэш)
	...
	Спец_Проверить(СсылкаНаТаблицуДляОбновления, "Не удалось найти таблицу в справочнике " + НаименованиеВPostgreSQL, "Спец_PostgreSQL.ОбработатьСтруктуруДляИндексации: "
																													  + СтруктураДляОбновления.ПолноеИмяМетаданных);

	Спец_ЗаписатьЛог("Обновление структуры таблиц PostgreSQL", Перечисления.Спец_УровниЛогирования.Информация, "Обработка структуры для индексации для таблицы "
...
#Область Индексы
	...
	Для Каждого СтруктураИндекса Из СтруктураДляОбновления.МассивИндексов Цикл
		...
		НовоеИмяИндекса = ШаблонИмяОбъекта + "index_" + СтрЗаменить(Спец_КомпонентаДополнительныеФункцииКлиентСервер.ПолучитьУникальныйИдентификатор(Кэш), "-", "_");
		...
		Для Каждого ИмяПоля Из СтруктураИндекса.МассивПолей Цикл
			...
			Если ИмяПоля = "_keyfield" Или ИмяПоля = "_numberprefix" Или ИмяПоля = "_recordkey" Тогда

				МассивТекстПолей.Добавить(СтрШаблон("%1 %2ASC NULLS LAST", ИмяПоля, ?(ПустаяСтрока(СтруктураИндекса.КлассОператоров), "", СтруктураИндекса.КлассОператоров + " ")));
		...
		ТекстУсловий = ?(СтруктураИндекса.МассивУсловий.Количество() > 0, "WHERE " + СтрСоединить(СтруктураИндекса.МассивУсловий, Символы.ПС + "		AND"), "");
		...
		ТекстЗапроса = СтрШаблон("%1 %2
								 ...
								 ТекстУсловий,
								 ?(ПустаяСтрока(СтруктураИндекса.ИмяТабличногоПространства), "", "TABLESPACE " + СокрЛП(СтруктураИндекса.ИмяТабличногоПространства)));
		...
		Иначе
			...
			НужноДобавлять = (СтруктураИндекса.Уникальный <> СтрокаТекущийИндекс.Уникальный) Или (СтруктураИндекса.Кластер <> СтрокаТекущийИндекс.Кластеризован)
							 Или ?(ПустаяСтрока(СтруктураИндекса.ИмяТабличногоПространства),
								   Ложь,
								   ВРег(СокрЛП(СтруктураИндекса.ИмяТабличногоПространства)) <> ВРег(СокрЛП(СтрокаТекущийИндекс.ТабличноеПространство))) Или (ТекущийFillFactor
																																							 <> СтруктураИндекса.Fillfactor);
	...
	Если МассивТекстЗапроса.Количество() Тогда

		ГотовыйТекстЗапроса = Спец_РаботаСБазойДанныхНапрямую.ПодставитьНаименованияТаблицДляТекстаЗапросаНапрямую(СтрСоединить(МассивТекстЗапроса, Символы.ПС
																																					+ Спец_КонстантыКлиентСервер.РазделительЗапросовВPostgreSQL()),
																												   Кэш);
		ТекстОшибки = "";
		...
		Если Не Успешно Тогда
			...
			Спец_Проверить(МассивДляУдаления.Количество() > 0, ТекстОшибки, "Спец_PostgreSQL.ОбработатьСтруктуруДляИндексации: " + СтруктураДляОбновления.ПолноеИмяМетаданных);
...
#Область Ограничения
	...
	Для Каждого СтруктураОграничения Из СтруктураДляОбновления.МассивОграничений Цикл
		...
		НовоеИмяОграничения = ШаблонИмяОбъекта + "constraint_" + СтрЗаменить(Спец_КомпонентаДополнительныеФункцииКлиентСервер.ПолучитьУникальныйИдентификатор(Кэш), "-", "_");
		...
		ТекстСтолбцы = Спец_РаботаСБазойДанныхНапрямую.ПодставитьНаименованияТаблицДляТекстаЗапросаНапрямую(СтрСоединить(МассивПолейДляОграничения, ","), Кэш);
	...
	Для Каждого СтрокаЛишнееОграничение Из ТаблицаТекущихОграничений Цикл
		...
		ТекстЗапросаДляУдаления = СтрШаблон("ALTER TABLE IF EXISTS public.%1 DROP CONSTRAINT IF EXISTS %2", НаименованиеВPostgreSQL, СтрокаЛишнееОграничение.ИмяОграничения);
...
Процедура КластеризацияОчисткаВсехТаблицVACUUM(Знач ИдентификаторЗадания, ТекстСообщений, Знач Кэш)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблицVACUUM", Перечисления.Спец_УровниЛогирования.Информация, "Инициализация VACUUM для таблиц", Кэш);

	ТекущийДеньНедели = ДеньНедели(ТекущаяДатаСеанса());

	НужноПринудительноЗапустить = Спец_ПолучитьЗначениеПеременной("PostgreSQL_ЗапуститьПолнуюОчисткуПринудительно", Ложь) = Истина;
	Если НужноПринудительноЗапустить Или ТекущийДеньНедели = 6 Или ТекущийДеньНедели = 7 Тогда // FULL VACUUM лучше запускать только по выходным (блокирует таблицу, может надолго зависнуть)
	...
	Пока ВыборкаРезультат.Следующий() Цикл
		...
		Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблицVACUUM", Перечисления.Спец_УровниЛогирования.Информация, Скрипт, Кэш);
...
Процедура КластеризацияОчисткаВсехТаблицCLUSTER(Знач ИдентификаторЗадания, ТекстСообщений, Знач Кэш)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблицCLUSTER", Перечисления.Спец_УровниЛогирования.Информация, "Инициализация CLUSTER для таблиц", Кэш);

	ТекущийКвартал = НачалоКвартала(ТекущаяДатаСеанса());

	НужноПринудительноЗапустить = Спец_ПолучитьЗначениеПеременной("PostgreSQL_ЗапуститьПолнуюОчисткуПринудительно", Ложь) = Истина;
	Если Не НужноПринудительноЗапустить Тогда
		...
		Если НачалоКвартала(ДатаПоследнейКластеризации) >= ТекущийКвартал Тогда
			Возврат;
		КонецЕсли;
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблицCLUSTER", Перечисления.Спец_УровниЛогирования.Информация, Скрипт, Кэш);
	...
	Спец_УстановитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ДатаПоследнейКластеризации, ТекущийКвартал);
...
Процедура КластеризацияОчисткаВсехТаблицREINDEX(Знач ИдентификаторЗадания, ТекстСообщений, Знач Кэш)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблицREINDEX", Перечисления.Спец_УровниЛогирования.Информация, "Инициализация REINDEX для таблиц", Кэш);
	...
	Если Не НужноПринудительноЗапустить Тогда
		...
		Если НачалоКвартала(ДатаПоследнейРеиндексации) >= ТекущийКвартал Тогда
			Возврат;
		КонецЕсли;
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблицREINDEX", Перечисления.Спец_УровниЛогирования.Информация, Скрипт, Кэш);
	Спец_РаботаСФоновымиЗаданиями.СообщитьСтатусВыполненияЗадания(Скрипт, ИдентификаторЗадания, 1);
	...
	Спец_УстановитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ДатаПоследнейРеиндексации, ТекущийКвартал);
...
Процедура КластеризацияОчисткаВсехТаблицСбросСтатистики(Знач ИдентификаторЗадания, ТекстСообщений, Знач Кэш)
	...
	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблицСбросСтатистики",
					 Перечисления.Спец_УровниЛогирования.Информация,
					 "Инициализация сброса статистики для таблиц",
					 Кэш);
	...
	Если НачалоМесяца(ДатаСбросаСтатистики) >= ТекущийМесяц Тогда
		Возврат;
	КонецЕсли;

	Спец_ЗаписатьЛог("Спец_PostgreSQL.КластеризацияОчисткаВсехТаблицСбросСтатистики", Перечисления.Спец_УровниЛогирования.Информация, "Сброс статистики", Кэш);
	...
	Спец_УстановитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ДатаСбросаСтатистики, ТекущийМесяц);
	...
	Если ЗначениеЗаполнено(НоваяСтрокаСтолбец.ТипСоставногоПоля) Тогда
		...
		Если НоваяСтрокаСтолбец.ТипСоставногоПоля = Перечисления.Спец_ТипыСоставныхПолейБазыДанных.Число И ОписаниеТипов.КвалификаторыЧисла.Разрядность > 0 Тогда
			...
			НоваяСтрокаСтолбец.ИмяТипаДанныхДляPostgreSQL = ?(ОписаниеТипов.КвалификаторыЧисла.РазрядностьДробнойЧасти > 0,
															  ...
															  СтрШаблон("numeric(%1)", Формат(ОписаниеТипов.КвалификаторыЧисла.Разрядность, "ЧДЦ=0; ЧН=0; ЧГ=")));
...
Функция УжеЕстьПереопределенияИндекса(Знач ИсходнаяСтрока)
	...
	Возврат СтрНайти(ИсходнаяСтрока, "WITH") > 0 Или СтрНайти(ИсходнаяСтрока, "WHERE") > 0 Или СтрНайти(ИсходнаяСтрока, "TABLESPACE") > 0 Или СтрНайти(ИсходнаяСтрока, "BTREE") = 0;
...
Функция ПолучитьСтолбцыТаблицыВPostgreSQL(Знач СсылкаНаТаблицу)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СтруктураТаблицPostgreSQLСтолбцы.ИмяСтолбца КАК ИмяСтолбцаВБД,
	...
	|	СтруктураТаблицPostgreSQLСтолбцы.ИмяТипаДанныхДляPostgreSQL КАК ИмяТипаДанныхВБД,
...
//		* Столбцы - Строка
//
Функция ПолучитьСтолбцыУникальныхИндексовИОграничений(Знач СсылкаНаТаблицу)

	Запрос = Новый Запрос();
	Запрос.Текст =
	
	"ВЫБРАТЬ
	|	ОграниченияТаблицы.Столбцы
	|ИЗ
	|	Справочник.Спец_СтруктураТаблицБазыДанных.Ограничения КАК ОграниченияТаблицы
	|ГДЕ
	|	ОграниченияТаблицы.Ссылка = &Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ИндексыТаблицы.Столбцы
	|ИЗ
	|	Справочник.Спец_СтруктураТаблицБазыДанных.Индексы КАК ИндексыТаблицы
	|ГДЕ
	|	ИндексыТаблицы.Ссылка = &Ссылка
	|	И ИндексыТаблицы.Уникальный";
...
//		* МассивИменКлючевыхИндексов - Массив из Массив
//
