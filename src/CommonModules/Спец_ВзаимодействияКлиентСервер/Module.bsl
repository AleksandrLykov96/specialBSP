...
Процедура ОтправитьEMail(Знач МассивАдресовПолучателей, Знач ТемаСообщения, Знач ТекстСообщения, Знач Отправитель = "auto@farmani.ru",
	...
	Спец_ЗаписатьЛог("Спец_ВзаимодействияКлиентСервер.ОтправитьEMail",
			ПредопределенноеЗначение("Перечисление.Спец_УровниЛогирования.Информация"),
			"Отправка сообщения: " + ТемаСообщения,
			Кэш);
	...
	Если СтруктураДополнительныхДанныхПоПисьму.Количество() Тогда
