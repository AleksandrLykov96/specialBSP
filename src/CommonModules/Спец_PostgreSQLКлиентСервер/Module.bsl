...
Функция НайтиСоздатьОбъектыВPostgreSQL(Знач ИсходныеДанные, Знач СтруктураПараметров, ТекстОшибок = "", Кэш = Неопределено) Экспорт
	...
	Если ЕстьТЧ
		...
		СтруктураРезультат.Успешно = ПройтиТабличныеЧастиПослеЗаписиНапрямую(СтруктураРезультат, МассивТабличныхЧастей, СтруктураПараметров, ИсходныеДанные, Кэш, ТекстОшибок);
...
Функция УдалитьДанныеПоОбъекту1СВPostgreSQL(Знач ИсходныеДанные, Знач ИмяМетаданныхИз1С, ТекстОшибок = "", Кэш = Неопределено) Экспорт
	...
	Если МассивТЧ.Количество() > 0 Тогда
		...
		ГотовыйТекстЗапроса = СтрСоединить(МассивТекстЗапроса, Спец_КонстантыКлиентСервер.РазделительЗапросовВPostgreSQL());
	...
	Если Спец_РаботаСКоллекциямиКлиентСервер.ЭтоТаблицаЗначений(ИсходныеДанные, ТипИсходныхДанных) Тогда
		...
		Для Каждого Колонка Из ИсходныеДанные.Колонки Цикл
			...
			Если Колонка.ТипЗначения.Типы().Количество() > 0 Тогда
				...
				СтруктураРезультат.СоответствиеВсехПолейPostrgeSQL[СтруктураПоля.ИмяСтолбцаВPostgreSQL] = СтруктураПоля;
...
Функция ПодготовитьСтруктуруПараметровДляПомещенияВоВременнуюТаблицу(Знач СтруктураПараметровДляПомещения, Знач СтруктураТаблицыPostgreSQL)
	...
	Для Каждого ИмяКолонки Из СтруктураПараметровДляПомещения.МассивИменКолонокОбновлять Цикл
		...
		Иначе // Руками добавленное поле
			...
			МассивСтруктурПолей = СтруктураПараметровДляПомещения.СоответствиеВсехПолейPostrgeSQL.Получить(ИмяКолонки);
			
			Спец_РаботаСКоллекциямиКлиентСервер.ЗначениеВМассив(МассивСтруктурПолей);
			Для Каждого СтруктураПоля Из МассивСтруктурПолей Цикл
				
				СтруктураРезультат.СоответствиеДляИсходныхДанных[ИмяКолонки] = Спец_РаботаСКоллекциямиКлиентСервер.ЗначениеВМассиве(СтруктураПоля);
...
//  ТекстОшибок - Строка
//
Функция ПройтиТабличныеЧастиПослеЗаписиНапрямую(Знач СтруктураРезультат, Знач МассивТабличныхЧастей, Знач СтруктураПараметров, Знач ИсходныеДанные, Знач Кэш, ТекстОшибок)
...
Функция НайтиСоздатьДанныеПоОбъекту1СВPostgreSQL_СформироватьТекстЗапроса_БазовыйЗапрос(Знач ИмяТаблицыPostgreSQL, Знач СтруктураПараметров, Знач СтруктураРезультатПомещения, Знач ТекстСвязиТаблиц, Знач ФормироватьГУИДСсылкиПоПолю, МассивПолейТаблицыДляЗаписи)
	...
	Для Каждого КлючЗначение Из СтруктураРезультатПомещения.СоответствиеВсехПолейPostrgeSQL Цикл
		...
		Для Каждого СтруктураПоля Из МассивПолейДляОбработки Цикл
			...
			ИначеЕсли СтруктураРезультатПомещения.МассивИменКолонокОбновлять.Найти(СтруктураПоля.ИмяВ1С) <> Неопределено
				Или (СтруктураПараметров.ФормироватьГУИДСсылкиПоПолю И СтруктураПараметров.ИмяПоляГУИДа = СтруктураПоля.ИмяСтолбцаВPostgreSQL)
				Или СтруктураПоля.ИмяСтолбцаВPostgreSQL = "postgresql_key" Тогда
			...
			Если Не (СтруктураПараметров.ФормироватьГУИДСсылкиПоПолю И СтруктураПараметров.ИмяПоляГУИДа = СтруктураПоля.ИмяСтолбцаВPostgreSQL)
				И СтруктураПоля.ИмяСтолбцаВPostgreSQL <> "postgresql_key" Тогда
...
Функция СтруктураПоляПоТипуЗначения(Знач ИмяПоля, Знач ТипОбъекта, Знач Кэш)
	...
	СтруктураПоля.ИмяСтолбцаВPostgreSQL = ИмяПоля;
