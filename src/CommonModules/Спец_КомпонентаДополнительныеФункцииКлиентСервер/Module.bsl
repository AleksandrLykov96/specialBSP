...
Функция ПолучитьКомпонентуBaseFunction(Знач КонстантноеИмяКомпоненты = Истина, Знач ПринудительноеПодключение = Ложь) Экспорт
	...
	Если Не КомпонентуМожноПодключать(НаименованиеКомпоненты, ПринудительноеПодключение) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Попытка
		
		Возврат Спец_ОбщегоНазначенияКлиентСервер.ПодключитьВнешнююКомпонентуДляРаботы(НаименованиеКомпоненты,
				"l_BaseFunction",
				"BaseFunction",
...
Функция СтрокаСоответствуетРегулярномуВыражению(Знач СтрокаДляПроверки, Знач РегулярноеВыражение, Кэш = Неопределено) Экспорт
	...
	// Наконец-то! Скоро, надеюсь, и на клиенте появится
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		Возврат СтрПодобнаПоРегулярномуВыражению(СтрокаДляПроверки, РегулярноеВыражение);
	#КонецЕсли
	// -- Спец_БСП.Версия1С_23_ВышеИлиРавно
	
	BaseFunction_ПодключитьЕслиНеПодключали(Кэш);
	Возврат ?(Кэш.КомпонентаBaseFunction <> Неопределено,
			Кэш.КомпонентаBaseFunction.СтрокаСоответствуетРегулярномуВыражению(СтрокаДляПроверки, РегулярноеВыражение) = Истина,
...
//  ИмяПользователя - Строка - Только для см. Спец_ОбменСДругимиБазамиКлиентСервер.ОтправитьЗапросыКНужнойБазе.
//  						   Позволяет переопределить имя пользователя для базовой авторизации.
//	Пароль - Строка - Только для см. Спец_ОбменСДругимиБазамиКлиентСервер.ОтправитьЗапросыКНужнойБазе.
//  						   Позволяет переопределить пароль для базовой авторизации.
//  ЛучшеЧерезФайлы - Булево - Если ИСТИНА, то ТелоЗапроса (если массив или что-то подобное) будет передан через файлы
// 
// Возвращаемое значение:
//  Структура:
...
Функция HTTPClientПолучитьСтруктуруЗапроса(Знач URL, Знач ИдентификаторЗапроса = "default", Знач ТипЗапроса = "POST", Знач ТелоЗапроса = "",
		Знач СоответствиеЗаголовков = Неопределено, Знач ИмяФайлаДляЧтения = "", Знач ИмяФайлаРезультат = "", Знач ЛучшеЧерезФайлы = Ложь) Экспорт
...
Функция PostgreSQLВыполнитьЗапрос(Знач ТекстЗапроса, Знач КоличествоСтрокВРезультате = -1, Знач ИмяФайлаРезультат = "", Кэш = Неопределено) Экспорт
	...
	Попытка
		...
		Спец_Проверить(РезультатВыполнения, "Не удалось выполнить запрос! Вернулся некорректный результат из компоненты");
		Возврат Спец_ОбщегоНазначенияКлиентСервер.ПолучитьСоответствиеJSON(РезультатВыполнения, , Ложь);
		
	Исключение
		
		Возврат Новый Структура("Успешно, ТекстОшибки, РезультатЗапроса",
				Ложь,
				?(ПустаяСтрока(Кэш.КомпонентаPostgreSQL.ТекстПоследнейОшибки),
						Спец_ОбщегоНазначенияКлиентСервер.ПодробноеПредставлениеОшибкиЧерезОбработкуОшибок(ИнформацияОбОшибке()),
						Кэш.КомпонентаPostgreSQL.ТекстПоследнейОшибки),
				Новый Массив());
...
Функция PostgreSQLВыполнитьЗапросБезРезультата(Знач ТекстЗапроса, ТекстОшибки = "", Кэш = Неопределено) Экспорт
	...
	Спец_Проверить(PostgreSQLСоединениеУспешноУстановлено(Кэш), "Не удалось выполнить запрос!");
...
Процедура PostgreSQLИнициализироватьДополнительныеФункцииДляРаботыС1С(Кэш = Неопределено) Экспорт
	...
	ТекстИнициализации = ТекстЗапросаДляИнициализацииДополнительныхФункцийВPostgreSQL(БазоваяСтоимостьОперации);
...
// Отправляет сигнал для завершения подвисших запросов в PostrgeSQL,
// время выполнения которых превышает КоличествоСекундДружественноеЗавершение (pg_cancel_backend)
// и КоличествоСекундЖесткогоЗавершения (pg_terminate_backend). Удаляются запросы только с чтением / модификацией базы данных
// (т.е. только те, которые содержат 'INSERT INTO', 'SELECT', 'UPDATE', 'DELETE FROM').
// Если в качестве количества секунд передано 0, то ничего не будет сделано (для выбранного типа завершения).
// ВНИМАНИЕ: В conf должно быть track_activities = on! Иначе будут убиты все сеансы (т.к. статус у них disbled)
// 
// Параметры:
//	КоличествоСекундДружественноеЗавершение - Число - Количество секунд ограничение для "дружественного" завершения запроса (pg_cancel_backend, SIGINT)
//	КоличествоСекундЖесткогоЗавершения - Число - Количество секунд ограничение для "жёсткого" завершения запроса (pg_terminate_backend, SIGTERM)
//	Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
//
Процедура PostgreSQLУдалитьПодвисшиеЗапросы(Знач КоличествоСекундДружественноеЗавершение = 0, Знач КоличествоСекундЖесткогоЗавершения = 0, Кэш = Неопределено) Экспорт
	
	Спец_Проверить(PostgreSQLСоединениеУспешноУстановлено(Кэш), "Не удалось подключить внешнюю компоненту PostgreSQL");
	
	ТекстЗапросаДляВыполнения = ТекстЗапросаДляУдаленияПодвисшихСеансовPostgreSQL(КоличествоСекундДружественноеЗавершение, КоличествоСекундЖесткогоЗавершения, Кэш);
	PostgreSQLВыполнитьЗапрос(ТекстЗапросаДляВыполнения, -1, "", Кэш);
...
////	// Причём на тип внешней компоненты не проверить, поэтому обращаемся к любому свойству.
	...
	Если Не Кэш.Свойство("КомпонентаBaseFunction") Тогда Кэш.Вставить("КомпонентаBaseFunction", ПолучитьКомпонентуBaseFunction()); КонецЕсли;
...
//	см. HTTPClientПолучитьСтруктуруЗапроса
//
Функция НоваяСтруктураHTTPЗапроса() Экспорт
...
// Параметры:
//  БазоваяСтоимостьОперации - Число
// 
// Возвращаемое значение:
//  Строка
//
Функция ТекстЗапросаДляИнициализацииДополнительныхФункцийВPostgreSQL(Знач БазоваяСтоимостьОперации) Экспорт
	...
	Возврат ТекстИнициализации;
	
КонецФункции

// Параметры:
//  КоличествоСекундДружественноеЗавершение - Число
//  КоличествоСекундЖесткогоЗавершения - Число
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
//
// Возвращаемое значение:
//  Строка
//
Функция ТекстЗапросаДляУдаленияПодвисшихСеансовPostgreSQL(Знач КоличествоСекундДружественноеЗавершение, Знач КоличествоСекундЖесткогоЗавершения, Кэш) Экспорт
	
	МассивТекстЗапроса = Новый Массив(); // Массив из Строка
	Если КоличествоСекундДружественноеЗавершение > 0 Тогда
		...
		МассивТекстЗапроса.Добавить(СтрШаблон(
				...
				|WHERE state != 'idle'
				|	AND usename = 'sa'
				|	AND xact_start <= CURRENT_TIMESTAMP - interval '%1 second'
				|	AND (query LIKE '%%INSERT INTO%%'
				|		OR query LIKE '%%SELECT%%'
				|		OR query LIKE '%%UPDATE%%'
				|		OR query LIKE '%%DELETE FROM%%')",
				
				Спец_PostgreSQLКлиентСервер.ПолучитьЗначениеДляPostgreSQLВТекстЗапроса(КоличествоСекундДружественноеЗавершение, Тип("Число"), Кэш)));
		
	КонецЕсли;
	Если КоличествоСекундЖесткогоЗавершения > 0 Тогда
		...
		МассивТекстЗапроса.Добавить(СтрШаблон(
				...
				Спец_PostgreSQLКлиентСервер.ПолучитьЗначениеДляPostgreSQLВТекстЗапроса(КоличествоСекундЖесткогоЗавершения, Тип("Число"), Кэш)));
...
////	// Причём на тип внешней компоненты не проверить, поэтому обращаемся к любому свойству.
	...
	Если Не Кэш.Свойство("КомпонентаHTTPClient") Тогда
		Кэш.Вставить("КомпонентаHTTPClient", ПолучитьКомпонентуHTTPClient());
	КонецЕсли;
...
////	// Причём на тип внешней компоненты не проверить, поэтому обращаемся к любому свойству.
	...
	Если Не БезИнициализацииПодключения Тогда
		...
		Исключение
			Кэш.Вставить("КомпонентаPostgreSQL", ПолучитьКомпонентуPostgreSQL());
		КонецПопытки;
...
//		* UnixTimestampОтвета - Число - UnixTimestamp, когда программа начала записывать результат в JSON (т.е. пришёл ответ)
...
////	// Причём на тип внешней компоненты не проверить, поэтому обращаемся к любому свойству.
	...
	Если Не Кэш.Свойство("КомпонентаMySQL") Тогда
		Кэш.Вставить("КомпонентаMySQL", ПолучитьКомпонентуMySQL());
	КонецЕсли;
...
//  ПринудительноеПодключение - Булево
// 
// Возвращаемое значение:
//  Булево
//
Функция КомпонентуМожноПодключать(Знач НаименованиеКомпоненты, Знач ПринудительноеПодключение) Экспорт
	
	Если Спец_ПолучитьКонстанту(ПредопределенноеЗначение("ПланВидовХарактеристик.Спец_Константы.ОтключитьИспользованиеКомпонент"), Ложь) = Истина Тогда
		Возврат Ложь;
	ИначеЕсли ПринудительноеПодключение Тогда
		Возврат Истина;
	КонецЕсли;
	
	#Если ТонкийКлиент Или ВебКлиент Или ТолстыйКлиентУправляемоеПриложение Тогда
		
		Если Спец_ОбщегоНазначенияВызовСервера.КомпонентаНеРаботаетУПользователя(НаименованиеКомпоненты) Тогда
			Возврат Ложь;
		КонецЕсли;
		
	#Иначе
		
		Если Спец_ОбщегоНазначения.КомпонентаНеРаботаетУПользователя(НаименованиеКомпоненты) Тогда
			Возврат Ложь;
		КонецЕсли;
