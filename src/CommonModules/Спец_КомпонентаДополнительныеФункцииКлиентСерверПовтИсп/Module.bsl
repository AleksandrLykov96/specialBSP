...
Функция ПолучитьКомпонентуBaseFunction(Знач КонстантноеИмяКомпоненты = Истина, Знач ПринудительноеПодключение = Ложь) Экспорт
	...
	Попытка
		...
		Если Не ПринудительноеПодключение
			И (НеПытатьсяРаботатьСКомпонентами()
					Или Спец_КомпонентаДополнительныеФункцииВызовСервера.КомпонентаНеРаботаетУПользователя("ОбщийМакет.Спец_КомпонентаLykovExtFunctions")) Тогда
...
Функция ПолучитьКомпонентуHTTPClient(Знач КонстантноеИмяКомпоненты = Истина, Знач ПринудительноеПодключение = Ложь) Экспорт
	...
	Попытка
		...
		Если Не ПринудительноеПодключение
			И (НеПытатьсяРаботатьСКомпонентами()
					Или Спец_КомпонентаДополнительныеФункцииВызовСервера.КомпонентаНеРаботаетУПользователя("ОбщийМакет.Спец_КомпонентаLykovHTTPClient")) Тогда
...
Функция ПолучитьКомпонентуPostgreSQL(Знач КонстантноеИмяКомпоненты = Истина, Знач ПринудительноеПодключение = Ложь) Экспорт
	...
	Попытка
		...
		Если Не ПринудительноеПодключение
			И (НеПытатьсяРаботатьСКомпонентами()
					Или Спец_КомпонентаДополнительныеФункцииВызовСервера.КомпонентаНеРаботаетУПользователя("ОбщийМакет.Спец_КомпонентаLykovPostgreSQL")) Тогда
...
Функция ПолучитьКомпонентуMySQL(Знач КонстантноеИмяКомпоненты = Истина, Знач ПринудительноеПодключение = Ложь) Экспорт
	...
	Попытка
		...
		Если Не ПринудительноеПодключение
			И (НеПытатьсяРаботатьСКомпонентами()
					Или Спец_КомпонентаДополнительныеФункцииВызовСервера.КомпонентаНеРаботаетУПользователя("ОбщийМакет.Спец_КомпонентаLykovMySQL")) Тогда
...
Функция ПолучитьСоответствиеВерсийМоихКомпонент() Экспорт
	...
	СоответствиеРезультат["ОбщийМакет.Спец_КомпонентаLykovHTTPClient"]   = НоваяСтруктураВерсииМоейКомпоненты("1.0.0.18", "ПолучитьКомпонентуHTTPClient");
...
//  Булево
//
Функция НеПытатьсяРаботатьСКомпонентами()
	
	Возврат Спец_ПолучитьКонстанту(ПредопределенноеЗначение("ПланВидовХарактеристик.Спец_Константы.ОтключитьИспользованиеКомпонент"), Ложь) = Истина;
