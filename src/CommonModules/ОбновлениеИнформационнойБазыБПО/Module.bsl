...
Процедура ОбновитьПодключаемоеОборудование(Параметры = Неопределено) Экспорт
	...
	ШаблонТекстаОшибки = НСтр("ru = 'Не удалось обработать обновление подключаемого оборудования по причине: %1'", Метаданные.ОсновнойЯзык.КодЯзыка);
	...
	Если МенеджерОборудованияВызовСервера.ИспользуетсяОфлайнОборудование() Тогда
		МодульМенеджерОфлайнОборудованияВызовСервераПереопределяемый = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("МенеджерОфлайнОборудованияВызовСервераПереопределяемый");
...
Процедура ОбновитьШаблоныЭтикетокИЦенников(Параметры = Неопределено) Экспорт    
	...
	Если МенеджерОборудованияВызовСервера.ИспользуетсяПечатьЭтикетокИЦенников() Тогда
		МодульПечатьЭтикетокИЦенниковБПО = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ПечатьЭтикетокИЦенниковБПО");
...
Процедура ОбработатьОфлайнОборудование(СсылкаНаПодключаемоеОборудование)
	...
	Если Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(ПодключаемоеОборудованиеОбъект, "УдалитьТипОфлайнОборудования") Тогда
	...
	КонецЕсли;
	Если Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(ПодключаемоеОборудованиеОбъект, "УдалитьВидТранспортаОфлайнОбмена") Тогда
	...
	Если Спец_РаботаСКоллекциямиКлиентСервер.ЕстьРеквизитОбъекта(ПодключаемоеОборудованиеОбъект, "УдалитьИдентификаторWebСервисОборудования") Тогда
