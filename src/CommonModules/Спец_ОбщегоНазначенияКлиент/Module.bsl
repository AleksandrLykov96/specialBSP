...
Асинх Функция ЗагрузитьФайлыНаСерверАсинх(Знач ПараметрыДиалога, Знач ИдентификаторФормы = Неопределено, Кэш = Неопределено) Экспорт
	...
	Ждать Спец_КомпонентаДополнительныеФункцииКлиент.BaseFunction_ПодключитьЕслиНеПодключалиАсинх(Кэш);
...
Асинх Функция ОтправитьHTTPЗапросыАсинх(Знач МассивИлиСтруктураЗапросов, Кэш = Неопределено) Экспорт
	...
	Для Каждого СтруктураЗапроса Из МассивИлиСтруктураЗапросов Цикл
		...
		HTTPОтвет = Неопределено; // HTTPОтвет
...
////	#КонецЕсли
////	
////	ОбработатьСтруктуруПараметровЗапускаПриложенияОбщиеКоманды(СтруктураПараметров);
...
// Параметры:
//  СтруктураПараметров - см. Спец_ОбщегоНазначения.ПолучитьСтруктуруПараметровДляЗапуска
//
Процедура ОбработатьСтруктуруПараметровЗапускаПриложенияОбщиеКоманды(Знач СтруктураПараметров)
	...
	// Так как серверный кэш сбрасывается на диск каждые 20 минут (при НЕ использовании), а после скидывания на диск он перестаёт работать "Ошибка формата потока"
	// Делаем его периодическое обновление
	ПодключитьОбработчикОжидания("Спец_ОбновитьСерверныйКэш", Спец_КонстантыКлиентСервер.СекундВМинуте(20) - 5);
