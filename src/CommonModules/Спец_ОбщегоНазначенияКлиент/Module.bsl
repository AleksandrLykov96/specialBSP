...
Асинх Функция ВывестиСостояниеАсинх(Знач ТекстСообщения, Знач Прогресс = 0, Знач Пояснение = "", Знач Картинка = Неопределено,
	...
	ОбещаниеОбработкиПрерывания = Неопределено; // Обещание
	Если ВозможностьПрерывания Тогда
	...
	НепосредственныйВыводСостояния(ТекстСообщения, Прогресс, Пояснение, Картинка);
	Если ОбещаниеОбработкиПрерывания <> Неопределено Тогда
...
Асинх Процедура ПриНачалеРаботыСистемыАсинх()
	...
	ОбещаниеРаботаСФайлами = ПодключитьРасширениеРаботыСФайламиАсинх();
...
Асинх Функция ВыполнитьКомандуСистемыАсинх_Windows(Знач СтрокаКоманды, Знач ДождатьсяЗавершения, Знач ВозвращатьРезультат)
	...
	Если ВозвращатьРезультат Тогда
		...
		// Маздай не умеет в UTF-8 при выводе в файл
		ДополнительныеПараметры = Спец_ОбщегоНазначенияКлиентСервер.ПолучитьСтруктуруДополнительныхПараметровДляВыполненияКоманды("cp866");
		...
		Возврат ВыполнитьКомандуСистемыАсинх_ОбработатьРезультат(ДополнительныеПараметры);
...
// Параметры:
//  ДополнительныеПараметры - см. Спец_ОбщегоНазначенияКлиентСервер.ПолучитьСтруктуруДополнительныхПараметровДляВыполненияКоманды
// 
// Возвращаемое значение:
