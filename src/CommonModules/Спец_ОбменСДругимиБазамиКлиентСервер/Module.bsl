...
Функция ПолучитьВсеОбъектыМетаданныхИзБазы(Знач База, Знач ИмяОбъектаОтбор = "all", ТекстыОшибок = "", Знач Кэш = Неопределено, Знач ОписаниеОповещения = Неопределено) Экспорт
	...
	СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктуруЗапроса("hs/specialExchange/metadata/" + ИмяОбъектаОтбор,
...
Функция СтруктураТаблицБДИзДругойБазы(Знач База, ТекстыОшибок = "", Знач Кэш = Неопределено, Знач ОписаниеОповещения = Неопределено) Экспорт
	...
	СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктуруЗапроса("hs/specialExchange/metadata/BD",
...
Функция ЗаписатьПараметрыОбменовВБазу(Знач База, ТекстыОшибок = "", Знач Кэш = Неопределено, Знач ОписаниеОповещения = Неопределено) Экспорт
	...
	#Если Клиент Тогда
		...
		Если Не ЧерезFTP И ОписаниеОповещения <> Неопределено Тогда
			...
			СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктуруЗапроса("hs/specialExchange/parameters",
...
Функция ПропинговатьБазу(Знач База, Знач СначалаЛокальныеАдресаБазы = Истина, Знач ТолькоНужныеАдреса = Истина, ТекстыОшибок = "", Знач Кэш = Неопределено, Знач ОписаниеОповещения = Неопределено) Экспорт
	...
	СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктуруЗапроса("hs/specialExchange/ping",
			"Пинг",
			"GET");
...
Функция ОтправитьОшибкуВБазу(Знач ТекстОшибкиИлиИнформацияОбОшибке, Знач ИмяПроцедуры = "", Знач База = Неопределено,
	...
	СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктуруЗапроса("hs/specialExchange/errors",
			"ЗаписатьОшибку",
			"PUT",
			ДляОтправки);
...
Функция ПолучитьFTPСоединениеДляОбменаСБазой(Знач ЭтоБазаОтправитель, Знач База, Знач ДополнительныйКаталогДляПерехода = "", Знач Кэш = Неопределено) Экспорт
	...
	FTPСоединение = Спец_ОбщегоНазначенияКлиентСервер.ПолучитьFTPСоединениеПоURLИПерейтиККаталогу(АдресFTP, Спец_КонстантыКлиентСервер.СекундВЧасе());
	Если FTPСоединение = Неопределено Тогда
...
Функция ОтправитьЗапросыКНужнойБазе(Знач База, Знач МассивИлиСтруктураЗапросов, Знач СначалаЛокальныеАдресаБазы = Истина, Знач ЭтоКороткиеЗапросы = Ложь,
	...
	Для i = 0 По МассивБазовыхURL.ВГраница() Цикл
		...
		СтруктураБазовогоURL        = Спец_РаботаСКоллекциямиКлиентСервер.СкопироватьСтруктуру(МассивБазовыхURL.Получить(i), Ложь);
		МассивЗапросовДляВыполнения = Новый Массив;
		СтрокаАвторизации           = СтрШаблон("%1:%2@", СтруктураБазовогоURL.Пользователь, СтруктураБазовогоURL.Пароль);
		ИмяПользователяДляОтправки  = СтруктураБазовогоURL.Пользователь;
		ПарольДляОтправки           = СтруктураБазовогоURL.Пароль;
		
		Если НуженЧистыйURL Тогда
			ЧистыйURL = СтрЗаменить(СтруктураБазовогоURL.БазовыйURL, СтрокаАвторизации, "");
		КонецЕсли;
		...
		Если i = 0 Тогда
			...
			Для Каждого СтруктураЗапроса Из МассивИлиСтруктураЗапросов Цикл
				...
				Если Не ПустаяСтрока(СтруктураЗапроса.ИмяПользователя) Тогда
					ИмяПользователяДляОтправки = СтруктураЗапроса.ИмяПользователя;
				КонецЕсли;
				Если Не ПустаяСтрока(СтруктураЗапроса.Пароль) Тогда
					ПарольДляОтправки = СтруктураЗапроса.Пароль;
				КонецЕсли;
		...
		Если НуженЧистыйURL Тогда
			...
			Кэш.КомпонентаHTTPClient.HTTPAUTH   = 1;
			Кэш.КомпонентаHTTPClient.Username   = ИмяПользователяДляОтправки;
			Кэш.КомпонентаHTTPClient.Password   = ПарольДляОтправки;
		...
		Иначе
			...
			Если ИмяПользователяДляОтправки <> СтруктураБазовогоURL.Пользователь Тогда
				
				НоваяСтрокаАвторизации = СтрШаблон("%1:%2@", ИмяПользователяДляОтправки, ПарольДляОтправки);
				Для Каждого СтруктураЗапроса Из МассивЗапросовДляВыполнения Цикл
					СтруктураЗапроса.URL = СтрЗаменить(СтруктураЗапроса.URL, СтрокаАвторизации, НоваяСтрокаАвторизации);
				КонецЦикла;
		...
		Для Каждого СтруктураЗапроса Из МассивРезультат Цикл
			...
			Если СтруктураЗапроса.КодОтвета > 0 Тогда
				...
				Спец_РаботаСКоллекциямиКлиентСервер.УдалитьЗначенияИзМассиваСтруктур(МассивИдентификаторовДляПовторногоВыполнения, "ИдентификаторЗапроса", СтруктураЗапроса.ИдентификаторЗапроса);
...
Функция ОжидатьЗавершенияВыполненияЗарегистрированныхКОтправкеЗапросов(Знач База, Знач МассивИлиСтруктураЗапросов, Знач СначалаЛокальныеАдресаБазы = Истина,
	...
	МассивРезультатДляВозврата            = Новый Массив;
	МассивЗапросовДляПовторногоВыполнения = Спец_РаботаСКоллекциямиКлиентСервер.СкопироватьРекурсивно(МассивИлиСтруктураЗапросов);
	...
	Для Каждого СтруктураЗапроса Из МассивРезультат Цикл
		...
		Если СтруктураЗапроса.КодОтвета > 0 Тогда
			...
			СтруктураЗапросаДляВыполненияЗаново = Спец_РаботаСКоллекциямиКлиентСервер.НайтиСтрокиВКоллекцииСтрок(СтруктураОтбора, МассивЗапросовДляПовторногоВыполнения, Истина);
			
			Если СтруктураЗапросаДляВыполненияЗаново <> Неопределено Тогда
				Спец_РаботаСКоллекциямиКлиентСервер.УдалитьЗначениеИзМассива(МассивЗапросовДляПовторногоВыполнения, СтруктураЗапросаДляВыполненияЗаново);
			КонецЕсли;
	...
	Если МассивЗапросовДляПовторногоВыполнения.Количество() Тогда
...
Процедура ОбработатьРезультатыВыполненияЗапросов(Знач МассивРезультатовЗапросов, ТекстыОшибок = "") Экспорт
	...
	Иначе
		...
		Для Каждого СтруктураРезультат Из МассивРезультатовЗапросов Цикл
			...
			// Если пароль содержится в URL - удаляем его
			СтруктураURI = Спец_ОбщегоНазначенияКлиентСервер.СтруктураURI(СтруктураРезультат.URL);
			URLДляОтображения = ?(ПустаяСтрока(СтруктураURI.Пароль),
					СтруктураРезультат.URL,
					СтрЗаменить(СтруктураРезультат.URL, СтруктураURI.Пароль, "*****"));
