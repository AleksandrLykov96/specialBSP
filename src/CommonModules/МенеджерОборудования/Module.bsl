...
Процедура ОбновитьПоставляемыеДрайвера() Экспорт
	...
	Если МенеджерОборудованияВызовСервера.ИспользуетсяУстройстваВвода() Тогда
		МодулиОбновления.Добавить(Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеУстройстваВводаОбновление"));       
	КонецЕсли;
	Если МенеджерОборудованияВызовСервера.ИспользуетсяЧекопечатающиеУстройства() Тогда
		МодулиОбновления.Добавить(Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеЧекопечатающиеУстройстваОбновление"));       
	КонецЕсли;
	Если МенеджерОборудованияВызовСервера.ИспользуетсяПлатежныеСистемы() Тогда
		МодулиОбновления.Добавить(Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеПлатежныеСистемыОбновление"));       
	КонецЕсли;          
	Если МенеджерОборудованияВызовСервера.ИспользуетсяДисплеиПокупателя() Тогда
		МодулиОбновления.Добавить(Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеДисплеиПокупателяОбновление"));       
	КонецЕсли;
	Если МенеджерОборудованияВызовСервера.ИспользуетсяВесовоеОборудование() Тогда
		МодулиОбновления.Добавить(Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеВесовоеОборудованиеОбновление"));       
	КонецЕсли;
	Если МенеджерОборудованияВызовСервера.ИспользуетсяТерминалыСбораДанных() Тогда
		МодулиОбновления.Добавить(Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеТерминалыСбораДанныхОбновление"));       
	КонецЕсли;
	Если МенеджерОборудованияВызовСервера.ИспользуетсяПринтерыЭтикеток() Тогда
		МодулиОбновления.Добавить(Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеПринтерыЭтикетокОбновление"));       
	КонецЕсли;
	Если МенеджерОборудованияВызовСервера.ИспользуетсяСчитывательRFID() Тогда
		МодулиОбновления.Добавить(Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеСчитывательRFIDОбновление"));       
	КонецЕсли;
...
Функция ОписаниеКомандыПанелиАдминистрирования(ИмяКомандыВФорме, АдресНастроек) Экспорт
	...
	Если Команда = Неопределено Тогда
		ВызватьИсключение СтрШаблон(
			...
			ИмяКомандыВФорме);
	КонецЕсли;
	ОписаниеКоманды = Спец_ОбщегоНазначения.ПреобразоватьСтрокуТаблицыЗначенийВСтруктуру(Команда);
...
Процедура ВыполнитьКомандуПанелиАдминистрирования(Знач Форма, Знач ПараметрыВызова, Знач Источник, Результат = Неопределено) Экспорт
	...
	ПараметрыСтрока = "";
	Если ПараметрыЭкспортнойПроцедуры <> Неопределено И ПараметрыЭкспортнойПроцедуры.Количество() > 0 Тогда
		Для Индекс = 0 По ПараметрыЭкспортнойПроцедуры.ВГраница() Цикл 
			ПараметрыСтрока = ПараметрыСтрока + "Параметры[" + XMLСтрока(Индекс) + "],";
		КонецЦикла;
		ПараметрыСтрока = Сред(ПараметрыСтрока, 1, СтрДлина(ПараметрыСтрока) - 1);
	КонецЕсли;
	
	Выполнить Обработчик + "(" + ПараметрыСтрока + ")";
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удается заблокировать удаляемый драйвер %1.'", 
				Метаданные.ОсновнойЯзык.КодЯзыка), Выборка.Представление);
...
Функция ДанныеФормы(Форма)
	...
	Для Номер = 1 По Количество Цикл
		...
		Если ПустаяСтрока(Команда.Идентификатор) Тогда
			Команда.Идентификатор = "Авто_" + Спец_ОбщегоНазначения.ПолучитьКонтрольнуюСуммуСтрокой(Команда.Менеджер + "/" + Команда.ИмяФормы + "/" + Команда.Обработчик);
		КонецЕсли;
	КонецЦикла;
