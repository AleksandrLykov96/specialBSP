...
Функция ПараметрыРаботыКлиентаПриЗапуске() Экспорт
	...
	Если ИспользуетсяБСП() Тогда
		МодульСтандартныеПодсистемыКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("СтандартныеПодсистемыКлиент");
...
Процедура ОбработкаВнешнегоСобытия(Источник, Событие, Данные) Экспорт
	...
	Если Не Результат Тогда
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='При обработке внешнего события от устройства произошла ошибка.'") + Символы.ПС + ОписаниеОшибки);
	КонецЕсли;
...
#КонецЕсли
	...
	Если ДанныеДрайвера.ПодключениеИзМакета Тогда
		...
		Иначе
			ПараметрыУстановки = ПодключаемоеОборудованиеДрайверКлиент.ПараметрыУстановкиКомпоненты();
			ПараметрыУстановки.ТекстПояснения = ТекстСообщения;
			ПодключаемоеОборудованиеДрайверКлиент.УстановитьКомпонентуИзМакета(Оповещение, ДанныеДрайвера.ИмяМакетаДрайвера, ПараметрыУстановки);
		КонецЕсли;
	Иначе
//		ПараметрыУстановки = ПодключаемоеОборудованиеДрайверКлиент.ПараметрыУстановки();
//		ПараметрыУстановки.ТекстПояснения = ТекстСообщения; 
//		ПодключаемоеОборудованиеДрайверКлиент.УстановитьКомпоненту(Оповещение, ДанныеДрайвера.ИдентификаторОбъекта, ДанныеДрайвера.ВерсияДрайвера, ПараметрыУстановки);
...
Процедура НачатьВыполнениеКоманды_ПослеОткрытииЧека(РезультатВыполнения, ПараметрыВыполнениеКоманды)  Экспорт
	...
	Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяЧекопечатающиеУстройства() Тогда
		МодульОборудованиеЧекопечатающиеУстройстваВызовСервера = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеЧекопечатающиеУстройстваВызовСервера");
	...
	Если РезультатВыполнения.ПродолжитьПечать Тогда        
		...
		Если НЕ ПустаяСтрока(ДополнительныеПараметры.АдресЧека) Тогда // Необходима печать слип-чека алко-продукции.
			...
			Если ПечатьСлипЧека И МенеджерОборудованияКлиентПовтИсп.ИспользуетсяЧекопечатающиеУстройства() Тогда
				...
				МодульОборудованиеЧекопечатающиеУстройстваКлиентСервер = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеЧекопечатающиеУстройстваКлиентСервер");
				МодульОборудованиеЧекопечатающиеУстройстваВызовСервера = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеЧекопечатающиеУстройстваВызовСервера");
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если ПараметрыВыполнениеКоманды.Команда = "CheckFiscalization" 
			...
			И ВыводQRКодаЧекаНаДисплейПокупателяПослеФискализации() Тогда
				...
				МодульОборудованиеДисплеиПокупателяКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеДисплеиПокупателяКлиент");
	...
	Иначе
		...
		Если ПараметрыВыполнениеКоманды.Команда = "Settlement" Тогда
			...
			Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяБазоваяФункциональность() Тогда
				...
				Попытка
					МодульЖурналРегистрацииКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ЖурналРегистрацииКлиент");
...
Процедура НачатьВыполнениеКоманды_ОтменаОперацииИЗавершение(РезультатВыполнения, ПараметрыВыполнениеКоманды) Экспорт
	...
	Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяПлатежныеСистемы() Тогда
		МодульОборудованиеПлатежныеСистемыКлиентСервер = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеПлатежныеСистемыКлиентСервер");
		МодульОборудованиеПлатежныеСистемыВызовСервера = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеПлатежныеСистемыВызовСервера");
...
Процедура ВыполнитьНастройкуОборудования_Завершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Иначе
			...
			Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(СообщениеОбОшибке);
		КонецЕсли;
	...
	Если ДанныеОчереди.НомерЧека < ДанныеОчереди.ЧекиВОчереди.Количество() Тогда
		...
		Если ЧекВОчереди.СтатусЧека = ПредопределенноеЗначение("Перечисление.СтатусЧекаККТВОчереди.Новый") 
			...
			Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяЧекопечатающиеУстройства() Тогда
				...
				МодульОборудованиеЧекопечатающиеУстройстваВызовСервера = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеЧекопечатающиеУстройстваВызовСервера");
				МодульОборудованиеЧекопечатающиеУстройстваКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеЧекопечатающиеУстройстваКлиент");
...
Процедура НачатьЗагрузкуДанныеИзТСД(ОповещениеПриЗавершении, УникальныйИдентификатор, СворачиватьДанные = Ложь) Экспорт
	...
	Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяТерминалыСбораДанных() Тогда
		МодульОборудованиеТерминалыСбораДанныхКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеТерминалыСбораДанныхКлиент");
...
Процедура НачатьВыгрузкуДанныеВТСД(ОповещениеПриЗавершении, УникальныйИдентификатор, ТаблицаВыгрузкиТоваров, ПолнаяВыгрузка = Истина) Экспорт
	...
	Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяТерминалыСбораДанных() Тогда              
		МодульОборудованиеТерминалыСбораДанныхКлиентСервер = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеТерминалыСбораДанныхКлиентСервер");
...
Процедура НачатьПолученияВесаСЭлектронныхВесов(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Ложь) Экспорт
	...
	Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяВесовоеОборудование() Тогда
		МодульОборудованиеВесовоеОборудованиеКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеВесовоеОборудованиеКлиент");
...
Процедура НачатьУстановкуВесаТары(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, Вес = Неопределено) Экспорт
	...
	Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяВесовоеОборудование() Тогда
		...
		МодульОборудованиеВесовоеОборудованиеКлиентСервер = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеВесовоеОборудованиеКлиентСервер");
		МодульОборудованиеВесовоеОборудованиеКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОборудованиеВесовоеОборудованиеКлиент");
...
Функция ЗамерВремениБПО(КлючеваяОперация, ПользовательскоеОповещение = Неопределено, ФиксироватьСОшибкой = Ложь, АвтоЗавершение = Ложь) Экспорт
	...
	Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяОценкаПроизводительности() Тогда
		...
		МодульОценкаПроизводительностиКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОценкаПроизводительностиКлиент");
...
//
// Параметры:
//  Результат - Структура - см. МенеджерОборудованияКлиентСервер.ПараметрыВыполненияОперацииНаОборудовании()
...
Функция ИспользуетсяБСП() Экспорт
	...
	Возврат Спец_ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.Пользователи");
КонецФункции
...
Процедура УстановитьКомпонентуИзМакета(Результат, Контекст) Экспорт
	...
	Если Не Результат.Установлено Тогда
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Ошибка установки компоненты: %1'"), Результат.ОписаниеОшибки));
	КонецЕсли;
	...
	ОповещениеУстановки = Новый ОписаниеОповещения("УстановитьКомпонентуИзМакета", ЭтотОбъект, Контекст);
	ПодключаемоеОборудованиеДрайверКлиент.УстановитьКомпонентуИзМакета(ОповещениеУстановки, ТекущийМакет);
...
Процедура ЗавершениеВыполнениеКоманды(РезультатОперации, ПараметрыВыполнениеКоманды);
	...
	Если ПараметрыВыполнениеКоманды.СообщенияВСлужбуТехническойПоддержки 
		...
		МодульСообщенияВСлужбуТехническойПоддержкиБПОКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("СообщенияВСлужбуТехническойПоддержкиБПОКлиент");
...
//   * ЛогФайлВключен - Структура - флаг включен ли лог файл
//   * ПутьКЛогФайлу  - Структура - путь к лог файлу
//   * ПечататьДокументы - Структура - Печатать документы на кассовой ленте
//   * ПечататьСлипЧек - Структура - печатать слип чек на эквайринговом терминале
...
//   * СписокВыбора - Список значений - возможные варианты значения параметра
// 
		...
		Если ЗначениеЗаполнено(ОписаниеКоманды.Обработчик) Тогда
			Если СтрНачинаетсяС(ОписаниеКоманды.Обработчик, "e1cib") Тогда
				...
				Если Спец_ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.Пользователи") Тогда
					МодульФайловаяСистемаКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ФайловаяСистемаКлиент");
			...
			Иначе
				...
				Иначе
					КлиентскийМодуль = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль(МассивПодстрок[0]);
