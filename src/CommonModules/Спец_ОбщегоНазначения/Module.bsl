...
Функция ПолучитьСтруктуруВходящегоПотокаHTTPСервисЗапроса(Знач HTTPСервисЗапрос) Экспорт
	...
	Если Не ПустаяСтрока(UID) И Метаданные.Справочники.Найти("Магазины") <> Неопределено Тогда
		
		Попытка
			
			СтруктураПотока.Отправитель = СериализаторXDTO.XMLЗначение(Тип("СправочникСсылка.Магазины"), UID);
			...
			Запрос = Новый Запрос(
					...
					|	ИСТИНА
					|ИЗ
					|	Справочник.Магазины КАК ТаблицаОбъекта
					|ГДЕ
					|	ТаблицаОбъекта.Ссылка = &Ссылка");
			
			Запрос.УстановитьПараметр("Ссылка", СтруктураПотока.Отправитель);
			
			Если Запрос.Выполнить().Пустой() Тогда
				СтруктураПотока.Отправитель = Неопределено;
			КонецЕсли;
