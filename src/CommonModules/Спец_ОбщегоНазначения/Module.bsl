...
Процедура ЗаписатьОшибки(Знач Источник, Знач ТекстОшибкиИлиИнформацияОбОшибке, Знач УровеньБазы = Неопределено, Знач ТолькоЗаписьОшибки = Ложь,
	...
	Если ИсключитьКэш Тогда
		...
		НаборЗаписей.ДополнительныеСвойства.Вставить("Спец_ПропуститьПроверки", Истина);
...
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
//  ТолькоПрочитать - Булево - Если ИСТИНА, то значение по умолчанию не будет записано (если в регистре нет значения)
// 
// Возвращаемое значение:
//  Произвольный
//
Функция ПолучитьКонстантуСпец(Знач КодИлиСсылкаПВХ, Знач ЗначениеПоУмолчанию = Неопределено, Знач НаДату = Неопределено, Кэш = Неопределено, Знач ТолькоПрочитать = Ложь) Экспорт
	
	Возврат ?(ТипЗнч(КодИлиСсылкаПВХ) = Тип("ПланВидовХарактеристикСсылка.Спец_Константы"),
			ПолучитьКонстантуСпецПоПВХ(КодИлиСсылкаПВХ, ЗначениеПоУмолчанию, НаДату, Кэш, ТолькоПрочитать),
			ПолучитьКонстантуСпецПоКоду(КодИлиСсылкаПВХ, ЗначениеПоУмолчанию, НаДату, Ложь, Кэш, ТолькоПрочитать));
...
Процедура УстановитьКонстантуСпец(Знач КодИлиСсылкаПВХ, Знач Значение, Знач ХранитьИсторию = Истина, Кэш = Неопределено) Экспорт
	...
	ПВХ = ?(ТипЗнч(КодИлиСсылкаПВХ) = Тип("ПланВидовХарактеристикСсылка.Спец_Константы"),
			КодИлиСсылкаПВХ,
			ПолучитьКонстантуСпецПоКоду(КодИлиСсылкаПВХ, Значение, ПериодКонстанты, Истина, Кэш, Ложь));
...
// Обновляем иерархию и наименования предопределённых элементов в соответствии с тем, как они указаны в конфигурации.
// 
// Параметры:
//  ОбъектМетаданныхИлиПолноеИмя - см. ПолучитьСтруктуруИлиТаблицуОбъектаДоЗаписи.ОбъектМетаданных
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
//
Процедура ОбновитьПредопределенныеЭлементыВСоответствииСФайламиКонфигурации(Знач ОбъектМетаданныхИлиПолноеИмя, Кэш = Неопределено) Экспорт
	
	СтруктураПоМетаданным = ПолучитьСтруктуруИмениТипаДанныхОбъекта(ОбъектМетаданныхИлиПолноеИмя, Кэш);
	Если СтруктураПоМетаданным.МетаданныеОбъекта.ПолучитьИменаПредопределенных().Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	КаталогТекущейКонфигурации = ПолучитьКаталогФайловТекущейКонфигурации();
	Если ПустаяСтрока(КаталогТекущейКонфигурации) Тогда
		Возврат;
	КонецЕсли;
	
	МассивГотовоеИмяВXML = Новый Массив; // Массив из Строка
	Для Каждого Имя Из СтрРазделить(СтруктураПоМетаданным.ПолноеИмяОбъекта, ".", Ложь) Цикл
		МассивГотовоеИмяВXML.Добавить(Спец_РаботаСКоллекциямиКлиентСервер.СвойствоСтруктуры(СтруктураИменОбъектовНаРусскомИАнглийском(), Имя, Имя));
	КонецЦикла;
	
	ГотовыйФайл = Новый Файл(КаталогТекущейКонфигурации + СтрСоединить(МассивГотовоеИмяВXML, ".") + ".Predefined.xml");
	Если ГотовыйФайл.Существует() Тогда
		
		СтруктураФайлаПредопределенных = Спец_ОбщегоНазначенияКлиентСервер.ПолучитьСоответствиеXML(ГотовыйФайл.ПолноеИмя, Истина, Ложь);
		Если Спец_РаботаСКоллекциямиКлиентСервер.ЭтоСтруктура(СтруктураФайлаПредопределенных.item) Тогда
			
			ОбработатьСтруктуруПредопределенногоЭлементаРекурсивно(СтруктураПоМетаданным, СтруктураФайлаПредопределенных.item, Неопределено, Кэш);
			
		Иначе
			
			Для Каждого СтруктураПредопределенногоЭлемента Из СтруктураФайлаПредопределенных.item Цикл
				ОбработатьСтруктуруПредопределенногоЭлементаРекурсивно(СтруктураПоМетаданным, СтруктураПредопределенногоЭлемента, Неопределено, Кэш);
			КонецЦикла;
	...
	Спец_ОбщегоНазначенияКлиентСервер.УдалитьФайлыНеМешаяРаботе(КаталогТекущейКонфигурации);
	
КонецПроцедуры

// Возвращает каталог с сохранённой текущей конфигурацией в формате XML (всё, кроме общих макетов). Файлы XML выгружаются в формате Plain, т.е. без иерархии.
// Обновляет раз в день, данные хранятся в регистре "Кэш для фоновых заданий"
// 
// Параметры:
//  ОбновитьПринудительно - Булево - если ИСТИНА, то конфигурация в кэше будет обновлена принудительно.
//
// Возвращаемое значение:
//  Строка - Каталог с файлами XML
//
Функция ПолучитьКаталогФайловТекущейКонфигурации(Знач ОбновитьПринудительно = Ложь) Экспорт
	
	СтруктураТекущейБазы = Спец_ОбщегоНазначенияКлиентСерверПовтИсп.СтруктураПараметровБазы();
	ИдентификаторДляКэша = СтруктураТекущейБазы.ИмяБазы + "_conf";
	КаталогРезультат     = КаталогВременныхФайлов() + ИдентификаторДляКэша + ПолучитьРазделительПути();
	
	УдалитьФайлы(КаталогРезультат);
	СоздатьКаталог(КаталогРезультат);
	
	НаборЗаписей = РегистрыСведений.Спец_КэшДляФоновыхЗаданий.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ИдентификаторЗадания.Установить(ИдентификаторДляКэша);
	
	НаборЗаписей.Прочитать();
	Если НаборЗаписей.Количество() = 0 Или НаборЗаписей.Получить(0).ВыходныеДанные.Получить() <= ТекущаяДатаСеанса() - Спец_КонстантыКлиентСервер.СекундВСутках() Тогда
		
		// Обновляем текущую конфигурацию в кэше
		СтрокаКоманды = СтрШаблон(
				"""%(1)1cv8"" DESIGNER /S""%2\%3"" /N""%4"" /P""%5"" /DumpConfigToFiles""%6"" -Format Plain /AppAutoCheckVersion+ /DisableStartupMessages /DisableStartupDialogs /DisableSplash /DisableUnrecoverableErrorMessage",
				КаталогПрограммы(),
				СтруктураТекущейБазы.АдресСервераИБ,
				СтруктураТекущейБазы.ИмяБазы,
				Спец_ПолучитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ИмяПользователяПоУмолчанию, ""),
				Спец_ПолучитьКонстанту(ПланыВидовХарактеристик.Спец_Константы.ПарольПользователяПоУмолчанию, ""),
				КаталогРезультат);
		...
		Если КодВозврата <> 0 Тогда
			ВызватьИсключение "Не удалось сохранить конфигурацию!";
		КонецЕсли;
		
		// Записываем данные в регистр
		МассивФайловВКаталоге = НайтиФайлы(КаталогРезультат, "*");
		МассивФайловДляЗаписи = Новый Массив(); // Массив из Структура
		
		Для Каждого ФайлКонфигурации Из МассивФайловВКаталоге Цикл
			
			// Файлы общих макетов не выгружаем
			Если СтрНачинаетсяС(ФайлКонфигурации.Имя, "CommonTemplate.") Или СтрНачинаетсяС(ФайлКонфигурации.Имя, "ConfigDumpInfo.") Тогда
				Продолжить;
			КонецЕсли;
			
			СтруктураФайла = Новый Структура("ИмяФайла, ДвоичныеДанныеФайла",
					ФайлКонфигурации.Имя,
					Новый ДвоичныеДанные(ФайлКонфигурации.ПолноеИмя));
			
			МассивФайловДляЗаписи.Добавить(СтруктураФайла);
		...
		НоваяЗапись.ИдентификаторЗадания           = ИдентификаторДляКэша;
		НоваяЗапись.ВходныеДанные                  = Новый ХранилищеЗначения(МассивФайловДляЗаписи, Новый СжатиеДанных(9));
		НоваяЗапись.ВыходныеДанные                 = Новый ХранилищеЗначения(ТекущаяДатаСеанса(), Новый СжатиеДанных(9));
		НоваяЗапись.Выполнено                      = Истина;
		НоваяЗапись.ИмяВыполняемогоФоновогоЗадания = "Здесь хранятся файлы XML текущей конфигурации";
		НоваяЗапись.Прогресс                       = 100;
		
		ЗаписатьОбъект(НаборЗаписей);
		
	Иначе
		
		Для Каждого СтруктураФайла Из НаборЗаписей.Получить(0).ВходныеДанные.Получить() Цикл
			СтруктураФайла.ДвоичныеДанныеФайла.Записать(КаталогРезультат + СтруктураФайла.ИмяФайла);
		КонецЦикла;
...
Функция ОбъектМодифицирован(Знач ОбъектДляПроверки, Знач СтруктураРеквизитовОтбор = Неопределено, Кэш = Неопределено) Экспорт
	...
	Если Кэш = Неопределено Тогда Кэш = Спец_ПолучитьКэш(); КонецЕсли;
	Если Спец_ОбработчикиПодписокНаСобытия.ПропуститьВсеПроверкиПоОбъекту(ОбъектДляПроверки, Неопределено, Кэш) Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ПолучитьКэш() Экспорт
	...
	Если АдресКэшКомпонент <> Неопределено И ЭтоАдресВременногоХранилища(АдресКэшКомпонент) Тогда
		...
		// Здесь может возникнуть <непростительная> "Ошибка формата потока"
		// Если я правильно понял, то это из-за того, что если не обращаться к временному хранилищу >20 минут,
		// то данные сбрасываются на диск, а нам такого нельзя (в кэше метаданные и внешние компоненты)
		// Решением будет только не доводить до такого (обращаться чаще, чем раз в 20 минут).
		// Попытка не спасает, сразу падает сеанс (ну или серверный вызов)
...
// Возвращаемое значение:
//	Структура из КлючЗначение:
//		* Ключ - Строка - Наименование метаданных в 1С
//		* Значение - Строка - Наименование метаданных в XML
//
Функция СтруктураИменОбъектовНаРусскомИАнглийском()
	
	Результат = Новый Структура;
	
	Результат.Вставить("РегистрБухгалтерии", "AccountingRegister");
	Результат.Вставить("РегистрНакопления", "AccumulationRegister");
	Результат.Вставить("БизнесПроцесс", "BusinessProcess");
	Результат.Вставить("РегистрРасчета", "CalculationRegister");
	Результат.Вставить("Справочник", "Catalog");
	Результат.Вставить("ПланСчетов", "ChartOfAccounts");
	Результат.Вставить("ПланВидовРасчета", "ChartOfCalculationTypes");
	Результат.Вставить("ПланВидовХарактеристик", "ChartOfCharacteristicTypes");
	Результат.Вставить("ГруппаКоманд", "CommandGroup");
	Результат.Вставить("ОбщийРеквизит", "CommonAttribute");
	Результат.Вставить("ОбщаяКоманда", "CommonCommand");
	Результат.Вставить("ОбщаяФорма", "CommonForm");
	Результат.Вставить("ОбщийМодуль", "CommonModule");
	Результат.Вставить("ОбщаяКартинка", "CommonPicture");
	Результат.Вставить("ОбщийМакет", "CommonTemplate");
	Результат.Вставить("Константа", "Constant");
	Результат.Вставить("Обработка", "DataProcessor");
	Результат.Вставить("ОпределяемыйТип", "DefinedType");
	Результат.Вставить("Документ", "Document");
	Результат.Вставить("ЖурналДокументов", "DocumentJournal");
	Результат.Вставить("НумераторДокументов", "DocumentNumerator");
	Результат.Вставить("Перечисление", "Enum");
	Результат.Вставить("ПодпискаНаСобытие", "EventSubscription");
	Результат.Вставить("ПланОбмена", "ExchangePlan");
	Результат.Вставить("КритерийОтбора", "FilterCriterion");
	Результат.Вставить("ФункциональнаяОпция", "FunctionalOption");
	Результат.Вставить("ПараметрФункциональныхОпций", "FunctionalOptionsParameter");
	Результат.Вставить("РегистрСведений", "InformationRegister");
	Результат.Вставить("Язык", "Language");
	Результат.Вставить("Отчет", "Report");
	Результат.Вставить("Роль", "Role");
	Результат.Вставить("РегламентноеЗадание", "ScheduledJob");
	Результат.Вставить("Последовательность", "Sequence");
	Результат.Вставить("ПараметрСеанса", "SessionParameter");
	Результат.Вставить("ХранилищеНастроек", "SettingsStorage");
	Результат.Вставить("Стиль", "Style");
	Результат.Вставить("ЭлементСтиля", "StyleItem");
	Результат.Вставить("Подсистема", "Subsystem");
	Результат.Вставить("Задача", "Task");
	Результат.Вставить("WebСервис", "WebService");
	Результат.Вставить("WSСсылка", "WSReference");
	Результат.Вставить("ПакетXDTO", "XDTOPackage");
	Результат.Вставить("HTTPСервис", "HTTPService");
	
	Возврат Результат;
	
КонецФункции

Процедура ОбработатьСтруктуруПредопределенногоЭлементаРекурсивно(Знач СтруктураПоМетаданным, Знач СтруктураПредопределенногоЭлемента, Знач Родитель, Кэш)
	
	// Ищем все предопределённые по имени. Если их несколько - оставляем один любой. Если нет вообще - создаём
	Запрос = Новый Запрос;
	Запрос.Текст =
	...
	|ИЗ
	|	" + СтруктураПоМетаданным.ПолноеИмяОбъекта + " КАК ТаблицаОбъекта
	|ГДЕ
	|	ТаблицаОбъекта.ИмяПредопределенныхДанных = &ИмяПредопределенныхДанных";
	
	Запрос.УстановитьПараметр("ИмяПредопределенныхДанных", СтруктураПредопределенногоЭлемента.Name);
	
	МассивСсылокРезультат = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"); // Массив из ЛюбаяСсылка
	Если МассивСсылокРезультат.Количество() > 1 Тогда
		
		ПравильнаяСсылка = МассивСсылокРезультат.Получить(0); // ЛюбаяСсылка
		Спец_РаботаСКоллекциямиКлиентСервер.УдалитьЗначениеИзМассива(МассивСсылокРезультат, ПравильнаяСсылка);
		УдалитьДублиПоСсылке(ПравильнаяСсылка, МассивСсылокРезультат, Кэш, Ложь);
		
		ПравильнаяСсылкаОбъект = ПолучитьОбъектСБлокированием(ПравильнаяСсылка, Кэш);
		
	ИначеЕсли МассивСсылокРезультат.Количество() = 1 Тогда
		
		ПравильнаяСсылкаОбъект = ПолучитьОбъектСБлокированием(МассивСсылокРезультат.Получить(0), Кэш);
		
	Иначе // МассивСсылокРезультат.Количество() = 0
		
		Если СтруктураПредопределенногоЭлемента.IsFolder = "true" Тогда
			ПравильнаяСсылкаОбъект = СтруктураПоМетаданным.МенеджерОбъекта.СоздатьГруппу();
		Иначе
			ПравильнаяСсылкаОбъект = СтруктураПоМетаданным.МенеджерОбъекта.СоздатьЭлемент();
		КонецЕсли;
		
		ПравильнаяСсылкаОбъект.ИмяПредопределенныхДанных = СтруктураПредопределенногоЭлемента.Name;
		
	КонецЕсли;
	
	Если СтруктураПоМетаданным.МетаданныеОбъекта.ДлинаКода > 0 Тогда
		ПравильнаяСсылкаОбъект.Код = СтруктураПредопределенногоЭлемента.Code;
	КонецЕсли;
	Если СтруктураПоМетаданным.МетаданныеОбъекта.ДлинаНаименования > 0 Тогда
		ПравильнаяСсылкаОбъект.Наименование = СтруктураПредопределенногоЭлемента.Description;
	КонецЕсли;
	Если СтруктураПоМетаданным.МетаданныеОбъекта.Иерархический Тогда
		ПравильнаяСсылкаОбъект.Родитель = Родитель;
	КонецЕсли;
	
	ЗаписатьОбъект(ПравильнаяСсылкаОбъект, , , , Кэш);
	
	Если СтруктураПредопределенногоЭлемента.Свойство("ChildItems") И СтруктураПредопределенногоЭлемента.ChildItems.Количество() > 0 Тогда
		
		Если Спец_РаботаСКоллекциямиКлиентСервер.ЭтоСтруктура(СтруктураПредопределенногоЭлемента.ChildItems.Item) Тогда
			
			ОбработатьСтруктуруПредопределенногоЭлементаРекурсивно(СтруктураПоМетаданным,
					СтруктураПредопределенногоЭлемента.ChildItems.Item,
					ПравильнаяСсылкаОбъект.Ссылка,
					Кэш);
			
		Иначе
			
			Для Каждого СтруктураПодчиненногоПредопределенного Из СтруктураПредопределенногоЭлемента.ChildItems.Item Цикл
				
				ОбработатьСтруктуруПредопределенногоЭлементаРекурсивно(СтруктураПоМетаданным,
						СтруктураПодчиненногоПредопределенного,
						ПравильнаяСсылкаОбъект.Ссылка,
						Кэш);
...
//  НаДату - Дата
//  Кэш - см. Спец_КлиентСерверГлобальный.Спец_ПолучитьКэш
//  ТолькоПрочитать - см. ПолучитьКонстантуСпец.ТолькоПрочитать
// 
// Возвращаемое значение:
//  см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
//
Функция ПолучитьКонстантуСпецПоПВХ(Знач ПВХ, Знач ЗначениеПоУмолчанию, Знач НаДату, Знач Кэш, Знач ТолькоПрочитать)
...
////	|	ХранимыеЗначенияКонстант.Период УБЫВ";
	...
	ИначеЕсли ТолькоПрочитать Тогда
		Возврат ПолучитьКонстантуСпецОбработатьЗначениеРезультат(ЗначениеПоУмолчанию, Кэш);
	Иначе
		Возврат ПолучитьКонстантуСпецЗаписатьЗначениеПоПВХ(ПВХ, ЗначениеПоУмолчанию, НаДату, ТипЗнч(ЗначениеПоУмолчанию), Кэш);
	КонецЕсли;
...
//  см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
//
Функция ПолучитьКонстантуСпецПоКоду(Знач Код, Знач ЗначениеПоУмолчанию, Знач НаДату, Знач ВернутьПВХ, Знач Кэш, Знач ТолькоПрочитать)
	...
	Возврат ?(ВГраница = 0,
			ПолучитьКонстантуСпецПоКодуБезРодителей(НормальныйКод, ЗначениеПоУмолчанию, НаДату, ВернутьПВХ, Кэш, ТолькоПрочитать),
			ПолучитьКонстантуСпецПоКодуСРодителями(МассивКодов, ВГраница, ЗначениеПоУмолчанию, НаДату, ВернутьПВХ, Кэш, ТолькоПрочитать));
...
//  см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
//
Функция ПолучитьКонстантуСпецПоКодуБезРодителей(Знач Код, Знач ЗначениеПоУмолчанию, Знач НаДату, Знач ВернутьПВХ, Знач Кэш, Знач ТолькоПрочитать)
...
//  см. ПолучитьКонстантуСпец.ЗначениеПоУмолчанию
//
Функция ПолучитьКонстантуСпецПоКодуСРодителями(Знач МассивКодов, Знач ВГраница, Знач ЗначениеПоУмолчанию, Знач НаДату, Знач ВернутьПВХ, Знач Кэш, Знач ТолькоПрочитать)
...
Функция НайтиСоздатьТекущегоПользователя()
	...
	НужноУстановитьПараметр = ПолучитьЗначениеПеременной("ЭтоНачалоРаботы1С") = Неопределено;
	Если НужноУстановитьПараметр = Неопределено Тогда
		
		УстановитьЗначениеПеременной("ЭтоНачалоРаботы1С", Истина);
		ОбновитьПовторноИспользуемыеЗначения();
		
	КонецЕсли;
	
	ПользовательРезультат = НайтиСоздатьПользователяПоИмени(Спец_ОбщегоНазначенияКлиентСервер.ТекущийПользователь1С());
	
	Если НужноУстановитьПараметр Тогда
		
		УстановитьЗначениеПеременной("ЭтоНачалоРаботы1С", Ложь);
		ОбновитьПовторноИспользуемыеЗначения();
		
	КонецЕсли;
	
	Возврат ПользовательРезультат;
