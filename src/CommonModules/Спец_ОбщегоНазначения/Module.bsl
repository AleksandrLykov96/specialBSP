...
Функция ПолучитьСтруктуруПараметровДляЗапуска() Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыСеанса.Спец_ТекущийПользователь) Тогда
		...
		ТаблицаТекущихПараметров = ПолучитьТаблицуТекущихПараметровУстановкиКомпонент();
		СоответствиеМоихВерсий = Спец_КомпонентаДополнительныеФункцииКлиентСерверПовтИсп.ПолучитьСоответствиеВерсийМоихКомпонент();
		
		Для Каждого КлючЗначение Из СоответствиеМоихВерсий Цикл
			
			СтрокаТекущийПараметр = ТаблицаТекущихПараметров.Найти(КлючЗначение.Ключ, "НаименованиеКомпоненты");
			Если СтрокаТекущийПараметр = Неопределено
				Или (КлючЗначение.Значение.НомерВерсии <> СтрокаТекущийПараметр.НомерПоследнейУстановленнойВерсии
						И Не СтрокаТекущийПараметр.КомпонентаНеРаботает) Тогда
				
				СтруктураРезультат.СоответствиеКомпонентДляПереустановки[КлючЗначение.Ключ] = НоваяСтруктураРаботыСКомпонентами(
						КлючЗначение.Значение.НаименованиеФункцииДляПодключения,
						КлючЗначение.Значение.НомерВерсии);
...
Процедура УстановитьПараметрыСеанса(Знач ИменаПараметровСеанса) Экспорт
	...
	Если ИменаПараметровСеанса = Неопределено Тогда
		...
		ТекущийПользовательСсылка = Справочники.Спец_Пользователи.НайтиПоНаименованию(ИмяПользователя, Истина); // СправочникСсылка.Спец_Пользователи
		...
		Иначе
			...
			НужныйСтиль = ЗначениеРеквизитаОбъекта(ТекущийПользовательСсылка, "ОсновнойСтиль"); // Строка
...
//  	* ИмяКомпьютераКлиента - Строка
//  	* КомпонентаНеРаботает - Булево
//  	* НомерПоследнейУстановленнойВерсии - Строка
//
Функция ПолучитьТаблицуТекущихПараметровУстановкиКомпонент()
	
	Запрос = Новый Запрос();
	Запрос.Текст =
	
	"ВЫБРАТЬ
	|	ПользователиПараметрыРаботыСКомпонентами.НаименованиеКомпоненты,
	|	ПользователиПараметрыРаботыСКомпонентами.ИмяКомпьютераКлиента,
	|	ПользователиПараметрыРаботыСКомпонентами.КомпонентаНеРаботает,
	|	ПользователиПараметрыРаботыСКомпонентами.НомерПоследнейУстановленнойВерсии
	|ИЗ
