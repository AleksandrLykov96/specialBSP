...
Функция ПолучитьКоличествоСтрокРезультатДинамическогоСписка(Знач ЭлементСписок) Экспорт
	...
	Настройки             = ЭлементСписок.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	...
	ПозицияИз = СтрНайти(Запрос.Текст, "ИЗ" + Символы.ПС, НаправлениеПоиска.СКонца);
	Если ПозицияИз = 0 Тогда
		Возврат КоличествоСтрокРезультат;
	КонецЕсли;
	
	ПозицияВыбрать = СтрНайти(Запрос.Текст, "ВЫБРАТЬ ", НаправлениеПоиска.СКонца, ПозицияИз);
	Если ПозицияВыбрать = 0 Тогда
		Возврат КоличествоСтрокРезультат;
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
			Сред(Запрос.Текст, ПозицияВыбрать, ПозицияИз - ПозицияВыбрать - 1),
			"ВЫБРАТЬ КОЛИЧЕСТВО(*) КАК Спец_КоличествоСтрокРезультат");
	
	ПозицияУпорядочить = СтрНайти(Запрос.Текст, "УПОРЯДОЧИТЬ ПО", НаправлениеПоиска.СКонца);
	Если ПозицияУпорядочить > 0 Тогда
		Запрос.Текст = Лев(Запрос.Текст, ПозицияУпорядочить);
	КонецЕсли;
	
	Для Каждого ПараметрДляУстановки Из МакетКомпоновки.ЗначенияПараметров Цикл
		Запрос.УстановитьПараметр(ПараметрДляУстановки.Имя, ПараметрДляУстановки.Значение);
	КонецЦикла;
