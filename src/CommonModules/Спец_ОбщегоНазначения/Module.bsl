...
//
Процедура ОбработкаЗаполненияОбъектаПоСтруктуре(Знач Источник, Знач ДанныеЗаполнения) Экспорт
...
Процедура ВнешниеКомпонентыДляПереустановки(СоответствиеКомпонентДляПереустановки)
	...
	|				ПО втПользователиТекущегоКомпьютера.Пользователь = ПользователиПараметрыРаботыСКомпонентами.Ссылка) КАК ТекущиеДанные
	...
	Пока ВыборкаПоНекорректнымДанным.Следующий() Цикл
		...
		СтруктураОтбора = Новый Структура("ИмяКомпьютераКлиента, ИмяПользователяОС", ИмяКомпьютераКлиента, ИмяПользователяОС);
	...
	Для Каждого СтрокаДляДобавления Из ТаблицаДанныеПоУстановленнымКомпонентам Цикл
		...
		Если Не СтрокаДляДобавления.КомпонентаНеРаботает Тогда
			Продолжить;
		КонецЕсли;
