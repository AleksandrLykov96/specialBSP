...
Функция НайтиСоздатьОбъектыРегистра(Знач ИсходныеДанные, Знач СтруктураПараметров, ТекстОшибок = "", Знач Кэш = Неопределено, Знач ТолькоНапрямую = Ложь) Экспорт
	...
	Если Не ПропуститьЗаписьНапрямую И Спец_РаботаСБазойДанныхНапрямуюВызовСервераПовтИсп.ЗаписьНапрямуюДоступна() Тогда
	...
	Если Спец_РаботаСБазойДанныхНапрямуюВызовСервераПовтИсп.ЗаписьНапрямуюДоступна() Тогда
		...
		Попытка
			...
			#Если Клиент Тогда
				...
				Если СтруктураРезультат <> Неопределено И СтруктураРезультат.Успешно Тогда
