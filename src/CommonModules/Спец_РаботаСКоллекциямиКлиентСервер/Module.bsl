...
Функция ВыгрузитьМассивЗначенийИзТаблицыЗначений(Знач ТаблицаЗначений, Знач ИмяКолонки, Знач ТолькоУникальныеЗначения, Знач ИсключаяПустые, Знач СтруктураОтбораДляВыгрузки)
	...
	Если ИсключаяПустые Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДляВыгрузки Цикл Попытка Если Не ЗначениеЗаполнено(СтрокаТаблицы[ИмяКолонки]) Тогда МассивСтрокДляУдаления.Добавить(СтрокаТаблицы); КонецЕсли; Исключение КонецПопытки; КонецЦикла;
		Для Каждого СтрокаДляУдаления Из МассивСтрокДляУдаления Цикл ТаблицаДляВыгрузки.Удалить(СтрокаДляУдаления); КонецЦикла;
