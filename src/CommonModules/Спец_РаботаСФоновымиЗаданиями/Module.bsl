...
Процедура ЗапуститьВыполнениеФоновогоЗадания(Знач СтруктураПараметровВыполнения, Знач ВремяОжидания = 3600, Знач ВремяОграничениеЕслиУжеЗапущено = 0) Экспорт
	...
	Спец_Проверить(ФоновоеЗадание <> Неопределено, "Не удалось запустить фоновое задание! Скорее всего, такое задание уже выполняется.", "Иницализация фонового задания: " + ИмяРегламентногоЗадания);
...
Функция ПолучитьФоновоеЗаданиеПоИдентификатору(Знач ИдентификаторЗадания)
	...
	Спец_Проверить(ФоновоеЗадание <> Неопределено, "1С подчистила хвосты, фоновое задание безвозвратно потеряно: " + Строка(ИдентификаторЗадания));
