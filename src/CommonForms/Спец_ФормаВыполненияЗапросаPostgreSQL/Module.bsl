...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "PostgreSQL" Тогда
		...
		ОбщееВремяВыполнения = Строка(Спец_КомпонентаДополнительныеФункцииКлиентСервер.ЗавершитьЗамер("НачатьВыполнениеЗапроса", Спец_ПолучитьКэш())) + " сек. ";
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ИдетВыполнениеЗапроса Тогда
		Отказ = Истина;
	КонецЕсли;
