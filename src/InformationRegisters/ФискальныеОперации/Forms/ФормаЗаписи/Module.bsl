...
Процедура ОтправитьНаПочту(Команда)
	...
	Если ЭлектроннаяПочта = ПустаяСтрокаПочты() Тогда   
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Электронная почта не указана.'"));
		Возврат;
	КонецЕсли;
	...
	Если МодульРассылкаЭлектронныхЧековВызовСервера <> Неопределено Тогда
		...
		МодульРассылкаЭлектронныхЧековВызовСервера.НачатьОтправкуЭлектронногоЧека(
			...
			ЭлектроннаяПочта,
			"");
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Электронный чек отправлен.'"));
	КонецЕсли;
...
Процедура СохранитьPDF_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не Результат Тогда
		Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось записать файл.'"));
	КонецЕсли;
