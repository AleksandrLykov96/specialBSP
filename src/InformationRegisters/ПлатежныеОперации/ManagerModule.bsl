...
Процедура ОчиститьРегистрЗаПрошлыеМесяцы(ДатаОчистки) Экспорт
	...
	Исключение
		...
		МенеджерОборудования.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Очистка платежных операций'", Метаданные.ОсновнойЯзык.КодЯзыка));
	КонецПопытки
	...
	Если ЗначениеЗаполнено(ИдентификаторЗапросаСтрока) Тогда
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Ошибка записи в журнал логирования платежных операций: %1'", Метаданные.ОсновнойЯзык.КодЯзыка), 
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			МенеджерОборудования.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Платежные операции'",  Метаданные.ОсновнойЯзык.КодЯзыка),
					ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
