...
Процедура КомандаПолнаяТаблица_ЗарегистрироватьВсеОбъектыУПолучателя(Команда)
	...
	СтруктураЗапроса = Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктуруЗапроса("hs/specialExchange/changes/register/" + ИмяТаблицыУПолучателя(Объект.База, ПолнаяТаблица_ИмяТаблицы),
...
Процедура ЗаполнитьДанныеФормыПоБазе_ЗарегистрированныеОбъектыОтправитель_ИнициализироватьЗапросНапрямую()
	...
	Если Кэш.КэшПараметровЗаполненияТаблиц["ТекстЗапросаЗарегистрированныеОбъектыОтправитель"] = Неопределено Тогда
		...
		"SELECT MAX(to_timestamp(unixTimestamp)::timestamp) AS ПоследняяДатаИзменения,
		|	objectRef AS ОбъектСсылочный,
		|	objectReg AS ОбъектРегистр,
		|	metadataName AS ПолноеИмяОбъектаМетаданных,
		|	priority AS Приоритет,
		|	itsReg AS ЭтоРегистр
		|FROM (
		...
		|GROUP BY ОбъектСсылочный, ОбъектРегистр, ПолноеИмяОбъектаМетаданных, Приоритет, ЭтоРегистр
		|ORDER BY ПоследняяДатаИзменения
		|LIMIT 50000";
...
#Если Не ВебКлиент Тогда
	...
	Для Каждого СтрокаРезультат Из СтруктураРезультат.РезультатЗапроса Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаРезультат, "ПолноеИмяОбъектаМетаданных, Приоритет, ЭтоРегистр");
		
		НоваяСтрока.ПоследняяДатаИзменения = Спец_СтроковыеФункцииКлиентСервер.СтрокаВДату(СтрокаРезультат.ПоследняяДатаИзменения, Кэш);
		Если НоваяСтрока.ЭтоРегистр Тогда
			
			НоваяСтрока.Объект = СтрокаРезультат.ОбъектРегистр;
		...
		Иначе
			...
			НоваяСтрока.Объект              = СериализаторXDTO.XMLЗначение(Тип(НужноеИмяТипа), СтрокаРезультат.ОбъектСсылочный);
...
Функция ЗаполнитьДанныеФормыПоБазе_ЗарегистрированныеОбъектыПолучатель_ЗаполнитьЧерезHTTP_СформироватьЗапрос(Знач База)
	...
	Возврат Спец_КомпонентаДополнительныеФункцииКлиентСервер.HTTPClientПолучитьСтруктуруЗапроса("hs/specialExchange/changes/getNewToView",
...
Процедура ЗаполнитьДанныеФормыПоБазе_Ошибки_ИнициализироватьЗапросНапрямую()
	...
	Если Кэш.КэшПараметровЗаполненияТаблиц["ТекстЗапросаОшибки"] = Неопределено Тогда
		...
		"SELECT baseTable.dateTime AS ДатаОшибки,
		|	baseTable.source AS Источник,
		|	baseTable.errors AS ТекстОшибки
		|FROM (
...
#Если Не ВебКлиент Тогда
	...
	Для Каждого СтруктураСтроки Из СтруктураРезультат.РезультатЗапроса Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтруктураСтроки, "Источник, ТекстОшибки");
		
		НоваяСтрока.ДатаОшибки = XMLЗначение(
				Тип("Дата"),
				Лев(СтруктураСтроки.ДатаОшибки, 10) + "T" + Сред(СтруктураСтроки.ДатаОшибки, 12));
...
Процедура ЗапуститьОбмен_ПослеВыполнения(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Спец_ОбщегоНазначенияКлиент.СообщитьПользователю(
			"Общее время выполнения: " + Спец_КомпонентаДополнительныеФункцииКлиентСервер.ЗавершитьЗамер("ЗапускОбменов", Спец_ПолучитьКэш()) + " сек.",
			, , , ,
...
Функция ИмяТаблицыУПолучателя(Знач База, Знач ИмяТаблицыОтправитель)
	...
	Возврат Спец_ОбщегоНазначения.ПолучитьСтруктуруПервойВыборкиЗапроса(Запрос).ПолноеИмяОбъектаМетаданныхПриемник;
