...
// Параметры:
//    Ссылка - ПланОбменаСсылка - Ссылка на узел обмена
//    Данные - Структура - Список имен реквизитов для чтения через запятую
//
...
Функция ПроверитьКорректностьНастроек(КлючНастройки = "") Экспорт
	...
	ИначеЕсли НРег(Прав(СокрЛП(НастройкаАдресВнешнейОбработкиЗапросов), 4)) = ".epf" Тогда
		...
		Исключение
			...
			Результат.НастройкаАдресВнешнейОбработкиЗапросов = Спец_ОбщегоНазначенияКлиентСервер.КраткоеПредставлениеОшибкиЧерезОбработкуОшибок(Информация);
...
Процедура ЗарегистрироватьИзменения(Знач Узел, Знач ОбъектРегистрации)
	...
	МодульОбменДаннымиСобытия = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОбменДаннымиСобытия");
...
Функция СтрокаВЧисло(Знач Текст)
	...
	Возврат Спец_СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Текст);
КонецФункции
...
Функция ОбщегоНазначенияОбщийМодуль(Знач ИмяМодуля)
	...
	Возврат Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль(ИмяМодуля);
	// -- 03.03.2022 Лыков А.А.
