...
Процедура РеквизитыОбъектаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	ТекстДляВставки = СтрШаблон(ШаблонВставки, ОписаниеРеквизита.Представление);
	...
	Для Индекс = 0 По РеквизитыОбъекта.Количество() - 1 Цикл
		...
		Если ОписаниеРеквизита.Изменять И ТипЗнч(ОписаниеРеквизита.Значение) = Тип("Строка") И СтрНачинаетсяС(ОписаниеРеквизита.Значение, "=") Тогда
			...
			Если ЕстьОшибкаВВыражении(ОписаниеРеквизита.Значение, ТекстОшибки) Тогда
				...
				Сообщение.Поле = СтрШаблон("РеквизитыОбъекта[%1].Значение", Формат(Индекс, "ЧГ=0"));
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	Заголовок = СтрШаблон(ШаблонЗаголовка, ПредставлениеТипа, КоличествоОбъектов);
	...
	УчитыватьИерархию = ОбъектМетаданныхИерархический(Параметры.МассивОбъектов[0], Кэш);
	ИерархияГрупп = ИерархияГруппИЭлементов(Параметры.МассивОбъектов[0], Кэш);
...
Процедура РазрешитьРедактированиеРеквизитов(ВыбранныйРеквизит)
	...
	Иначе
		...
		Если Спец_ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
			МодульЗапретРедактированияРеквизитовОбъектовКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ЗапретРедактированияРеквизитовОбъектовКлиент");
...
Процедура ИзменитьПорциюОбъектов()
	...
	Иначе
		МодульДлительныеОперацииКлиент = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ДлительныеОперацииКлиент");
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ШаблонСообщения = СтрШаблон(НСтр("ru = 'Изменения выполнены во всех выбранных элементах (%2).'"), ТекущееСостояниеИзменения.КоличествоОбъектовДляОбработки);
	Иначе
		...
		Иначе
			ШаблонСообщения = СтрШаблон(НСтр("ru = 'Изменения выполнены частично.
	...
	Элементы.НадписьРезультатыОбработки.Заголовок = ШаблонСообщения;
...
Функция ТипыИзменяемыхОбъектов()
	...
	Для Каждого ВидОбъектов Из СтрРазделить(ВидыИзменяемыхОбъектов, ",", Ложь) Цикл
		МенеджерОбъекта = МенеджерОбъектаПоПолномуИмени(ВидОбъектов, Кэш);
...
Процедура ЗавершающиеДействияПриИзмененииСервер()
	Если ДлительнаяОперация.Свойство("ИдентификаторЗадания") Тогда
		МодульДлительныеОперации = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ДлительныеОперации");
...
Функция ИзменяемыеРеквизиты(ИмяТабличнойЧасти = "РеквизитыОбъекта")
	...
	Возврат Спец_ОбщегоНазначения.ПреобразоватьТаблицуЗначенийВМассивСтрок(ТаблицаРеквизитов.Выгрузить(Новый Структура("Изменять", Истина)));
КонецФункции
...
Процедура ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	ПараметрыЗадания.Вставить("ДоступныеРеквизиты", Спец_ОбщегоНазначения.ПреобразоватьТаблицуЗначенийВМассивСтрок(РеквизитыОбъекта.Выгрузить(, "Имя,Представление,ВидОперации,Свойство")));
	...
	Если Не Объект.ИзменятьВТранзакции Или Не Спец_ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.БазоваяФункциональность") Тогда
...
&НаСервереБезКонтекста
Функция ОбъектМетаданныхИерархический(СсылкаПервого, Кэш = Неопределено)
	
	ВидОбъектаПоСсылке = ВидОбъектаПоСсылке(СсылкаПервого, Кэш);
...
&НаСервереБезКонтекста
Функция ИерархияГруппИЭлементов(СсылкаПервого, Кэш = Неопределено)
...
Функция ВыбранныеОбъекты(НастройкиОтбора = Неопределено, ТекстСообщенияОбОшибке = "")
	...
	Если Не ПустаяСтрока(ВидыИзменяемыхОбъектов) Тогда
		...
		Исключение
			ТекстСообщенияОбОшибке = Спец_ОбщегоНазначенияКлиентСервер.КраткоеПредставлениеОшибкиЧерезОбработкуОшибок(ИнформацияОбОшибке());
			Возврат Результат;
		КонецПопытки;
...
Функция БлокируемыеРеквизитыОбъекта(ИмяОбъекта)
	...
	Если Спец_ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
		МодульЗапретРедактированияРеквизитовОбъектов = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ЗапретРедактированияРеквизитовОбъектов");
		...
		Если ВерсияБСПСоответствуетТребованиям Тогда
			...
			Если ТипЗнч(ДоступныеМетоды) = Тип("Массив") И (ДоступныеМетоды.Количество() = 0
				Или ДоступныеМетоды.Найти("РеквизитыНеРедактируемыеВГрупповойОбработке") <> Неопределено) Тогда
					МенеджерОбъекта = МенеджерОбъектаПоПолномуИмени(ВидИзменяемыхОбъектов, Кэш);
...
Функция ФильтруемыеРеквизиты()
	...
	Для Каждого ИмяТипа Из СтрРазделить(ВидыИзменяемыхОбъектов, ",", Ложь) Цикл
		...
		ФильтруемыеРеквизиты = ФильтрРедактированияПоТипу(ОбъектМетаданных, Кэш);
...
//   ФильтруемыеРеквизиты - Массив из Строка
//   ДоступныеРеквизиты - Массив из Строка
// Возвращаемое значение:
	...
	Если Спец_ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		...
		Если МодульУправлениеСвойствами <> Неопределено Тогда
			...
			Для Каждого ВидОбъекта Из СписокВидовИзменяемыхОбъектов Цикл
				МенеджерОбъекта = МенеджерОбъектаПоПолномуИмени(ВидОбъекта, Кэш);
...
Процедура ДобавитьДополнительныеРеквизитыИСведенияВНабор()
	...
	Если КонтекстныйВызов Тогда
		...
		Для Каждого ДанныеОбъекта Из ВыбранныеОбъекты Цикл
			...
			ВидОбъектаПоСсылке = ВидОбъектаПоСсылке(ОбъектДляИзменения, Кэш);
...
Процедура ДобавитьСвойствоВСписокРеквизитов(Свойство)
	ОписаниеСвойства = Спец_ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Свойство, "Ссылка,Наименование,ТипЗначения,ЭтоДополнительноеСведение");
	...
	Для Каждого ОписаниеРеквизита Из Реквизиты Цикл
		...
		Иначе
			...
			Для Каждого Тип Из ОписаниеРеквизита.Тип.Типы() Цикл
				Если Спец_ОбщегоНазначения.ЭтоСсылочныйТип(Тип, Тип) Тогда
					ЭтоСсылка = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
//
&НаСервере
Функция ФильтрРедактированияПоТипу(ОбъектМетаданных, Кэш)
	...
	ФильтрПоТипуОбъекта = ФильтрТаблица.НайтиСтроки(Новый Структура("ТипОбъекта", 
		ИмяБазовогоТипаПоОбъектуМетаданных(ОбъектМетаданных, Кэш)));
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Иначе
		...
		Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
			Пояснение = СтрШаблон(ШаблонПояснения, ИзменяемыеРеквизиты.Количество());
		ИначеЕсли ИзменяемыеРеквизиты.Количество() > 0 Тогда
			Пояснение = СтрШаблон(ШаблонПояснения, ИменаРеквизитов(ИзменяемыеРеквизиты));
		Иначе
			Пояснение = "";
		КонецЕсли;
		...
		Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
			...
			КонецЕсли;
			Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
				Пояснение = Пояснение + СтрШаблон(ШаблонПояснения, 
	...
	Если ПустаяСтрока(КодАлгоритма) Тогда
		КодАлгоритма = СтрШаблон(НСтр("ru='// Доступные переменные:
...
Функция ПредставлениеСвязейПараметровВыбора(СвязиПараметровВыбора, ОбъектМетаданных)
	...
	Если СвязанныеРеквизиты.Количество() > 0 Тогда
		...
		КонецЕсли;
		Результат = СтрШаблон(ШаблонПредставленияСвязей, СтрСоединить(СвязанныеРеквизиты, ", "));
	КонецЕсли;
...
//
&НаСервереБезКонтекста
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя, Кэш)
	
	СтруктураПоМетаданным = Спец_ОбщегоНазначения.ПолучитьСтруктуруИмениТипаДанныхОбъекта(ПолноеИмя, Кэш);
...
//
&НаСервереБезКонтекста
Функция ВидОбъектаПоСсылке(Ссылка, Кэш = Неопределено)
	
	Возврат ВидОбъектаПоТипу(ТипЗнч(Ссылка), Кэш);
...
// 
&НаСервереБезКонтекста
Функция ВидОбъектаПоТипу(Тип, Кэш = Неопределено)
	
	СтруктураПоМетаданным = Спец_ОбщегоНазначения.ПолучитьСтруктуруИмениТипаДанныхОбъекта(Тип, Кэш);
...
Функция ОбъектЯвляетсяГруппой(Объект)
	...
	Кэш = Спец_ПолучитьКэш();
	Если Спец_ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта, Кэш, Тип("ОбъектМетаданных")) Тогда
	...
	ИначеЕсли НЕ Спец_ОбщегоНазначения.ЭтоПланВидовХарактеристик(МетаданныеОбъекта, Кэш, Тип("ОбъектМетаданных")) Тогда
		Возврат Ложь;
	...
	Возврат Спец_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ЭтоГруппа", , Кэш);
...
//
&НаСервереБезКонтекста
Функция ИмяБазовогоТипаПоОбъектуМетаданных(ОбъектМетаданных, Кэш = Неопределено)
	
	СтруктураПоТипу = Спец_ОбщегоНазначения.ПолучитьСтруктуруИмениТипаДанныхОбъекта(ОбъектМетаданных, Кэш);
	Возврат СтруктураПоТипу.ТипОбъектаМножественноеЧисло;
