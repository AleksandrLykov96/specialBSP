...
//	Оповещение = Новый ОписаниеОповещения("УстановитьКорневыеСертификаты_Завершение", ЭтотОбъект);
//	ЭлектронныеСертификатыКлиент.НачатьПроверкуДоверенныхСертификатов(Оповещение);
...
&НаКлиенте
Процедура УстановитьКорневыеСертификаты(Команда)
//	Оповещение = Новый ОписаниеОповещения("УстановитьКорневыеСертификаты_Завершение", ЭтотОбъект);
//	ЭлектронныеСертификатыКлиент.НачатьУстановкуДоверенныхСертификатов(Оповещение);
КонецПроцедуры

&НаКлиенте
Процедура УстановитьКорневыеСертификаты_Завершение(РезультатОперации, ДополнительныеПараметры) Экспорт
...
//		СписокСертификатов = ЭлектронныеСертификатыКлиент.СписокДоверенныхСертификатов();
//		Текст = "";
...
//		КонецЦикла;
//		Элементы.ГруппаКорневыеСертификатыНеУстановлены.Видимость = Ложь;
//		Элементы.ГруппаКорневыеСертификатыУстановлены.Видимость   = Истина;
//		Элементы.ДекорацияКорневыеСертификатыУстановленыРасширеннаяПодсказка.Заголовок = Текст;
//		
//	Иначе
//		Элементы.ГруппаКорневыеСертификатыНеУстановлены.Видимость = Истина;
//		Элементы.ГруппаКорневыеСертификатыУстановлены.Видимость   = Ложь;
//		Элементы.УстановитьКорневыеСертификатыРасширеннаяПодсказка.Заголовок = РезультатОперации.ОписаниеОшибки;
//	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьВидимостьРазделов()
	...
	Если МенеджерОборудованияВызовСервера.ИспользуетсяЭлектронныеСертификатыНСПК() Тогда
		МодульЭлектронныеСертификаты = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ЭлектронныеСертификаты");
		Элементы.ГруппаКорневыеСертификаты.Видимость = МодульЭлектронныеСертификаты.ВерсияБСПДопустимаДляУстановкиКорневыхСертификатов();
	Иначе
		Элементы.ГруппаКорневыеСертификаты.Видимость = Ложь;
	КонецЕсли;
...
Процедура ИзменитьРасписаниеЗадания(ИмяЗадания, РасписаниеРегламентногоЗадания)
	...
	Если Спец_ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.РегламентныеЗадания") Тогда
		
		МодульРегламентныеЗаданияСервер = Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль("РегламентныеЗаданияСервер");
...
Процедура УстановитьТекстНадписиРегламентнойНастройки(Задание, Элемент)
	...
	Иначе
		Если Задание.Метаданные = Метаданные.РегламентныеЗадания.ОчисткаИсторииОперацийОчередиЧеков Тогда
			ТекстЗадания = НСтр("ru = 'Расписание очистки очереди чеков'");
		ИначеЕсли Задание.Метаданные = Метаданные.РегламентныеЗадания.ОчисткаИсторииПлатежныхОпераций Тогда
			ТекстЗадания = НСтр("ru = 'Расписание очистки платежных операций'");
		ИначеЕсли Задание.Метаданные = Метаданные.РегламентныеЗадания.ОчисткаИсторииФискальныхОпераций Тогда
			ТекстЗадания = НСтр("ru = 'Расписание очистки фискальных операций'");
		Иначе
