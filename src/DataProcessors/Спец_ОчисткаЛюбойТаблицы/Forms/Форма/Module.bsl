...
Процедура КомандаОчистить(Команда)
	...
	ТипДляОчистки = Спец_ОбщегоНазначенияКлиентПовтИсп.ПолучитьСтруктуруИмениТипаДанныхОбъекта(ИмяТаблицы).Тип; 
	Если ТипДляОчистки <> Тип("Неопределено") Тогда
		ОповеститьОбИзменении(ТипДляОчистки);
	КонецЕсли;
