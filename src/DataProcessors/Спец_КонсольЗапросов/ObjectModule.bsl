...
Функция ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, ТДРезультатаЗапроса, ПараметрыВыводаЗапроса, ОтчетПоВыполнениюЗапроса, МеткаЗапроса) Экспорт
	...
	Если Кэш.КомпонентаBaseFunction = Неопределено Тогда
		...
		ОтчетПоВыполнениюЗапроса.ВремяВыполнения = (ТекущаяУниверсальнаяДатаВМиллисекундах() - Начало) / 1000;
		
	Иначе
		
		Кэш.КомпонентаBaseFunction.НачатьЗамер("Погрешность");
		Погрешность = Кэш.КомпонентаBaseFunction.ЗавершитьЗамер("Погрешность");
		
		Кэш.КомпонентаBaseFunction.НачатьЗамер("Запрос");
		МассивЗапросов = Запрос.ВыполнитьПакет();
		ОтчетПоВыполнениюЗапроса.ВремяВыполнения = Кэш.КомпонентаBaseFunction.ЗавершитьЗамер("Запрос") - Погрешность;
...
Функция ЭтоСсылка(Тип) Экспорт
	...
	// ++ 21.01.2022 Лыков А.А.
	Возврат Спец_ОбщегоНазначения.ЭтоСсылка(Тип);
	// -- 21.01.2022 Лыков А.А.
...
Функция ПутьККонфигурационномуФайлу()
	...
	ФайлУказатель = Новый Файл(КаталогаОбщихКонфигурационныхФайлов + ПолучитьРазделительПути() + "conf.cfg");
	...
	Для каждого БлокКонфигурации Из СтруктураФайлаКонфигурации.БлокиКонфигурации Цикл
		...
		Исключение
			...
			ТекстОшибки = СтрЗаменить(НСтр("ru = 'Не удалось прочитать конфигурационный файл logcfg.xml по причине: %1'"),
				"%1", Спец_ОбщегоНазначенияКлиентСервер.ПодробноеПредставлениеОшибкиЧерезОбработкуОшибок(ИнформацияОбОшибке()));
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	// ++ 03.03.2022 Лыков А.А.
	Возврат Спец_ОбщегоНазначенияКлиентСервер.ОбщийМодуль(Имя);
	// -- 03.03.2022 Лыков А.А.
...
Функция ИмяСУБД()
	...
	// ++ 21.01.2022 Лыков А.А.
	Возврат Спец_ОбщегоНазначенияПовтИсп.СтруктураПараметровБазы().ИмяБазы;
	// -- 21.01.2022 Лыков А.А.
